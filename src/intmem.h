...
#ifdef _MSC_VER
  #include <stdlib.h>
static uint8_t bswap_rt(uint8_t V) {
  return V;
}
static uint16_t bswap_rt(unsigned short V) {
  return _byteswap_ushort(V);
}
static_assert(sizeof(uint32_t) == sizeof(unsigned long),
              "unsigned long isn't 32-bit wide!");
static uint32_t bswap_rt(uint32_t V) {
  return _byteswap_ulong(V);
}
static uint64_t bswap_rt(uint64_t V) {
...
static uint16_t bswap_rt(uint16_t V) {
...
static INTMEM_CE int8_t bswap(int8_t V) {
  return V;
}
