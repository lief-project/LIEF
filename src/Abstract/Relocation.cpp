...
uint64_t Relocation::address() const { return address_; }

size_t Relocation::size() const { return size_; }

void Relocation::address(uint64_t address) { address_ = address; }

void Relocation::size(size_t size) { size_ = static_cast<uint8_t>(size); }
...
std::ostream& operator<<(std::ostream& os, const Relocation& entry) {
  os << std::hex;
  os << std::left << std::setw(10) << entry.address() << std::setw(4)
     << std::dec << entry.size();
  return os;
}
