...
VectorStream::VectorStream(std::vector<uint8_t> data)
    : binary_{std::move(data)}, size_{binary_.size()} {
  stype_ = STREAM_TYPE::VECTOR;
}

result<const void*> VectorStream::read_at(uint64_t offset,
                                          uint64_t size) const {
...
result<size_t> VectorStream::asn1_read_tag(int tag) {
  ...
  increment_pos(reinterpret_cast<uintptr_t>(p) -
...
result<std::string> VectorStream::asn1_read_alg() {
  ...
  ret =
      mbedtls_oid_get_numeric_string(oid_str.data(), oid_str.size(), &alg_oid);
  if (ret <= 0) {
    return make_error_code(lief_errors::read_error);
  }
...
result<std::string> VectorStream::asn1_read_oid() {
  ...
  int ret =
      mbedtls_oid_get_numeric_string(oid_str.data(), oid_str.size(), &buf);
  if (ret == MBEDTLS_ERR_OID_BUF_TOO_SMALL) {
    LIEF_DEBUG(
        "asn1_read_oid: mbedtls_oid_get_numeric_string return "
...
const std::vector<uint8_t>& VectorStream::content() const { return binary_; }
