...
size_t inst_size_from_format(INST_FORMATS fmt) {
  static const std::map<INST_FORMATS, size_t> size_map{
      {INST_FORMATS::F_00x, SIZE_MAX}, {INST_FORMATS::F_10x, 2},
...
bool is_switch_array(const uint8_t* ptr, const uint8_t* end) {
  ...
  const size_t underlying_struct_size =
      std::min({sizeof(packed_switch), sizeof(sparse_switch),
  ...
  const auto ident = SWITCH_ARRAY_IDENT(uint16_t(ptr[1] << 8) | opcode);
...
size_t switch_array_size(const uint8_t* ptr, const uint8_t* end) {
  ...
  const auto remaining = size_t(end - ptr);
  const auto opcode = OPCODES{*ptr};
  const auto ident = SWITCH_ARRAY_IDENT(uint16_t(ptr[1] << 8) | opcode);
