...
ok_error_t Builder::build() {
  const char* type =
      ((binary_->type_ == ELF_CLASS::ELFCLASS32) ? "ELF32" : "ELF64");
  ...
  switch (file_type) {
    ...
    case E_TYPE::ET_CORE: {
      ...
      if (!res) {
        LIEF_ERR(
            "The builder failed for the given executable/library. "
    ...
    case E_TYPE::ET_REL: {
      ...
      if (!res) {
        LIEF_ERR(
            "The builder failed for the given object file. "
            "Check the error output");
        return make_error_code(lief_errors::build_error);
      }
      return ok();
...
ok_error_t Builder::build_exe_lib() {
  ...
  if (pt_interp != nullptr) {
    ...
    if (interpt_size > pt_interp->physical_size() ||
        config_.force_relocations) {
      ...
      LIEF_DEBUG(".interp: -0x{:x} bytes",
                 pt_interp->physical_size() - interpt_size);
    }

  } else if (!binary_->interpreter_
                  .empty()) {  // Access private field directly as we want to
  ...
  if (dt_gnu_hash != nullptr) {
    ...
    if (section != nullptr) {
      if (needed_size > section->size() || config_.force_relocations) {
        ...
        LIEF_DEBUG("{}: -0x{:x} bytes", section->name(),
                   section->size() - needed_size);
      }
    } else {
  ...
  if (binary_->has(SEGMENT_TYPES::PT_DYNAMIC)) {
    ...
    if (dynamic_needed_size > section->size() || config_.force_relocations) {
      ...
      LIEF_DEBUG("{}: -0x{:x} bytes", section->name(),
                 section->size() - dynamic_needed_size);
    }
  }
  ...
  if (dt_rela != nullptr || dt_rel != nullptr) {
    const size_t dyn_reloc_needed_size =
        layout->dynamic_relocations_size<ELF_T>();
    ...
    if (section != nullptr) {
      if (dyn_reloc_needed_size > section->size() ||
          config_.force_relocations) {
        LIEF_DEBUG("[-] Need to relocate {} section (0x{:x} new bytes)",
        ...
        LIEF_DEBUG("{}: -0x{:x} bytes", section->name(),
                   section->size() - dyn_reloc_needed_size);
      }
    } else {
  ...
  if (dt_jmprel != nullptr) {
    const size_t plt_reloc_needed_size =
        layout->pltgot_relocations_size<ELF_T>();
    const uint64_t dt_reloc_addr = dt_jmprel->value();
    Section* section = binary_->section_from_virtual_address(dt_reloc_addr);
    if (section != nullptr) {
      if (plt_reloc_needed_size > section->size() ||
          config_.force_relocations) {
        LIEF_DEBUG("[-] Need to relocate {} section (0x{:x} new bytes)",
        ...
        LIEF_DEBUG("{}: -0x{:x} bytes", section->name(),
                   section->size() - plt_reloc_needed_size);
      }
    } else {
  ...
  if (dt_symtab != nullptr) {
    ...
    if (section != nullptr) {
      if (dynsym_needed_size > section->size() || config_.force_relocations) {
        ...
        LIEF_DEBUG("{}: -0x{:x} bytes", section->name(),
                   section->size() - dynsym_needed_size);
      }
    } else {
  ...
  if (dt_init_array != nullptr && dt_init_arraysz != nullptr) {
    const size_t needed_size =
        layout->dynamic_arraysize<ELF_T>(DYNAMIC_TAGS::DT_INIT_ARRAY);
    ...
    if (array_section != nullptr) {
      if (needed_size > array_section->size()) {
        if (binary_->has_symbol("__libc_start_main")) {
          LIEF_WARN(
              "Relocating .init_array on Linux may corrupt the final binary");
        }
        ...
        LIEF_DEBUG("{}: -0x{:x} bytes", array_section->name(),
                   array_section->size() - needed_size);
      }
    } else {
  ...
  if (dt_preinit_array != nullptr && dt_preinit_arraysz != nullptr) {
    const size_t needed_size =
        layout->dynamic_arraysize<ELF_T>(DYNAMIC_TAGS::DT_PREINIT_ARRAY);
  ...
  if (dt_fini_array != nullptr && dt_fini_arraysz != nullptr) {
    const size_t needed_size =
        layout->dynamic_arraysize<ELF_T>(DYNAMIC_TAGS::DT_FINI_ARRAY);
  ...
  if (dt_versym != nullptr) {
    ...
    if (sv_section != nullptr) {
      if (symver_needed_size > sv_section->size() ||
          config_.force_relocations) {
        LIEF_DEBUG(
            "[-] Need to relocate .gnu.version section (0x{:x} new bytes)",
            symver_needed_size - sv_section->size());
        layout->relocate_symver(symver_needed_size);
      } else {
        LIEF_DEBUG("{}: -0x{:x} bytes", sv_section->name(),
                   sv_section->size() - symver_needed_size);
      }
    } else {
  ...
  if (dt_verdef != nullptr) {
    ...
    if (svdef_section != nullptr) {
      if (symvdef_needed_size > svdef_section->size() ||
          config_.force_relocations) {
        LIEF_DEBUG(
            "[-] Need to relocate .gnu.version_d section (0x{:x} new bytes)",
            symvdef_needed_size - svdef_section->size());
        layout->relocate_symverd(symvdef_needed_size);
      } else {
        LIEF_DEBUG("{}: -0x{:x} bytes", svdef_section->name(),
                   svdef_section->size() - symvdef_needed_size);
      }
    } else {
  ...
  if (dt_verneed != nullptr) {
    ...
    if (svreq_section != nullptr) {
      if (symvreq_needed_size > svreq_section->size() ||
          config_.force_relocations) {
        LIEF_DEBUG(
            "[-] Need to relocate .gnu.version_r section (0x{:x} new bytes)",
            symvreq_needed_size - svreq_section->size());
        layout->relocate_symverr(symvreq_needed_size);
      } else {
        LIEF_DEBUG("{}: -0x{:x} bytes", svreq_section->name(),
                   svreq_section->size() - symvreq_needed_size);
      }
    } else {
  ...
  if (header.section_name_table_idx() > 0) {
    ...
    if (shstr_size > string_names_section->size() ||
        config_.force_relocations) {
      LIEF_DEBUG("[-] Need to relocate '{}' section (0x{:x} new bytes)",
  ...
  if (sec_symtab != nullptr) {
    ...
    // added symbols => We have to craft a new section that will contain the
    // symtab
  ...
  if (build_opt_.gnu_hash || build_opt_.dt_hash) {
    ...
    LIEF_SW_END("hast table built in {}",
  ...
  if (build_opt_.dyn_str && dt_strtab != nullptr) {
    const uint64_t dyn_strtab_va = dt_strtab->value();
    Section* dyn_strtab_section =
        binary_->section_from_virtual_address(dyn_strtab_va);
    if (dyn_strtab_section == nullptr) {
      LIEF_ERR(
          "Can't find .dynstr section associated with the virtual address "
          "0x{:x}",
          dyn_strtab_va);
      return make_error_code(lief_errors::file_format_error);
    }
  ...
  if (build_opt_.sym_versym && binary_->has(DYNAMIC_TAGS::DT_VERSYM)) {
    ...
    LIEF_SW_END(".gnu.version built in {}",
                duration_cast<std::chrono::milliseconds>(sw.elapsed()));
  }
  ...
  if (build_opt_.sym_verdef && binary_->has(DYNAMIC_TAGS::DT_VERDEF)) {
    ...
    LIEF_SW_END(".gnu.version_d built in {}",
                duration_cast<std::chrono::milliseconds>(sw.elapsed()));
  }
  ...
  if (build_opt_.sym_verneed && binary_->has(DYNAMIC_TAGS::DT_VERNEED)) {
    ...
    LIEF_SW_END(".gnu.version_r built in {}",
                duration_cast<std::chrono::milliseconds>(sw.elapsed()));
  }
...
ok_error_t Builder::process_object_relocations() {
  ...
  ObjectFileLayout::relocations_map_t& relocations_map =
      layout->relocation_map();
  ObjectFileLayout::rel_sections_size_t& rel_sections_size =
      layout->rel_sections_size();

  for (Section& sec : sections) {
    const ELF_SECTION_TYPES type = sec.type();
    if (type != ELF_SECTION_TYPES::SHT_RELA &&
        type != ELF_SECTION_TYPES::SHT_REL) {
      continue;
    }
...
ok_error_t Builder::build_relocatable() {
  ...
  if (!layout->is_strtab_shared_shstrtab() &&
    ...
    if (sec_symtab == nullptr) {
      LIEF_ERR(
          "Object file without a symtab section is not supported. Please "
...
std::vector<std::string> Builder::optimize(
  ...
  for (const auto& val : string_table) {
    ...
    std::reverse(std::begin(string_table_optimized.back()),
                 std::end(string_table_optimized.back()));
  }

  std::sort(std::begin(string_table_optimized),
  ...
  // as all elements that can be merged are adjacent we can just go through the
  // list once and memorize one we merged to calculate the offsets later
...
ok_error_t Builder::build_sections() {
  ...
  const std::unordered_map<std::string, size_t>& shstr_map =
      layout_->shstr_map();
  for (size_t i = 0; i < binary_->sections_.size(); i++) {
    ...
    if (it == std::end(shstr_map)) {
      LIEF_WARN("Can't find string offset for section name '{}'",
                section->name());
    } else {
    ...
    if (section_headers_offset > 0) {
      ...
      LIEF_DEBUG("[Header ] {:20}: 0x{:010x} - 0x{:010x}", section->name(),
...
ok_error_t Builder::build_segments() {
  ...
  LIEF_DEBUG("sizeof(PHDR): 0x{:x}",
             binary_->segments_.size() * sizeof(Elf_Phdr));
  ...
  for (const std::unique_ptr<Segment>& segment : binary_->segments_) {
    if (segment->physical_size() > 0) {
      ...
      LIEF_DEBUG("[W] {:<13} 0x{:016x}: 0x{:010x} - 0x{:010x} (0x{:x})",
                 ...
                 segment->file_offset(),
                 segment->file_offset() + content.size(), content.size());
  ...
  const Elf_Off segment_header_offset =
      binary_->header().program_headers_offset();

  LIEF_DEBUG("Write segments header 0x{} -> 0x{}", segment_header_offset,
...
ok_error_t Builder::build_static_symbols() {
  ...
  std::stable_sort(std::begin(binary_->static_symbols_),
                   std::end(binary_->static_symbols_),
                   [](const std::unique_ptr<Symbol>& lhs,
  ...
  const auto it_first_exported_symbol = std::find_if(
      std::begin(binary_->static_symbols_), std::end(binary_->static_symbols_),
      [](const std::unique_ptr<Symbol>& sym) { return sym->is_exported(); });

  const auto first_exported_symbol_index = static_cast<uint32_t>(std::distance(
      std::begin(binary_->static_symbols_), it_first_exported_symbol));
  ...
  if (symbol_section->link() == 0 ||
    ...
    LIEF_ERR(
        "Unable to find a string section associated with the symbol section "
        "(sh_link)");
    return make_error_code(lief_errors::file_format_error);
  }
  ...
  // On recent compilers, the symtab string table is merged with the section
  // name table
    ...
    switch (entry->tag()) {
      ...
      case DYNAMIC_TAGS::DT_INIT_ARRAY: {
        if (build_opt_.init_array) {
          Section* array_section =
              binary_->get(ELF_SECTION_TYPES::SHT_INIT_ARRAY);
          ...
          std::vector<uint8_t> array_content(array.size() * sizeof(Elf_Addr),
                                             0);
      ...
      case DYNAMIC_TAGS::DT_FINI_ARRAY: {
        if (build_opt_.fini_array) {
          Section* array_section =
              binary_->get(ELF_SECTION_TYPES::SHT_FINI_ARRAY);
          DynamicEntry* dt_array_size =
      ...
      case DYNAMIC_TAGS::DT_PREINIT_ARRAY: {
        if (build_opt_.fini_array) {
          Section* array_section =
              binary_->get(ELF_SECTION_TYPES::SHT_PREINIT_ARRAY);
          DynamicEntry* dt_array_size =
...
ok_error_t Builder::build_symbol_hash() {
  ...
  std::vector<uint8_t> new_hash_table((nbucket + nchain + 2) * sizeof(uint32_t),
                                      0);
  auto* const new_hash_table_ptr =
      reinterpret_cast<uint32_t*>(new_hash_table.data());
...
ok_error_t Builder::build_hash_table() {
  ...
  if (build_opt_.gnu_hash && gnu_hash_sec != nullptr) {
    ...
    gnu_hash_sec->content(
        static_cast<ExeLayout*>(layout_.get())->raw_gnuhash());
  }
  if (has_error) {
    return make_error_code(lief_errors::build_error);
  }
...
ok_error_t Builder::build_dynamic_symbols() {
  ...
  // Find the section associated with the address
  Section* symbol_table_section =
      binary_->section_from_virtual_address(symbol_table_va);
  if (symbol_table_section == nullptr) {
...
ok_error_t Builder::build_section_relocations() {
  ...
  Binary::it_object_relocations object_relocations =
      binary_->object_relocations();
  ...
  for (const auto& p : layout->relocation_map()) {
    ...
    // sort relocations by offset. It is not required by the ELF standard but
    // some linkers (like ld) rely on this kind of sort for sections such as
    // .eh_frame;
    ...
    for (const Relocation* reloc : relocs) {
      ...
      if (sym != nullptr) {
        const auto it_sym = std::find_if(
            std::begin(binary_->static_symbols_),
            std::end(binary_->static_symbols_),
            [sym](const std::unique_ptr<Symbol>& s) { return s.get() == sym; });
        if (it_sym == std::end(binary_->static_symbols_)) {
          LIEF_WARN("Can find the relocation's symbol '{}'", sym->name());
          continue;
        }

        symidx = static_cast<uint32_t>(
            std::distance(std::begin(binary_->static_symbols_), it_sym));
      }
      ...
      if (std::is_same<ELF_T, details::ELF32>::value) {
        ...
        info = (static_cast<Elf_Xword>(symidx) << 32) |
               (reloc->type() & 0xffffffffL);
      }
  ...
  for (auto& p : section_content) {
    ...
    LIEF_DEBUG("Fill section {} with 0x{:x} bytes", sec->name(),
               ios.raw().size());
    sec->content(ios.raw());
  }
  return ok();
...
ok_error_t Builder::build_dynamic_relocations() {
  ...
  for (const Relocation& relocation : binary_->dynamic_relocations()) {
    ...
    if (symbol != nullptr) {
      const std::string& name = symbol->name();
      const auto it_name =
          std::find_if(std::begin(binary_->dynamic_symbols_),
                       std::end(binary_->dynamic_symbols_),
      ...
      idx = static_cast<uint32_t>(
          std::distance(std::begin(binary_->dynamic_symbols_), it_name));
    }
    ...
    if (std::is_same<ELF_T, details::ELF32>::value) {
      ...
      r_info = (static_cast<Elf_Xword>(info) << 32) |
               (relocation.type() & 0xffffffffL);
    }
...
ok_error_t Builder::build_pltgot_relocations() {
  ...
  Binary::it_pltgot_relocations pltgot_relocations =
  ...
  if (dt_pltrel != nullptr) {
    is_rela =
        dt_pltrel->value() == static_cast<uint64_t>(DYNAMIC_TAGS::DT_RELA);
  }
  DynamicEntry* dt_jmprel = binary_->get(DYNAMIC_TAGS::DT_JMPREL);
  ...
  Section* relocation_section =
      binary_->section_from_virtual_address(dt_jmprel->value());
  if (relocation_section == nullptr) {
  ...
  for (const Relocation& relocation : binary_->pltgot_relocations()) {
    ...
    if (std::is_same<ELF_T, details::ELF32>::value) {
      ...
      info = (static_cast<Elf_Xword>(idx) << 32) |
             (relocation.type() & 0xffffffffL);
    }
...
ok_error_t Builder::build_symbol_requirement() {
  ...
  if (svr_nb != binary_->symbol_version_requirements_.size()) {
    LIEF_WARN(
        "The number of symbol version requirement "
  ...
  for (SymbolVersionRequirement& svr : binary_->symbols_version_requirement()) {
    ...
    SymbolVersionRequirement::it_aux_requirement svars =
        svr.auxiliary_symbols();
    ...
    for (SymbolVersionAuxRequirement& svar : svars) {
      ...
      aux_header.vna_next = static_cast<Elf_Word>(
          svar_idx < (svars.size() - 1) ? sizeof(Elf_Vernaux) : 0);
...
ok_error_t Builder::build_symbol_definition() {
  ...
  if (svd_nb != binary_->symbol_version_definition_.size()) {
    LIEF_WARN(
        "The number of symbol version definition entries "
  ...
  for (const SymbolVersionDefinition& svd :
    ...
    if (svd_idx < (svd_nb - 1)) {
      next_symbol_offset =
          sizeof(Elf_Verdef) + svas.size() * sizeof(Elf_Verdaux);
    }
    ...
    header.vd_aux =
        static_cast<Elf_Word>(!svas.empty() > 0 ? sizeof(Elf_Verdef) : 0);
    ...
    for (const SymbolVersionAux& sva : svas) {
      ...
      aux_header.vda_next = static_cast<Elf_Word>(
          sva_idx < (svas.size() - 1) ? sizeof(Elf_Verdaux) : 0);
...
ok_error_t Builder::build_symbol_version() {
  ...
  if (binary_->symbol_version_table_.size() !=
      binary_->dynamic_symbols_.size()) {
    LIEF_WARN(
        "The number of symbol version is different from the number of dynamic "
        "symbols {} != {}",
