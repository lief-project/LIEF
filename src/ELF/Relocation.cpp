...
Relocation::Relocation(ARCH arch) { architecture_ = arch; }
...
Relocation::Relocation(uint64_t address, uint32_t type, int64_t addend,
                       bool isRela)
...
int64_t Relocation::addend() const { return addend_; }

uint32_t Relocation::type() const { return type_; }
...
bool Relocation::is_rela() const { return isRela_; }

bool Relocation::is_rel() const { return !isRela_; }

ARCH Relocation::architecture() const { return architecture_; }

RELOCATION_PURPOSES Relocation::purpose() const { return purpose_; }

bool Relocation::has_symbol() const { return symbol_ != nullptr; }

bool Relocation::has_section() const { return section_ != nullptr; }

uint32_t Relocation::info() const { return info_; }

size_t Relocation::size() const {
  switch (architecture()) {
    case ARCH::EM_X86_64: {
      const auto it =
          relocation_x86_64_sizes.find(static_cast<RELOC_x86_64>(type()));
      if (it == std::end(relocation_x86_64_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_x86_64>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_386: {
      const auto it =
          relocation_i386_sizes.find(static_cast<RELOC_i386>(type()));
      if (it == std::end(relocation_i386_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_i386>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_ARM: {
      ...
      if (it == std::end(relocation_ARM_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_ARM>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_AARCH64: {
      const auto it =
          relocation_AARCH64_sizes.find(static_cast<RELOC_AARCH64>(type()));
      if (it == std::end(relocation_AARCH64_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_AARCH64>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_MIPS: {
      const auto it =
          relocation_MIPS_sizes.find(static_cast<RELOC_MIPS>(type()));
      if (it == std::end(relocation_MIPS_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_MIPS>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_PPC: {
      const auto it =
          relocation_PPC_sizes.find(static_cast<RELOC_POWERPC32>(type()));
      if (it == std::end(relocation_PPC_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_POWERPC32>(type())));
        return -1u;
      }
      return it->second;
    }
    ...
    case ARCH::EM_PPC64: {
      const auto it =
          relocation_PPC64_sizes.find(static_cast<RELOC_POWERPC64>(type()));
      if (it == std::end(relocation_PPC64_sizes)) {
        LIEF_ERR("{} - {}", to_string(architecture()),
                 to_string(static_cast<RELOC_POWERPC64>(type())));
        return -1u;
      }
      return it->second;
    }
...
void Relocation::addend(int64_t addend) { addend_ = addend; }

void Relocation::type(uint32_t type) { type_ = type; }

void Relocation::info(uint32_t v) { info_ = v; }

void Relocation::symbol(Symbol* sym) { symbol_ = sym; }

void Relocation::section(Section* section) { section_ = section; }

void Relocation::purpose(RELOCATION_PURPOSES purpose) { purpose_ = purpose; }
...
std::ostream& operator<<(std::ostream& os, const Relocation& entry) {
  ...
  os << std::setw(10) << entry.address() << std::setw(10) << relocation_type
     << std::setw(4) << std::dec << entry.size() << std::setw(10) << std::hex
     << entry.addend() << std::setw(10) << std::hex << entry.info()
     << std::setw(10) << to_string(entry.purpose()) << std::setw(10)
     << symbol_name;
