 ...
 public:
  ...
  size_t dynstr_size() {
    ...
    std::vector<std::string> string_table_optimized =
        Builder::optimize<Symbol, decltype(binary_->dynamic_symbols_)>(
            binary_->dynamic_symbols_,
    ...
    LIEF_SW_END(".dynstr values computed in {}",
  ...
  size_t symbol_sysv_hash_size() {
    ...
    if (nchain_ < binary_->dynamic_symbols_.size()) {
      LIEF_DEBUG("nchain of .hash section changes from {:d} to {:d}", nchain_,
  ...
  size_t symbol_gnu_hash_size() {
    ...
    // MANDATORY !
    std::stable_sort(std::begin(binary_->dynamic_symbols_) + symndx,
    ...
    raw_gnuhash.write_conv_array<uint32_t>(buckets).write_conv_array<uint32_t>(
        hash_values);
  ...
  size_t symbol_vdef_size() {
    ...
    for (const SymbolVersionDefinition& svd :
         binary_->symbols_version_definition()) {
      computed_size +=
          sizeof(Elf_Verdef) + svd.symbols_aux().size() * sizeof(Elf_Verdaux);
    }
    return computed_size;
  }
  ...
  inline void relocate_dynamic(uint64_t size) { dynamic_size_ = size; }

  inline void relocate_dynstr(bool val) { relocate_dynstr_ = val; }

  inline void relocate_shstr(bool val) { relocate_shstrtab_ = val; }

  inline void relocate_strtab(bool val) { relocate_strtab_ = val; }

  inline void relocate_gnu_hash(bool val) { relocate_gnu_hash_ = val; }

  inline void relocate_sysv_hash(uint64_t size) { sysv_size_ = size; }

  inline void relocate_dynsym(uint64_t size) { dynsym_size_ = size; }

  inline void relocate_symver(uint64_t size) { sver_size_ = size; }

  inline void relocate_symverd(uint64_t size) { sverd_size_ = size; }

  inline void relocate_symverr(uint64_t size) { sverr_size_ = size; }

  inline void relocate_preinit_array(uint64_t size) { preinit_size_ = size; }

  inline void relocate_init_array(uint64_t size) { init_size_ = size; }

  inline void relocate_fini_array(uint64_t size) { fini_size_ = size; }

  inline void relocate_dyn_reloc(uint64_t size) { dynamic_reloc_size_ = size; }

  inline void relocate_plt_reloc(uint64_t size) { pltgot_reloc_size_ = size; }

  inline void relocate_interpreter(uint64_t size) { interp_size_ = size; }

  inline void relocate_notes(bool value) { relocate_notes_ = value; }

  inline void relocate_symtab(size_t size) { symtab_size_ = size; }

  inline const std::vector<uint8_t>& raw_dynstr() const { return raw_dynstr_; }
  ...
  inline const std::vector<uint8_t>& raw_notes() const { return raw_notes_; }
  ...
  result<bool> relocate() {
    ...
    if (interp_size_ > 0 && !binary_->has(SEGMENT_TYPES::PT_INTERP)) {
      ...
      if (interp == nullptr) {
        LIEF_DEBUG("Could not add a new interp segment.");
        return make_error_code(lief_errors::allocation_error);
      }
      LIEF_DEBUG("Interp Segment: 0x{:x}:0x{:x}", interp->virtual_address(),
                 interp->virtual_size());
    }
    ...
    if (read_segment > 0) {
      ...
      if (new_rsegment == nullptr) {
        LIEF_DEBUG("Could not allocate a new R segment.");
        return make_error_code(lief_errors::allocation_error);
      }

      LIEF_DEBUG("R-Segment: 0x{:x}:0x{:x}", new_rsegment->virtual_address(),
    ...
    if (read_write_segment > 0) {
      ...
      if (new_rwsegment == nullptr) {
        LIEF_DEBUG("Could not allocate a new RW segment.");
        return make_error_code(lief_errors::allocation_error);
      }

      LIEF_DEBUG("RW-Segment: 0x{:x}:0x{:x}", new_rwsegment->virtual_address(),
    ...
    if (interp_size_ > 0) {
      ...
      result<uint64_t> maybe_offset =
          binary_->virtual_address_to_offset(va_r_base);
    ...
    if (relocate_notes_) {
      ...
      note_segment->file_offset(offset);
    ...
    if (dynamic_size_ > 0) {
      ...
      result<uint64_t> maybe_offset =
          binary_->virtual_address_to_offset(va_rw_base);
      ...
      dynamic_segment->file_offset(offset);
    ...
    if (dynamic_reloc_size_ > 0) {
      ...
      DynamicEntry* dt_reloc =
          is_rela ? dt_rela : binary_->get(DYNAMIC_TAGS::DT_REL);
      ...
      Section* dyn_relocation_section =
      ...
      dyn_relocation_section->offset(offset);
    ...
    if (pltgot_reloc_size_ > 0) {
      ...
      relocation_section->offset(offset);
    ...
    if (init_size_ > 0) {  // .init_array
      ...
      if (binary_->header().file_type() == E_TYPE::ET_DYN) {
        ...
        // we need to change the relocation associated with the former
        // .init_array
        ...
        for (size_t i = 0; i < array.size(); ++i) {
          ...
          if (it_reloc == std::end(relocations_addresses_)) {
            LIEF_ERR("Missing relocation for .init_array[{:d}]: 0x{:x}", i,
                     array[i]);
            continue;
          }
      ...
      init_array_section->offset(offset);
    ...
    if (preinit_size_ > 0) {  // .preinit_array
      ...
      if (raw_dt_preinit == nullptr ||
          !DynamicEntryArray::classof(raw_dt_preinit)) {
        LIEF_ERR("DT_PREINIT_ARRAY not found");
        return make_error_code(lief_errors::file_format_error);
      }
      DynamicEntryArray* dt_preinit_array =
          raw_dt_preinit->as<DynamicEntryArray>();
      ...
      if (binary_->header().file_type() == E_TYPE::ET_DYN) {
        ...
        for (size_t i = 0; i < array.size(); ++i) {
          auto it_reloc =
              relocations_addresses_.find(array_base_address + i * sizeof_p);
          if (it_reloc == std::end(relocations_addresses_)) {
            LIEF_ERR("Missing relocation for .preinit_array[{:d}]: 0x{:x}", i,
                     array[i]);
            continue;
          }
          Relocation* reloc = it_reloc->second;
      ...
      preinit_array_section->offset(offset);
    ...
    if (fini_size_ > 0) {  // .fini_array
      ...
      if (binary_->header().file_type() == E_TYPE::ET_DYN) {
        ...
        for (size_t i = 0; i < array.size(); ++i) {
          auto it_reloc =
              relocations_addresses_.find(array_base_address + i * sizeof_p);
          if (it_reloc == std::end(relocations_addresses_)) {
            LIEF_ERR("Missing relocation for .fini_array[{:d}]: 0x{:x}", i,
                     array[i]);
            continue;
          }
          Relocation* reloc = it_reloc->second;
      ...
      fini_array_section->offset(offset);
    ...
    if (relocate_strtab_) {
      ...
      if (is_strtab_shared_shstrtab()) {
        LIEF_ERR("Inconsistency");  // The strtab should be located in the
                                    // .shstrtab section
        return make_error_code(lief_errors::file_format_error);
      }
      ...
      strtab.alignment(1);
      Section* new_strtab_ptr = binary_->add(strtab, /* loaded */ false);
      if (new_strtab_ptr == nullptr) {
        LIEF_DEBUG("Could not allocate a new strtab.");
        return make_error_code(lief_errors::allocation_error);
      }

      // This is safe because we already confirmed it's not nullptr.
      Section& new_strtab = *new_strtab_ptr;
      LIEF_DEBUG("New .strtab section: #{:d} {} 0x{:x} (size: {:x})",
                 strtab_idx, new_strtab.name(), new_strtab.file_offset(),
                 new_strtab.size());

      Section* sec_symtab = binary_->get(ELF_SECTION_TYPES::SHT_SYMTAB);
      if (sec_symtab != nullptr) {
        LIEF_DEBUG("Link section {} with the new .strtab (idx: #{:d})",
    ...
    if (symtab_size_ > 0) {
      ...
      if (sec_symtab != nullptr) {
        ...
        LIEF_DEBUG("Removing the old section: {} 0x{:x} (size: 0x{:x})",
                   sec_symtab->name(), sec_symtab->file_offset(),
                   sec_symtab->size());
      ...
      Section* new_symtab_ptr = binary_->add(symtab, /* loaded */ false);

      if (new_symtab_ptr == nullptr) {
        LIEF_DEBUG("Could not allocate a new symtab.");
        return make_error_code(lief_errors::allocation_error);
      }

      Section& new_symtab = *new_symtab_ptr;
      LIEF_DEBUG("New .symtab section: {} 0x{:x} (size: {:x})",
                 new_symtab.name(), new_symtab.file_offset(),
                 new_symtab.size());
    }
    ...
    if (segment_note != nullptr) {
      ...
      for (const Note& note : binary_->notes()) {
        ...
        if (binary_->has(type) && !has_section) {
          ...
          Section* section_added_ptr = binary_->add(section, /*loaded */ false);

          if (section_added_ptr == nullptr) {
            LIEF_DEBUG("Could not add a new section.");
            return make_error_code(lief_errors::allocation_error);
          }

          Section& section_added = *section_added_ptr;
          section_added.offset(segment_note->file_offset() + note_offset);
          section_added.size(note.size());
          section.virtual_address(segment_note->virtual_address() +
                                  note_offset);
  ...
  inline uint32_t sysv_nchain() const { return nchain_; }
