...
Header::Header(const details::Elf32_Ehdr& header)
  ...
  std::copy(reinterpret_cast<const uint8_t*>(header.e_ident),
            reinterpret_cast<const uint8_t*>(header.e_ident) +
                static_cast<size_t>(IDENTITY::EI_NIDENT),
            std::begin(identity_));
}
...
E_TYPE Header::file_type() const { return file_type_; }

ARCH Header::machine_type() const { return machine_type_; }
...
VERSION Header::object_file_version() const { return object_file_version_; }
...
uint32_t Header::processor_flag() const { return processor_flags_; }
...
bool Header::has(ARM_EFLAGS f) const {
  ...
  switch (f) {
    ...
    case ARM_EFLAGS::EF_ARM_EABI_VER5: {
      return (processor_flag() &
              static_cast<uint32_t>(ARM_EFLAGS::EF_ARM_EABIMASK)) ==
...
Header::arm_flags_list_t Header::arm_flags_list() const {
  ...
  std::copy_if(std::begin(details::arm_eflags_array),
...
bool Header::has(MIPS_EFLAGS f) const {
  ...
  switch (f) {
    ...
    case MIPS_EFLAGS::EF_MIPS_ABI_EABI64: {
      return ((processor_flag() &
               static_cast<uint32_t>(MIPS_EFLAGS::EF_MIPS_ABI)) &
              fn) > 0;
    }
    ...
    case MIPS_EFLAGS::EF_MIPS_MACH_LS3A: {
      return ((processor_flag() &
               static_cast<uint32_t>(MIPS_EFLAGS::EF_MIPS_MACH)) &
              fn) > 0;
    }
    ...
    case MIPS_EFLAGS::EF_MIPS_ARCH_ASE_MDMX: {
      return ((processor_flag() &
               static_cast<uint32_t>(MIPS_EFLAGS::EF_MIPS_ARCH_ASE)) &
              fn) > 0;
    }
    ...
    case MIPS_EFLAGS::EF_MIPS_ARCH_64R6: {
      return (processor_flag() &
              static_cast<uint32_t>(MIPS_EFLAGS::EF_MIPS_ARCH)) == fn;
    }
...
Header::mips_flags_list_t Header::mips_flags_list() const {
  ...
  std::copy_if(std::begin(details::mips_eflags_array),
...
Header::ppc64_flags_list_t Header::ppc64_flags_list() const {
  ...
  std::copy_if(std::begin(details::ppc64_eflags_array),
...
Header::hexagon_flags_list_t Header::hexagon_flags_list() const {
  ...
  std::copy_if(std::begin(details::hexagon_eflags_array),
...
uint32_t Header::program_header_size() const { return program_header_size_; }

uint32_t Header::numberof_segments() const { return numberof_segments_; }

uint32_t Header::section_header_size() const { return section_header_size_; }

uint32_t Header::numberof_sections() const { return numberof_sections_; }

uint32_t Header::section_name_table_idx() const {
  return section_string_table_idx_;
}

const Header::identity_t& Header::identity() const { return identity_; }

Header::identity_t& Header::identity() {
  return const_cast<Header::identity_t&>(
      static_cast<const Header*>(this)->identity());
}

ELF_CLASS Header::identity_class() const {
  return static_cast<ELF_CLASS>(
      identity_[static_cast<size_t>(IDENTITY::EI_CLASS)]);
}

ELF_DATA Header::identity_data() const {
  return static_cast<ELF_DATA>(
      identity_[static_cast<size_t>(IDENTITY::EI_DATA)]);
}

VERSION Header::identity_version() const {
  return static_cast<VERSION>(
      identity_[static_cast<size_t>(IDENTITY::EI_VERSION)]);
}

OS_ABI Header::identity_os_abi() const {
  return static_cast<OS_ABI>(
      identity_[static_cast<size_t>(IDENTITY::EI_OSABI)]);
}

uint32_t Header::identity_abi_version() const {
  return static_cast<uint32_t>(
      identity_[static_cast<size_t>(IDENTITY::EI_ABIVERSION)]);
}

void Header::file_type(E_TYPE type) { file_type_ = type; }

void Header::machine_type(ARCH machineType) { machine_type_ = machineType; }
...
void Header::identity(const std::string& identity) {
  std::copy(std::begin(identity), std::end(identity), std::begin(identity_));
}
...
void Header::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const Header& hdr) {
  ...
  os << std::hex << std::left;
  os << std::setw(33) << std::setfill(' ') << "Magic:" << ident_magic
     << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Class:" << to_string(hdr.identity_class()) << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Endianness:" << to_string(hdr.identity_data()) << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Version:" << to_string(hdr.identity_version()) << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "OS/ABI:" << to_string(hdr.identity_os_abi()) << std::endl;
  os << std::setw(33) << std::setfill(' ') << "ABI Version:" << std::dec
     << hdr.identity_abi_version() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Machine type:" << to_string(hdr.machine_type()) << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "File type:" << to_string(hdr.file_type()) << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Object file version:" << to_string(hdr.object_file_version())
     << std::endl;
  os << std::setw(33) << std::setfill(' ') << "Entry Point:"
     << "0x" << hdr.entrypoint() << std::endl;
  os << std::setw(33) << std::setfill(' ') << "Program header offset:"
     << "0x" << hdr.program_headers_offset() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Section header offset:" << hdr.section_headers_offset() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Processor Flag:" << hdr.processor_flag() << " " << processor_flags_str
     << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Header size:" << hdr.header_size() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Size of program header:" << hdr.program_header_size() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Number of program header:" << hdr.numberof_segments() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Size of section header:" << hdr.section_header_size() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Number of section headers:" << hdr.numberof_sections() << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "Section Name Table idx:" << hdr.section_name_table_idx() << std::endl;
