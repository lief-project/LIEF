...
AndroidNote::AndroidNote(Note& note) : NoteDetails::NoteDetails{note} {}

AndroidNote* AndroidNote::clone() const { return new AndroidNote(*this); }

uint32_t AndroidNote::sdk_version() const { return sdk_version_; }

std::string AndroidNote::ndk_version() const { return ndk_version_; }

std::string AndroidNote::ndk_build_number() const { return ndk_build_number_; }
...
void AndroidNote::parse() {
  ...
  sdk_version_ =
      *reinterpret_cast<const uint32_t*>(desc.data() + sdk_version_offset);

  // Parse NDK Version
  if (desc.size() < (ndk_version_offset + ndk_version_size)) {
    return;
  }
  ndk_version_ = std::string{
      reinterpret_cast<const char*>(desc.data()) + ndk_version_offset,
      ndk_version_size};
  ...
  ndk_build_number_ = std::string{
      reinterpret_cast<const char*>(desc.data()) + ndk_build_number_offset,
...
void AndroidNote::build() {
  ...
  std::copy(reinterpret_cast<const uint8_t*>(&sdk_version_),
  ...
  std::copy(reinterpret_cast<const uint8_t*>(ndk_version_.data()),
  ...
  std::copy(reinterpret_cast<const uint8_t*>(ndk_build_number_.data()),
            reinterpret_cast<const uint8_t*>(ndk_build_number_.data()) +
                ndk_build_number_.size(),
            desc.data() + ndk_build_number_offset);
}

void AndroidNote::accept(Visitor& visitor) const { visitor.visit(*this); }
...
void AndroidNote::dump(std::ostream& os) const {
  os << std::setw(33) << std::setfill(' ') << "SDK Version: " << std::dec
     << sdk_version() << std::endl;
  os << std::setw(33) << std::setfill(' ') << "NDK Version: " << ndk_version()
     << std::endl;
  os << std::setw(33) << std::setfill(' ')
     << "NDK Builder Number: " << ndk_build_number() << std::endl;
}
