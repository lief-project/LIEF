...
std::unique_ptr<X86ISA> X86ISA::create_isa_1(X86ISA::FLAG flag,
                                             BinaryStream& stream) {
  ...
  while (bitmask) {
    ...
    switch (bit) {
      case GNU_PROPERTY_X86_ISA_1_BASELINE:
        values.emplace_back(flag, ISA::BASELINE);
        break;
...
std::unique_ptr<X86ISA> X86ISA::create_compat_isa_1(X86ISA::FLAG flag,
                                                    BinaryStream& stream,
                                                    bool is_compat2) {
  uint32_t bitmask = stream.read<uint32_t>().value_or(0);
    ...
    if (is_compat2) {
      ...
      switch (bit) {
        case GNU_PROPERTY_X86_COMPAT_ISA_1_486:
          values.emplace_back(flag, ISA::I486);
          break;
        case GNU_PROPERTY_X86_COMPAT_ISA_1_586:
          values.emplace_back(flag, ISA::I586);
          break;
        case GNU_PROPERTY_X86_COMPAT_ISA_1_686:
          values.emplace_back(flag, ISA::I686);
          break;
...
std::unique_ptr<X86ISA> X86ISA::create(uint32_t type, BinaryStream& stream) {
  switch (type) {
    ...
    case GNU_PROPERTY_X86_COMPAT_ISA_1_USED:
      return create_compat_isa_1(FLAG::USED, stream, /*is_compat2*/ false);
    case GNU_PROPERTY_X86_COMPAT_ISA_1_NEEDED:
      return create_compat_isa_1(FLAG::NEEDED, stream, /*is_compat2*/ false);

    case GNU_PROPERTY_X86_COMPAT_2_ISA_1_USED:
      return create_compat_isa_1(FLAG::USED, stream, /*is_compat2*/ true);
    case GNU_PROPERTY_X86_COMPAT_2_ISA_1_NEEDED:
      return create_compat_isa_1(FLAG::NEEDED, stream, /*is_compat2*/ true);
