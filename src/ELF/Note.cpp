...
Note::Note(const Note& other)
  ...
  details_ = std::make_pair(
      details.first, std::unique_ptr<NoteDetails>{details.second->clone()});
}
...
Note::Note()
    : type_{NOTE_TYPES::NT_UNKNOWN},
      details_{std::make_pair(NOTE_TYPES::NT_UNKNOWN,
                              std::make_unique<NoteDetails>())} {}

Note::Note(std::string name, uint32_t type, description_t description,
           Binary* binary)
    : binary_{binary},
      name_{std::move(name)},
...
Note::Note(const std::string& name, NOTE_TYPES type,
           const description_t& description, Binary* binary)
    : Note::Note{name, static_cast<uint32_t>(type), description, binary} {}

Note::Note(const std::string& name, NOTE_TYPES_CORE type,
           const description_t& description, Binary* binary)
    : Note::Note{name, static_cast<uint32_t>(type), description, binary} {
  is_core_ = true;
  details();
...
const std::string& Note::name() const { return name_; }

NOTE_TYPES Note::type() const { return type_; }

NOTE_TYPES_CORE Note::type_core() const {
  return static_cast<NOTE_TYPES_CORE>(type_);
}

const Note::description_t& Note::description() const { return description_; }

Note::description_t& Note::description() { return description_; }

bool Note::is_core() const { return is_core_; }

bool Note::is_android() const { return name() == AndroidNote::NAME; }

const NoteDetails& Note::details() const { return *(details_.second); }
...
void Note::name(const std::string& name) { name_ = name; }
...
void Note::accept(Visitor& visitor) const { visitor.visit(*this); }
...
bool Note::operator!=(const Note& rhs) const { return !(*this == rhs); }

void Note::dump(std::ostream& os) const {
  const description_t& desc = description();

  std::string description_str = std::accumulate(
      std::begin(desc), std::begin(desc) + std::min<size_t>(16, desc.size()),
      std::string{}, [](const std::string& a, uint8_t v) {
  ...
  const std::string type_str =
      is_core() ? to_string(type_core()) : to_string(type());
  os << std::setw(33) << std::setfill(' ') << "Type:" << type_str << std::endl;
  os << std::setw(33) << std::setfill(' ') << "Description:" << description_str
     << std::endl;

  if (!is_core()) {
    // GOLD VERSION
    if (type() == NOTE_TYPES::NT_GNU_GOLD_VERSION) {
      std::string version_str{reinterpret_cast<const char*>(desc.data()),
                              desc.size()};
    ...
    if (type() == NOTE_TYPES::NT_GNU_BUILD_ID) {
      std::string hash = std::accumulate(
          std::begin(desc), std::end(desc), std::string{},
      ...
      os << std::setw(33) << std::setfill(' ') << "ID Hash:" << hash
         << std::endl;
    }
  }
