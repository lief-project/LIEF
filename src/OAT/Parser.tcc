...
void Parser::parse_dex_files<OAT88_t>() {
  return parse_dex_files<OAT79_t>();
}

template<>
void Parser::parse_oat_classes<OAT88_t>() {
  return parse_oat_classes<OAT79_t>();
}
...
void Parser::parse_binary<OAT64_t>() {
  ...
  if (oat_binary_->has_symbol("oatdata")) {
    const LIEF::ELF::Symbol* oat_data = oat_binary_->get_symbol("oatdata").as<LIEF::ELF::Symbol>();

    raw_oat.reserve(oat_data->size());

    const std::vector<uint8_t>& raw_data = oat_binary_->get_content_from_virtual_address(oat_data->value(), oat_data->size());
  ...
  if (oat_binary_->has_symbol("oatexec")) {
    const LIEF::ELF::Symbol* oat_exec = oat_binary_->get_symbol("oatexec").as<LIEF::ELF::Symbol>();

    exec_start_ = oat_exec->value();
    exec_size_  = oat_exec->size();

    const std::vector<uint8_t>& raw_oatexec = oat_binary_->get_content_from_virtual_address(oat_exec->value(), oat_exec->size());
  ...
  stream_ = std::unique_ptr<VectorStream>(new VectorStream{std::move(raw_oat)});

  parse_header<OAT64_t>();
  parse_dex_files<OAT64_t>();
  parse_oat_classes<OAT64_t>();
}
...
void Parser::parse_binary<OAT79_t>() {
  ...
  parse_header<OAT79_t>();
  parse_dex_files<OAT79_t>();

  parse_type_lookup_table<OAT79_t>();
  parse_oat_classes<OAT79_t>();
}
...
void Parser::parse_binary<OAT88_t>() {
  ...
  parse_header<OAT88_t>();
  parse_dex_files<OAT88_t>();

  parse_type_lookup_table<OAT88_t>();
  parse_oat_classes<OAT88_t>();
}
...
void Parser::parse_binary<OAT124_t>() {
  ...
  parse_header<OAT124_t>();
  parse_dex_files<OAT124_t>();
  if (has_vdex()) {
    parse_type_lookup_table<OAT124_t>();
    parse_oat_classes<OAT124_t>();
  }
}
...
void Parser::parse_binary<OAT131_t>() {
  ...
  parse_header<OAT131_t>();
  parse_dex_files<OAT131_t>();

  if (has_vdex()) {
    parse_type_lookup_table<OAT131_t>();
    parse_oat_classes<OAT131_t>();
  }
}
...
void Parser::parse_header() {
  ...
  const oat_header& oat_hdr = stream_->peek<oat_header>(0);
  oat_binary_->header_ = &oat_hdr;
  LIEF_DEBUG("Nb dex files: #{:d}", oat_binary_->header_.nb_dex_files());
...
void Parser::parse_header_keys() {
  ...
  const size_t keys_size = oat_binary_->header_.key_value_size();

  std::string key_values;

  const char* keys_start = stream_->peek_array<char>(keys_offset, keys_size, /* check */false);
  ...
  for (HEADER_KEYS key : header_keys_list) {
    ...
    if (pos != std::string::npos) {
      ...
      oat_binary_->header_.dex2oat_context_.emplace(key, value);
    }
  }
}
...
void Parser::parse_type_lookup_table() {
  ...
  //for (size_t i = 0; i < oat_binary_->dex_files_.size(); ++i) {

  //  const DexFile* oat_dex_file = oat_binary_->oat_dex_files_[i];
...
void Parser::parse_oat_classes() {
  ...
  for (size_t dex_idx = 0; dex_idx < oat_binary_->oat_dex_files_.size(); ++dex_idx) {
    DexFile* oat_dex_file = oat_binary_->oat_dex_files_[dex_idx];
    ...
    for (size_t class_idx = 0; class_idx < nb_classes; ++class_idx) {
      ...
      // OAT Status
      OAT_CLASS_STATUS status = static_cast<OAT_CLASS_STATUS>(stream_->read<int16_t>());

      // OAT Type
      OAT_CLASS_TYPES type = static_cast<OAT_CLASS_TYPES>(stream_->read<uint16_t>());
      ...
      if (type == OAT_CLASS_TYPES::OAT_CLASS_SOME_COMPILED) {
        method_bitmap_size = stream_->read<uint32_t>();
        const uint32_t nb_entries = method_bitmap_size / sizeof(uint32_t);

        const uint32_t* raw = stream_->read_array<uint32_t>(nb_entries, /* check */false);
        if (raw != nullptr) {
      ...
      oat_binary_->classes_.emplace(cls.fullname(), oat_class);


      // Methods Offsets
      const uint64_t method_offsets = stream_->pos();
      parse_oat_methods<OAT_T>(method_offsets, oat_class, cls);
    }
  }
}
...
void Parser::parse_oat_methods(uint64_t methods_offsets, Class* clazz, const DEX::Class& dex_class) {
  ...
  for (size_t method_idx = 0; method_idx < methods.size(); ++method_idx) {
    ...
    uint32_t code_off = stream_->peek<uint32_t>(methods_offsets + computed_index * sizeof(uint32_t));
    ...
    if (!stream_->can_read<oat_quick_method_header>(quick_method_header_off)) {
      break;
    }

    const oat_quick_method_header& quick_header = stream_->peek<oat_quick_method_header>(quick_method_header_off);
    ...
    if (quick_header.code_size > 0) {
      ...
      const uint8_t* code = stream_->peek_array<uint8_t>(code_off, quick_header.code_size, /* check */false);
      if (code != nullptr) {
    ...
    if (quick_header.code_size == 0 && vmap_table_offset > 0) {
      ...
      for (size_t pc = 0, round = 0; pc < method.bytecode().size(); ++round) {
        ...
        uint32_t new_pc = static_cast<uint32_t>(stream_->read_uleb128());
        ...
        uint32_t index = static_cast<uint32_t>(stream_->read_uleb128());
        oat_method->dex_method().insert_dex2dex_info(pc, index);
      }

    }
    clazz->methods_.push_back(oat_method.get());
    oat_binary_->methods_.push_back(oat_method.release());
  }
