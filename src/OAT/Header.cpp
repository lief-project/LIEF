...
std::ostream& operator<<(std::ostream& os, const Header& hdr) {
  ...
  os << std::setw(WIDTH) << std::setfill(' ')
     ...
     << hdr.quick_generic_jni_trampoline_offset() << '\n';
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick IMT Conflict Trampoline Offset:" << std::hex
     << hdr.quick_imt_conflict_trampoline_offset() << '\n';
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick Resolution Trampoline Offset:" << std::hex
     << hdr.quick_resolution_trampoline_offset() << '\n';
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick to Interpreter Bridge Offset:" << std::hex
     << hdr.quick_to_interpreter_bridge_offset() << '\n';

  os << '\n';

  os << std::setw(WIDTH) << std::setfill(' ') << "Image Patch Delta:" << std::dec
     << hdr.image_patch_delta() << '\n';

  os << '\n';

  os << std::setw(WIDTH) << std::setfill(' ')
     << "Image File Location OAT Checksum:" << std::hex
     << hdr.image_file_location_oat_checksum() << '\n';
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Image File Location OAT Begin:" << std::hex
     << hdr.image_file_location_oat_data_begin() << '\n';

  os << '\n';

  for (const auto& p : hdr.key_values()) {
    os << std::setw(WIDTH) << std::setfill(' ')
       << Header::key_to_string(p.key) + ":" << *p.value << '\n';
  }
