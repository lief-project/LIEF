...
oat_version_t Header::version() const { return version_; }

INSTRUCTION_SETS Header::instruction_set() const { return instruction_set_; }

uint32_t Header::checksum() const { return checksum_; }

uint32_t Header::nb_dex_files() const { return dex_file_count_; }

uint32_t Header::executable_offset() const { return executable_offset_; }

uint32_t Header::i2i_bridge_offset() const { return i2i_bridge_offset_; }
...
int32_t Header::image_patch_delta() const { return image_patch_delta_; }
...
uint32_t Header::key_value_size() const { return key_value_store_size_; }

uint32_t Header::oat_dex_files_offset() const { return oat_dex_files_offset_; }
...
Header::it_const_key_values_t Header::key_values() const {
  std::remove_const<it_const_key_values_t::container_type>::type
      key_values_list;
  for (const auto& p : dex2oat_context_) {
    HEADER_KEYS key = p.first;
    std::string value = dex2oat_context_.at(key);
...
std::string* Header::operator[](HEADER_KEYS key) { return get(key); }
...
std::ostream& operator<<(std::ostream& os, const Header& hdr) {
  ...
  // std::endl; os << std::setw(33) << std::setfill(' ') << "Location:"  <<
  // ident_magic << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ') << "Checksum:" << std::hex
     << hdr.checksum() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Instruction set:" << to_string(hdr.instruction_set()) << std::endl;
  // os << std::setw(33) << std::setfill(' ') << "Instruction set features:"  <<
  // ident_magic << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ') << "Dex file count:" << std::dec
     << hdr.nb_dex_files() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Executable offset:" << std::hex << hdr.executable_offset()
     << std::endl;

  os << std::endl;

  os << std::setw(WIDTH) << std::setfill(' ')
     << "Interpreter to Interpreter Bridge Offset:" << std::hex
     << hdr.i2i_bridge_offset() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Interpreter to Compiled Code Bridge Offset:" << std::hex
     << hdr.i2c_code_bridge_offset() << std::endl;

  os << std::endl;

  os << std::setw(WIDTH) << std::setfill(' ')
     << "JNI dlsym lookup offset:" << std::hex << hdr.jni_dlsym_lookup_offset()
     << std::endl;

  os << std::endl;

  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick Generic JNI Trampoline Offset:" << std::hex
     << hdr.quick_generic_jni_trampoline_offset() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick IMT Conflict Trampoline Offset:" << std::hex
     << hdr.quick_imt_conflict_trampoline_offset() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick Resolution Trampoline Offset:" << std::hex
     << hdr.quick_resolution_trampoline_offset() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Quick to Interpreter Bridge Offset:" << std::hex
     << hdr.quick_to_interpreter_bridge_offset() << std::endl;

  os << std::endl;

  os << std::setw(WIDTH) << std::setfill(' ')
     << "Image Patch Delta:" << std::dec << hdr.image_patch_delta()
     << std::endl;

  os << std::endl;

  os << std::setw(WIDTH) << std::setfill(' ')
     << "Image File Location OAT Checksum:" << std::hex
     << hdr.image_file_location_oat_checksum() << std::endl;
  os << std::setw(WIDTH) << std::setfill(' ')
     << "Image File Location OAT Begin:" << std::hex
     << hdr.image_file_location_oat_data_begin() << std::endl;

  os << std::endl;

  for (const auto& p : hdr.key_values()) {
    os << std::setw(WIDTH) << std::setfill(' ')
       << Header::key_to_string(p.first) + ":" << p.second << std::endl;
  }
