...
Section::Section(std::string name) { this->name(std::move(name)); }
...
void Section::content(const Section::content_t& data) {
  ...
  std::move(std::begin(data), std::end(data), content.data() + relative_offset);
}
...
uint64_t Section::address() const { return virtual_address(); }

uint32_t Section::alignment() const { return align_; }

uint32_t Section::relocation_offset() const { return relocations_offset_; }

uint32_t Section::numberof_relocations() const { return nbof_relocations_; }
...
uint32_t Section::reserved1() const { return reserved1_; }

uint32_t Section::reserved2() const { return reserved2_; }

uint32_t Section::reserved3() const { return reserved3_; }

uint32_t Section::raw_flags() const { return flags_; }

Section::it_relocations Section::relocations() { return relocations_; }
...
Section::flag_list_t Section::flags_list() const {
  ...
  std::copy_if(std::begin(section_flags_array), std::end(section_flags_array),
...
void Section::address(uint64_t address) { virtual_address(address); }

void Section::alignment(uint32_t align) { align_ = align; }
...
void Section::flags(uint32_t flags) { flags_ = flags_ | flags; }

void Section::reserved1(uint32_t reserved1) { reserved1_ = reserved1; }

void Section::reserved2(uint32_t reserved2) { reserved2_ = reserved2; }

void Section::reserved3(uint32_t reserved3) { reserved3_ = reserved3; }
...
bool Section::has_segment() const { return segment_ != nullptr; }

SegmentCommand* Section::segment() {
  return const_cast<SegmentCommand*>(
      static_cast<const Section*>(this)->segment());
}

const SegmentCommand* Section::segment() const { return segment_; }
...
std::ostream& operator<<(std::ostream& os, const Section& section) {
  ...
  os << std::hex;
  os << std::left << std::setw(17) << section.name() << std::setw(17)
     << section.segment_name() << std::setw(10) << section.address()
     << std::setw(10) << section.size() << std::setw(10) << section.offset()
     << std::setw(10) << section.alignment() << std::setw(30)
     << to_string(section.type()) << std::setw(20)
     << section.relocation_offset() << std::setw(20)
     << section.numberof_relocations() << std::setw(10) << section.reserved1()
     << std::setw(10) << section.reserved2() << std::setw(10)
     << section.reserved3() << std::setw(10) << flags_str;
