...
struct dyld_chained_starts_in_image {
  ...
  // that segment followed by pool of dyld_chain_starts_in_segment data
};
...
struct dyld_chained_starts_in_segment {
  ...
  uint32_t
      max_valid_pointer; // for 32-bit OS, any value beyond this is not a pointer
  uint16_t page_count;   // how many pages are in array
  // NOTE(romain): Removed as it will be read entry per entry by the Stream
  // uint16_t  page_start[1];      // each entry is offset in each page of first
  // element in chain or DYLD_CHAINED_PTR_START_NONE if no fixups on page
  ...
  // index into chain_starts[] which is a list of starts the last of which has the
  // high bit set
};
#pragma pack(pop)
static_assert(sizeof(dyld_chained_starts_in_segment) == 22,
              "dyld_chained_starts_in_segment: Wrong size");
...
// chain as out of range rebases.  If an entry in the chain is > max_valid_pointer,
// then it is not a pointer.  To restore the value, subtract off the bias, which is
