...
ok_error_t Builder::build_segments() {
  ...
  for (SegmentCommand& segment : binary->segments()) {
    ...
    for (uint32_t i = 0; i < segment.numberof_sections(); ++i) {
      ...
      if (std::is_same<section_t, details::section_64>::
              value) {  // TODO: Move to if constexpr when LIEF will use C++17
...
ok_error_t Builder::build(DylibCommand* library) {
  ...
  const uint32_t raw_size =
      sizeof(details::dylib_command) + library->name().size() + 1;
  ...
  if (library->original_data_.size() != size_needed ||
    ...
    LIEF_WARN(
        "Not enough spaces to rebuild {}. Size required: 0x{:x} vs 0x{:x}",
  ...
  raw_cmd.dylib.current_version = static_cast<uint32_t>(
      DylibCommand::version2int(library->current_version()));
  raw_cmd.dylib.compatibility_version = static_cast<uint32_t>(
...
ok_error_t Builder::build(DylinkerCommand* linker) {
  ...
  const uint32_t raw_size =
      sizeof(details::dylinker_command) + linker->name().size() + 1;
  const uint32_t size_needed = align(raw_size, sizeof(typename T::uint));
  const uint32_t padding = size_needed - raw_size;
...
ok_error_t Builder::build(VersionMin* version_min) {
  ...
  version_min->original_data_.insert(std::end(version_min->original_data_),
                                     padding, 0);
  return ok();
}
...
ok_error_t Builder::build(SourceVersion* source_version) {
  ...
  raw_cmd.version =
      static_cast<uint64_t>(static_cast<uint64_t>(version[0]) << 40 |
  ...
  source_version->original_data_.insert(
      std::end(source_version->original_data_), padding, 0);
  return ok();
}
...
ok_error_t Builder::build(MainCommand* main_cmd) {
  ...
  main_cmd->original_data_.insert(std::end(main_cmd->original_data_), padding,
                                  0);
  return ok();
}
...
ok_error_t Builder::build(DyldInfo* dyld_info) {
  ...
  // /!\ Force to update relocation cache that is used by the following
  // functions
  ...
  }
  {
    ...
    LIEF_SW_END("update_rebase_info(): {}",
                duration_cast<std::chrono::milliseconds>(sw.elapsed()));
  }
  {
    LIEF_SW_START(sw);
    dyld_info->update_binding_info();
    LIEF_SW_END("update_binding_info(): {}",
                duration_cast<std::chrono::seconds>(sw.elapsed()));
  }
  ...
  std::tie(raw_cmd.weak_bind_off, raw_cmd.weak_bind_size) =
      dyld_info->weak_bind();
  std::tie(raw_cmd.lazy_bind_off, raw_cmd.lazy_bind_size) =
      dyld_info->lazy_bind();
  ...
  dyld_info->original_data_.insert(std::end(dyld_info->original_data_), padding,
                                   0);
  return ok();
}
...
ok_error_t Builder::build(FunctionStarts* function_starts) {
  ...
  // Pad to pointer size
  size_t padding = align(packed_functions.size(), sizeof(typename T::uint)) -
                   packed_functions.size();
  packed_functions.insert(std::end(packed_functions), padding, 0);

  // Find the segment associated with LC_FUNCTION_STARTS
  SegmentCommand* segment =
      binary_->segment_from_offset(function_starts->data_offset());
  if (segment == nullptr) {
  ...
  uint64_t relative_offset =
      function_starts->data_offset() - segment->file_offset();
  if (relative_offset > content.size() ||
      (relative_offset + packed_functions.size()) > content.size()) {
  ...
  function_starts->original_data_.insert(
      std::end(function_starts->original_data_), struct_padding, 0);
  return ok();
}
...
std::vector<std::string> Builder::optimize(
  ...
  // TODO(romain): This function is also used in the ELF's builder -> move it in
  // ian 'utils' part?
...
ok_error_t Builder::build(SymbolCommand* symbol_command) {
  ...
  //   // end of the string pool.  The stabs strings are not used when
  //   calculated the UUID for the image.
  ...
  std::vector<std::string> string_table_opt =
      optimize<Symbol, decltype(symbols)>(
  ...
  SegmentCommand* segment =
      binary_->segment_from_offset(symbol_command->strings_offset());
  if (segment == nullptr) {
  ...
  span<uint8_t> content = segment->writable_content();
  uint64_t relative_offset =
      symbol_command->strings_offset() - segment->file_offset();
  if (relative_offset > content.size() ||
      (relative_offset + symname_data.size()) > content.size()) {
    LIEF_WARN("Symbols names don't fit in the segment");
    return make_error_code(lief_errors::build_error);
  }
  std::move(std::begin(symname_data), std::end(symname_data),
            content.data() + relative_offset);
  ...
  if (nlist_table.size() !=
      symbol_command->numberof_symbols() * sizeof(nlist_t)) {
  ...
  if (relative_offset > content.size() ||
      (relative_offset + raw_nlist_table.size()) > content.size()) {
  ...
  symtab.symoff = static_cast<uint32_t>(
      symbol_command->symbol_offset());  // **Usually** After the header
  symtab.nsyms = static_cast<uint32_t>(symbol_command->numberof_symbols());
  symtab.stroff = static_cast<uint32_t>(
      symbol_command->strings_offset());  // **Usually** After nlist table
  ...
  delta = delta -
          (symbol_command->strings_offset() + symbol_command->strings_size());
  symtab.strsize =
      static_cast<uint32_t>(symbol_command->strings_size() + delta);

  symbol_command->original_data_.clear();
  symbol_command->original_data_.reserve(sizeof(details::symtab_command));

  std::move(reinterpret_cast<const uint8_t*>(&symtab),
...
ok_error_t Builder::build(DynamicSymbolCommand* symbol_command) {
  ...
  rawcmd.iundefsym =
      static_cast<uint32_t>(symbol_command->idx_undefined_symbol());
  rawcmd.nundefsym =
      static_cast<uint32_t>(symbol_command->nb_undefined_symbols());
  rawcmd.tocoff = static_cast<uint32_t>(symbol_command->toc_offset());
  rawcmd.ntoc = static_cast<uint32_t>(symbol_command->nb_toc());
  rawcmd.modtaboff =
      static_cast<uint32_t>(symbol_command->module_table_offset());
  ...
  rawcmd.nextrel =
      static_cast<uint32_t>(symbol_command->nb_external_relocations());
  rawcmd.locreloff =
      static_cast<uint32_t>(symbol_command->local_relocation_offset());
  rawcmd.nlocrel =
      static_cast<uint32_t>(symbol_command->nb_local_relocations());
...
ok_error_t Builder::build(SubFramework* sf) {
  ...
  const uint32_t raw_size =
      sizeof(details::sub_framework_command) + sf->umbrella().size() + 1;
  const uint32_t size_needed = align(raw_size, sizeof(typename T::uint));
  const uint32_t padding = size_needed - raw_size;
  ...
  raw_cmd.cmdsize = static_cast<uint32_t>(size_needed);
  raw_cmd.umbrella =
      static_cast<uint32_t>(sizeof(details::sub_framework_command));
...
ok_error_t Builder::build(DyldEnvironment* de) {
  ...
  const uint32_t raw_size =
      sizeof(details::dylinker_command) + de->value().size() + 1;
  const uint32_t size_needed = align(raw_size, sizeof(typename T::uint));
  const uint32_t padding = size_needed - raw_size;

  if (de->original_data_.size() != size_needed || de->size() != size_needed) {
    LIEF_WARN(
        "Not enough spaces to rebuild {}. Size required: 0x{:x} vs 0x{:x}",
...
ok_error_t Builder::build(ThreadCommand* tc) {
  ...
  if (tc->original_data_.size() != size_needed || tc->size() != size_needed) {
    LIEF_WARN(
        "Not enough spaces to rebuild 'ThreadCommand'. Size required: 0x{:x} "
        "vs 0x{:x}",
...
ok_error_t Builder::build(BuildVersion* bv) {
  ...
  if (bv->original_data_.size() != size_needed || bv->size() != size_needed) {
    LIEF_WARN(
        "Not enough spaces to rebuild 'BuildVersion'. Size required: 0x{:x} vs "
        "0x{:x}",
  ...
  raw_cmd.minos =
      static_cast<uint32_t>(minos[0] << 16 | minos[1] << 8 | minos[2]);
  raw_cmd.sdk = static_cast<uint32_t>(sdk[0] << 16 | sdk[1] << 8 | sdk[2]);
  ...
  std::vector<uint8_t> raw_tools(
      raw_cmd.ntools * sizeof(details::build_tool_version), 0);
  auto* tools_array =
      reinterpret_cast<details::build_tool_version*>(raw_tools.data());
  ...
  std::move(std::begin(raw_tools), std::end(raw_tools),
            std::back_inserter(bv->original_data_));
