...
BindingInfo::BindingInfo(BINDING_CLASS cls, BIND_TYPES type, uint64_t address,
                         int64_t addend, int32_t oridnal, bool is_weak,
                         bool is_non_weak_definition, uint64_t offset)
    : class_{cls},
      binding_type_{type},
...
bool BindingInfo::has_segment() const { return segment_ != nullptr; }

const SegmentCommand* BindingInfo::segment() const { return segment_; }

SegmentCommand* BindingInfo::segment() {
  return const_cast<SegmentCommand*>(
      static_cast<const BindingInfo*>(this)->segment());
}

bool BindingInfo::has_symbol() const { return symbol_ != nullptr; }

const Symbol* BindingInfo::symbol() const { return symbol_; }

Symbol* BindingInfo::symbol() {
  return const_cast<Symbol*>(static_cast<const BindingInfo*>(this)->symbol());
}

bool BindingInfo::has_library() const { return library_ != nullptr; }

const DylibCommand* BindingInfo::library() const { return library_; }

DylibCommand* BindingInfo::library() {
  return const_cast<DylibCommand*>(
      static_cast<const BindingInfo*>(this)->library());
}

BINDING_CLASS BindingInfo::binding_class() const { return class_; }

void BindingInfo::binding_class(BINDING_CLASS bind_class) {
  class_ = bind_class;
}

BIND_TYPES BindingInfo::binding_type() const { return binding_type_; }

void BindingInfo::binding_type(BIND_TYPES type) { binding_type_ = type; }

int32_t BindingInfo::library_ordinal() const { return library_ordinal_; }

void BindingInfo::library_ordinal(int32_t ordinal) {
  library_ordinal_ = ordinal;
}

int64_t BindingInfo::addend() const { return addend_; }

void BindingInfo::addend(int64_t addend) { addend_ = addend; }

bool BindingInfo::is_weak_import() const { return is_weak_import_; }

void BindingInfo::set_weak_import(bool val) { is_weak_import_ = val; }

uint64_t BindingInfo::address() const { return address_; }

void BindingInfo::address(uint64_t addr) { address_ = addr; }

uint64_t BindingInfo::original_offset() const { return offset_; }

void BindingInfo::accept(Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const BindingInfo& binding_info) {
  ...
  os << std::setw(13) << "Class: " << to_string(binding_info.binding_class())
     << std::endl;
  os << std::setw(13) << "Type: " << to_string(binding_info.binding_type())
     << std::endl;
