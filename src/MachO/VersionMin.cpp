...
VersionMin::VersionMin(const details::version_min_command& version_cmd)
    : LoadCommand::LoadCommand{static_cast<LOAD_COMMAND_TYPES>(version_cmd.cmd),
                               version_cmd.cmdsize},
...
VersionMin* VersionMin::clone() const { return new VersionMin(*this); }

const VersionMin::version_t& VersionMin::version() const { return version_; }

void VersionMin::version(const VersionMin::version_t& version) {
  version_ = version;
}

const VersionMin::version_t& VersionMin::sdk() const { return sdk_; }

void VersionMin::sdk(const VersionMin::version_t& version) { sdk_ = version; }

void VersionMin::accept(Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& VersionMin::print(std::ostream& os) const {
  ...
  os << std::setw(10) << "Version: " << std::dec << version[0] << "."
