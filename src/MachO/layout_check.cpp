...
template<typename T>
inline bool greater_than_add_or_overflow(uint32_t LHS, uint32_t RHS, T B) {
  return (LHS > B) || (RHS > (B - LHS));
}

template<typename T>
inline bool greater_than_add_or_overflow(uint64_t LHS, uint64_t RHS, T B) {
  return (LHS > B) || (RHS > (B - LHS));
}
...
// Mirror of Image::validStructureLinkedit (version: dyld-1235.2)
bool LayoutChecker::check_validate_linkedit() {
  struct chunk_t {
    enum class KIND {
      UNKNOWN = 0,
      SYMTAB, SYMTAB_STR,
  ...
  for (size_t i = 0; i < chunks.size(); ++i) {
    ...
    if (greater_than_add_or_overflow((uint64_t)chunk.file_offset,
                                     chunk.size, linkedit_file_offset_end))
    {
...
bool LayoutChecker::check_chained_fixups() {
  ...
  if (greater_than_add_or_overflow(fixups->imports_offset(),
                                   fmt_entry_size * fixups->imports_count(),
  ...
  for (const DyldChainedFixups::chained_starts_in_segment& seg_start : starts_info) {
    ...
    uint32_t max_overflow_idx = (seg_start.size - sizeof(details::dyld_chained_starts_in_segment)) / sizeof(uint16_t);
