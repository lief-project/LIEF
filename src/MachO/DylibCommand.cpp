...
DylibCommand* DylibCommand::clone() const { return new DylibCommand(*this); }

const std::string& DylibCommand::name() const { return name_; }

uint32_t DylibCommand::timestamp() const { return timestamp_; }
...
void DylibCommand::name(const std::string& name) { name_ = name; }

void DylibCommand::timestamp(uint32_t timestamp) { timestamp_ = timestamp; }

void DylibCommand::current_version(DylibCommand::version_t currentVersion) {
  current_version_ = version2int(currentVersion);
}

void DylibCommand::compatibility_version(
    DylibCommand::version_t compatibilityVersion) {
  compatibility_version_ = version2int(compatibilityVersion);
}

void DylibCommand::accept(Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& DylibCommand::print(std::ostream& os) const {
  ...
  const DylibCommand::version_t& compatibility_version =
      this->compatibility_version();
  LoadCommand::print(os);
  os << std::hex;
  os << std::left << std::setw(35) << name() << timestamp() << " - "

     << std::dec << current_version[0] << "." << current_version[1] << "."
     << current_version[2] << " - "

     << compatibility_version[0] << "." << compatibility_version[1] << "."
...
DylibCommand DylibCommand::create(LOAD_COMMAND_TYPES type,
                                  const std::string& name, uint32_t timestamp,
...
DylibCommand DylibCommand::load_dylib(const std::string& name,
                                      ...
                                      uint32_t current_version,
                                      uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_LOAD_DYLIB, name,
...
DylibCommand DylibCommand::weak_dylib(const std::string& name,
                                      ...
                                      uint32_t current_version,
                                      uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_LOAD_WEAK_DYLIB, name,
                              timestamp, current_version, compat_version);
}

DylibCommand DylibCommand::id_dylib(const std::string& name, uint32_t timestamp,
                                    uint32_t current_version,
                                    uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_ID_DYLIB, name, timestamp,
...
DylibCommand DylibCommand::reexport_dylib(const std::string& name,
                                          ...
                                          uint32_t current_version,
                                          uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_REEXPORT_DYLIB, name,
                              timestamp, current_version, compat_version);
}
...
DylibCommand DylibCommand::load_upward_dylib(const std::string& name,
                                             ...
                                             uint32_t current_version,
                                             uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_LOAD_UPWARD_DYLIB, name,
                              timestamp, current_version, compat_version);
}
...
DylibCommand DylibCommand::lazy_load_dylib(const std::string& name,
                                           ...
                                           uint32_t current_version,
                                           uint32_t compat_version) {
  return DylibCommand::create(LOAD_COMMAND_TYPES::LC_LAZY_LOAD_DYLIB, name,
                              timestamp, current_version, compat_version);
}
