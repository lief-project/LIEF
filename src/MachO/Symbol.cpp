...
uint8_t Symbol::type() const { return type_; }

uint8_t Symbol::numberof_sections() const { return numberof_sections_; }

uint16_t Symbol::description() const { return description_; }

SYMBOL_ORIGINS Symbol::origin() const { return origin_; }

void Symbol::type(uint8_t type) { type_ = type; }

void Symbol::numberof_sections(uint8_t nbsections) {
  numberof_sections_ = nbsections;
}

void Symbol::description(uint16_t desc) { description_ = desc; }
...
bool Symbol::has_export_info() const { return export_info_ != nullptr; }

const ExportInfo* Symbol::export_info() const { return export_info_; }

ExportInfo* Symbol::export_info() {
  return const_cast<ExportInfo*>(
      static_cast<const Symbol*>(this)->export_info());
}

bool Symbol::has_binding_info() const { return binding_info_ != nullptr; }

const BindingInfo* Symbol::binding_info() const { return binding_info_; }

BindingInfo* Symbol::binding_info() {
  return const_cast<BindingInfo*>(
      static_cast<const Symbol*>(this)->binding_info());
}
...
std::ostream& operator<<(std::ostream& os, const Symbol& symbol) {
  ...
  // if ((symbol.type_ & MACHO_SYMBOL_TYPES::N_TYPE) ==
  // MACHO_SYMBOL_TYPES::N_TYPE) {
  //   type = to_string(
  //       static_cast<N_LIST_TYPES>(symbol.type_ &
  //       MACHO_SYMBOL_TYPES::N_TYPE));
  // } else if((symbol.type_ & MACHO_SYMBOL_TYPES::N_STAB) > 0) {
  //   type = to_string(MACHO_SYMBOL_TYPES::N_STAB);
  // } else if((symbol.type_ & MACHO_SYMBOL_TYPES::N_PEXT) ==
  // MACHO_SYMBOL_TYPES::N_PEXT) {
  //   type = to_string(MACHO_SYMBOL_TYPES::N_PEXT);
  // }  else if((symbol.type_ & MACHO_SYMBOL_TYPES::N_EXT) ==
  // MACHO_SYMBOL_TYPES::N_EXT) {
  //   type = to_string(MACHO_SYMBOL_TYPES::N_EXT);
  // }

  os << std::hex;
  os << std::left;
  os << std::setw(30) << symbol.name() << std::setw(10) << type << std::setw(10)
     << symbol.description() << std::setw(20) << symbol.value();
  return os;
}
}  // namespace MachO
}  // namespace LIEF
