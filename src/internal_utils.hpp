...
std::vector<std::string>
  ...
  for (const auto& val : string_table) {
    ...
    std::reverse(string_table_optimized.back().begin(),
                 string_table_optimized.back().end());
  }
  ...
  // list once and memorize ones we merged to calculate the offsets later
...
auto make_empty_iterator() {
  ...
  return make_range<typename T::Iterator>(typename T::Iterator(std::move(begin)),
