...
CodeViewPDB CodeViewPDB::from_pdb70(signature_t sig, uint32_t age,
                                    const std::string& filename) {
  return {CODE_VIEW_SIGNATURES::CVS_PDB_70, sig, age, filename};
}
CodeViewPDB CodeViewPDB::from_pdb20(uint32_t signature, uint32_t age,
                                    const std::string& filename) {
...
CodeViewPDB* CodeViewPDB::clone() const { return new CodeViewPDB{*this}; }

CodeViewPDB::signature_t CodeViewPDB::signature() const { return signature_; }

uint32_t CodeViewPDB::age() const { return age_; }

const std::string& CodeViewPDB::filename() const { return filename_; }
...
void CodeViewPDB::age(uint32_t age) { age_ = age; }

void CodeViewPDB::filename(const std::string& filename) {
  filename_ = filename;
}

void CodeViewPDB::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
