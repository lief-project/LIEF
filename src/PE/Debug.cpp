...
uint32_t Debug::characteristics() const { return characteristics_; }

uint32_t Debug::timestamp() const { return timestamp_; }

uint16_t Debug::major_version() const { return majorversion_; }

uint16_t Debug::minor_version() const { return minorversion_; }

DEBUG_TYPES Debug::type() const { return type_; }

uint32_t Debug::sizeof_data() const { return sizeof_data_; }

uint32_t Debug::addressof_rawdata() const { return addressof_rawdata_; }

uint32_t Debug::pointerto_rawdata() const { return pointerto_rawdata_; }

bool Debug::has_code_view() const { return code_view_ != nullptr; }

const CodeView* Debug::code_view() const { return code_view_.get(); }

CodeView* Debug::code_view() {
  return const_cast<CodeView*>(static_cast<const Debug*>(this)->code_view());
}

bool Debug::has_pogo() const { return pogo_ != nullptr; }

const Pogo* Debug::pogo() const { return pogo_.get(); }
...
void Debug::timestamp(uint32_t timestamp) { timestamp_ = timestamp; }
...
void Debug::type(DEBUG_TYPES new_type) { type_ = new_type; }

void Debug::sizeof_data(uint32_t sizeof_data) { sizeof_data_ = sizeof_data; }
...
void Debug::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
...
bool Debug::operator!=(const Debug& rhs) const { return !(*this == rhs); }
...
std::ostream& operator<<(std::ostream& os, const Debug& entry) {
  ...
  os << std::setw(20) << "Characteristics:" << entry.characteristics()
     << std::endl;
  ...
  os << std::setw(20) << "Address of rawdata:" << entry.addressof_rawdata()
     << std::endl;
  os << std::setw(20) << "Pointer to rawdata:" << entry.pointerto_rawdata()
     << std::endl;
