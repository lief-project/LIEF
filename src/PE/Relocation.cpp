...
Relocation::Relocation(const details::pe_base_relocation_block& header)
    : block_size_{header.BlockSize}, virtual_address_{header.PageRVA} {}
...
uint32_t Relocation::virtual_address() const { return virtual_address_; }

uint32_t Relocation::block_size() const { return block_size_; }

Relocation::it_const_entries Relocation::entries() const { return entries_; }

Relocation::it_entries Relocation::entries() { return entries_; }
...
void Relocation::block_size(uint32_t block_size) { block_size_ = block_size; }

void Relocation::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
