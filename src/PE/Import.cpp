...
const ImportEntry* Import::get_entry(const std::string& name) const {
  const auto it_entry = std::find_if(
      ...
      [&name](const ImportEntry& entry) { return entry.name() == name; });
...
Import::it_entries Import::entries() { return entries_; }

Import::it_const_entries Import::entries() const { return entries_; }
...
void Import::name(const std::string& name) { name_ = name; }

const DataDirectory* Import::directory() const { return directory_; }

DataDirectory* Import::directory() {
  return const_cast<DataDirectory*>(
      static_cast<const Import*>(this)->directory());
}

const DataDirectory* Import::iat_directory() const { return iat_directory_; }

DataDirectory* Import::iat_directory() {
  return const_cast<DataDirectory*>(
      static_cast<const Import*>(this)->iat_directory());
}
...
uint32_t Import::forwarder_chain() const { return forwarder_chain_; }

uint32_t Import::timedatestamp() const { return timedatestamp_; }

void Import::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
...
bool Import::operator!=(const Import& rhs) const { return !(*this == rhs); }

std::ostream& operator<<(std::ostream& os, const Import& entry) {
  os << std::hex;
  os << std::left << std::setw(20) << entry.name() << std::setw(10)
     << entry.import_lookup_table_rva() << std::setw(10)
     << entry.import_address_table_rva() << std::setw(10)
     << entry.forwarder_chain() << std::setw(10) << entry.timedatestamp()
     << std::endl;
