...
uint32_t RichHeader::key() const { return key_; }

RichHeader::it_entries RichHeader::entries() { return entries_; }

RichHeader::it_const_entries RichHeader::entries() const { return entries_; }

void RichHeader::key(uint32_t key) { key_ = key; }
...
void RichHeader::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }

std::vector<uint8_t> RichHeader::raw() const { return raw(0); }
...
std::vector<uint8_t> RichHeader::raw(uint32_t xor_key) const {
  ...
  wstream.write(RICH_MAGIC).write(xor_key);
