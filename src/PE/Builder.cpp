...
ok_error_t Builder::build() {
  ...
  if (binary_->has_resources() && binary_->resources_ != nullptr &&
      build_resources_) {
...
ok_error_t Builder::build_relocation() {
  ...
  for (const Relocation& relocation : binary_->relocations()) {
    ...
    relocation_header.PageRVA =
        static_cast<uint32_t>(relocation.virtual_address());

    uint32_t block_size =
        static_cast<uint32_t>((relocation.entries().size()) * sizeof(uint16_t) +
    ...
    for (const RelocationEntry& entry : relocation.entries()) {
      ...
      content.insert(std::end(content), reinterpret_cast<uint8_t*>(&data),
  ...
  Section new_relocation_section{
      ".l" +
      std::to_string(static_cast<uint32_t>(
          DATA_DIRECTORY::BASE_RELOCATION_TABLE))};  // .l5 -> lief.relocation
  ...
  // Pad with 0
  content.insert(std::end(content), size_aligned - content.size(), 0);
...
ok_error_t Builder::build_resources() {
  ...
  const uint64_t content_size_aligned =
      align(content.size(), binary_->optional_header().file_alignment());
  ...
  Section new_section_rsrc{".l" + std::to_string(static_cast<uint32_t>(
                                      DATA_DIRECTORY::RESOURCE_TABLE))};
  new_section_rsrc.characteristics(0x40000040);
  new_section_rsrc.content(content);

  Section& rsrc_section =
      binary_->add_section(new_section_rsrc, PE_SECTION_TYPES::RESOURCE);

  construct_resources(*node, &content, &offset_header, &offset_data,
                      &offset_name, rsrc_section.virtual_address(), 0);
...
// Pre-computation
//
ok_error_t Builder::compute_resources_size(ResourceNode& node,
                                           uint32_t* header_size,
                                           uint32_t* data_size,
                                           uint32_t* name_size) {
  if (!node.name().empty()) {
    *name_size +=
        sizeof(uint16_t) + (node.name().size() + 1) * sizeof(char16_t);
  }
...
//
ok_error_t Builder::construct_resources(ResourceNode& node,
                                        std::vector<uint8_t>* content,
  ...
  if (node.is_directory()) {
    ...
    rsrc_header.Characteristics =
        static_cast<uint32_t>(rsrc_directory.characteristics());
    rsrc_header.TimeDateStamp =
        static_cast<uint32_t>(rsrc_directory.time_date_stamp());
    rsrc_header.MajorVersion =
        static_cast<uint16_t>(rsrc_directory.major_version());
    rsrc_header.MinorVersion =
        static_cast<uint16_t>(rsrc_directory.minor_version());
    rsrc_header.NumberOfNameEntries =
        static_cast<uint16_t>(rsrc_directory.numberof_name_entries());
    rsrc_header.NumberOfIDEntries =
        static_cast<uint16_t>(rsrc_directory.numberof_id_entries());
    ...
    *offset_header +=
        node.childs().size() * sizeof(details::pe_resource_directory_entries);

    // Build childs
    // ============
    for (ResourceNode& child : node.childs()) {
      if ((static_cast<uint32_t>(child.id()) & 0x80000000) !=
          0u) {  // There is a name
        ...
        auto* length_ptr =
            reinterpret_cast<uint16_t*>(content->data() + *offset_name);
        *length_ptr = name.size();
        auto* name_ptr = reinterpret_cast<char16_t*>(
            content->data() + *offset_name + sizeof(uint16_t));
      ...
      if (child.is_directory()) {
        ...
        std::copy(reinterpret_cast<uint8_t*>(&entry_header),
                  reinterpret_cast<uint8_t*>(&entry_header) +
                      sizeof(details::pe_resource_directory_entries),
                  content->data() + current_offset);

        current_offset += sizeof(details::pe_resource_directory_entries);
        construct_resources(child, content, offset_header, offset_data,
                            offset_name, base_rva, depth + 1);
      } else {  // DATA
        details::pe_resource_directory_entries entry_header;
    ...
    std::copy(reinterpret_cast<uint8_t*>(&data_header),
              reinterpret_cast<uint8_t*>(&data_header) +
                  sizeof(details::pe_resource_data_entry),
              content->data() + *offset_header);
...
ok_error_t Builder::build(const DosHeader& dos_header) {
  ...
  raw_dos_header.UsedBytesInTheLastPage =
      static_cast<uint16_t>(dos_header.used_bytes_in_the_last_page());
  raw_dos_header.FileSizeInPages =
      static_cast<uint16_t>(dos_header.file_size_in_pages());
  raw_dos_header.NumberOfRelocationItems =
      static_cast<uint16_t>(dos_header.numberof_relocation());
  raw_dos_header.HeaderSizeInParagraphs =
      static_cast<uint16_t>(dos_header.header_size_in_paragraphs());
  raw_dos_header.MinimumExtraParagraphs =
      static_cast<uint16_t>(dos_header.minimum_extra_paragraphs());
  raw_dos_header.MaximumExtraParagraphs =
      static_cast<uint16_t>(dos_header.maximum_extra_paragraphs());
  raw_dos_header.InitialRelativeSS =
      static_cast<uint16_t>(dos_header.initial_relative_ss());
  ...
  raw_dos_header.InitialRelativeCS =
      static_cast<uint16_t>(dos_header.initial_relative_cs());
  raw_dos_header.AddressOfRelocationTable =
      static_cast<uint16_t>(dos_header.addressof_relocation_table());
  raw_dos_header.OverlayNumber =
      static_cast<uint16_t>(dos_header.overlay_number());
  ...
  raw_dos_header.AddressOfNewExeHeader =
      static_cast<uint16_t>(dos_header.addressof_new_exeheader());
  ...
  ios_.seekp(0);
  ios_.write(reinterpret_cast<const uint8_t*>(&raw_dos_header),
             sizeof(details::pe_dos_header));
  if (!binary_->dos_stub().empty() && build_dos_stub_) {
    if (sizeof(details::pe_dos_header) + binary_->dos_stub().size() >
...
ok_error_t Builder::build(const Header& bHeader) {
  ...
  header.PointerToSymbolTable =
      static_cast<uint32_t>(bHeader.pointerto_symbol_table());
  header.NumberOfSymbols = static_cast<uint32_t>(bHeader.numberof_symbols());
  // TODO: Check
  header.SizeOfOptionalHeader =
      static_cast<uint16_t>(bHeader.sizeof_optional_header());
  ...
  ios_.write(reinterpret_cast<const uint8_t*>(&header),
...
ok_error_t Builder::build(const DataDirectory& data_directory) {
  ...
  ios_.write(reinterpret_cast<uint8_t*>(&header),
...
ok_error_t Builder::build(const Section& section) {
  ...
  header.PointerToRelocations =
      static_cast<uint32_t>(section.pointerto_relocation());
  header.PointerToLineNumbers =
      static_cast<uint32_t>(section.pointerto_line_numbers());
  header.NumberOfRelocations =
      static_cast<uint16_t>(section.numberof_relocations());
  header.NumberOfLineNumbers =
      static_cast<uint16_t>(section.numberof_line_numbers());
  header.Characteristics = static_cast<uint32_t>(section.characteristics());

  const std::string& sec_name = section.fullname();
  uint32_t name_length =
      std::min<uint32_t>(sec_name.size() + 1, sizeof(header.Name));
  std::copy(sec_name.c_str(), sec_name.c_str() + name_length,
            std::begin(header.Name));
