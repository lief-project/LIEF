...
DosHeader::DosHeader(const details::pe_dos_header& header)
  ...
  std::copy(reinterpret_cast<const uint16_t*>(header.Reserved),
            reinterpret_cast<const uint16_t*>(header.Reserved) + 4,
            std::begin(reserved_));
  std::copy(reinterpret_cast<const uint16_t*>(header.Reserved2),
            reinterpret_cast<const uint16_t*>(header.Reserved2) + 10,
            std::begin(reserved2_));
}

uint16_t DosHeader::magic() const { return magic_; }

uint16_t DosHeader::used_bytes_in_the_last_page() const {
  return usedBytesInTheLastPage_;
}

uint16_t DosHeader::file_size_in_pages() const { return fileSizeInPages_; }

uint16_t DosHeader::numberof_relocation() const { return numberOfRelocation_; }
...
uint16_t DosHeader::initial_relative_ss() const { return initialRelativeSS_; }

uint16_t DosHeader::initial_sp() const { return initialSP_; }

uint16_t DosHeader::checksum() const { return checksum_; }

uint16_t DosHeader::initial_ip() const { return initialIP_; }

uint16_t DosHeader::initial_relative_cs() const { return initialRelativeCS_; }

uint16_t DosHeader::addressof_relocation_table() const {
  return addressOfRelocationTable_;
}

uint16_t DosHeader::overlay_number() const { return overlayNumber_; }

std::array<uint16_t, 4> DosHeader::reserved() const { return reserved_; }

uint16_t DosHeader::oem_id() const { return oEMid_; }

uint16_t DosHeader::oem_info() const { return oEMinfo_; }

std::array<uint16_t, 10> DosHeader::reserved2() const { return reserved2_; }

uint32_t DosHeader::addressof_new_exeheader() const {
  return addressOfNewExeHeader_;
}

void DosHeader::magic(uint16_t magic) { magic_ = magic; }
...
void DosHeader::initial_sp(uint16_t initialSP) { initialSP_ = initialSP; }

void DosHeader::checksum(uint16_t checksum) { checksum_ = checksum; }

void DosHeader::initial_ip(uint16_t initialIP) { initialIP_ = initialIP; }
...
void DosHeader::oem_id(uint16_t oEMid) { oEMid_ = oEMid; }

void DosHeader::oem_info(uint16_t oEMinfo) { oEMinfo_ = oEMinfo; }
...
void DosHeader::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const DosHeader& entry) {
  ...
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Magic: " << entry.magic_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Used Bytes In The LastPage: " << entry.usedBytesInTheLastPage_
     << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "File Size In Pages: " << entry.fileSizeInPages_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Number Of Relocation: " << entry.numberOfRelocation_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Header Size In Paragraphs: " << entry.headerSizeInParagraphs_
     << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Minimum Extra Paragraphs: " << entry.minimumExtraParagraphs_
     << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Maximum Extra Paragraphs: " << entry.maximumExtraParagraphs_
     << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Initial Relative SS: " << entry.initialRelativeSS_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Initial SP: " << entry.initialSP_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Checksum: " << entry.checksum_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Initial IP: " << entry.initialIP_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Initial Relative CS: " << entry.initialRelativeCS_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Address Of Relocation Table: " << entry.addressOfRelocationTable_
     << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Overlay Number: " << entry.overlayNumber_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "OEM id: " << entry.oEMid_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "OEM info: " << entry.oEMinfo_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Address Of New Exe Header: " << entry.addressOfNewExeHeader_
     << std::endl;
  return os;
}
