...
bool ResourceDialog::is_extended() const { return signature_ == 0xFFFF; }

uint32_t ResourceDialog::extended_style() const { return ext_style_; }
...
uint32_t ResourceDialog::style() const { return style_; }

std::set<WINDOW_STYLES> ResourceDialog::style_list() const {
  std::set<WINDOW_STYLES> styles;
  std::copy_if(std::begin(details::window_styles_array),
...
std::set<DIALOG_BOX_STYLES> ResourceDialog::dialogbox_style_list() const {
  ...
  std::copy_if(std::begin(details::dialog_box_styles_array),
...
int16_t ResourceDialog::x() const { return x_; }

int16_t ResourceDialog::y() const { return y_; }

int16_t ResourceDialog::cx() const { return cx_; }

int16_t ResourceDialog::cy() const { return cy_; }

ResourceDialog::it_const_items ResourceDialog::items() const { return items_; }

RESOURCE_LANGS ResourceDialog::lang() const { return lang_; }

RESOURCE_SUBLANGS ResourceDialog::sub_lang() const { return sublang_; }

void ResourceDialog::lang(RESOURCE_LANGS lang) { lang_ = lang; }
...
uint16_t ResourceDialog::version() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.dlgVer does not "
        "exist");
  }
  return version_;
}
...
uint16_t ResourceDialog::signature() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.signature does not "
        "exist");
  }
  return signature_;
}
...
uint32_t ResourceDialog::help_id() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.helpID does not "
        "exist");
  }
  return help_id_;
}
...
uint16_t ResourceDialog::weight() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.weight does not "
        "exist");
  }
  return weight_;
}
...
uint8_t ResourceDialog::charset() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.charset does not "
        "exist");
  }
  return charset_;
}
...
uint16_t ResourceDialog::point_size() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.pointsize does not "
        "exist");
  }
  return point_size_;
}
...
bool ResourceDialog::is_italic() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.italic does not "
        "exist");
  }
  return italic_;
}
...
const std::u16string& ResourceDialog::title() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.title does not "
        "exist");
  }
...
const std::u16string& ResourceDialog::typeface() const {
  if (!is_extended()) {
    LIEF_WARN(
        "This dialog is not an extended one. DLGTEMPLATEEX.typeface does not "
        "exist");
  }
...
void ResourceDialog::accept(Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const ResourceDialog& dialog) {
  ...
  std::string styles_str = std::accumulate(
      std::begin(styles), std::end(styles), std::string{},
  ...
  std::string dialogbox_styles_str = std::accumulate(
      std::begin(dialogbox_styles), std::end(dialogbox_styles), std::string{},
      [](const std::string& a, DIALOG_BOX_STYLES b) {
        return a.empty() ? to_string(b) : a + ", " + to_string(b);
      });

  const std::set<EXTENDED_WINDOW_STYLES>& ext_styles =
      dialog.extended_style_list();
  std::string ext_styles_str = std::accumulate(
      std::begin(ext_styles), std::end(ext_styles), std::string{},
  ...
  os << std::dec << dialog.x() << ", " << dialog.y() << ", " << dialog.cx()
     << ", " << dialog.cy() << std::endl;
  ...
  os << "Lang: " << to_string(dialog.lang()) << " / "
     << to_string(dialog.sub_lang()) << std::endl;

  if (dialog.is_extended()) {
    os << "Title: \"" << u16tou8(dialog.title()) << "\"" << std::endl;
    os << "Font: \"" << std::dec << dialog.point_size() << " "
       << u16tou8(dialog.typeface()) << "\""
       << ", " << std::boolalpha << dialog.is_italic() << ", " << std::dec
       << static_cast<uint32_t>(dialog.charset()) << std::endl;
  }
