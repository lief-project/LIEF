...
ResourceFixedFileInfo::ResourceFixedFileInfo(const ResourceFixedFileInfo&) =
    default;
ResourceFixedFileInfo& ResourceFixedFileInfo::operator=(
    const ResourceFixedFileInfo&) = default;
...
ResourceFixedFileInfo::ResourceFixedFileInfo(
    const details::pe_resource_fixed_file_info& header)
    : signature_{header.signature},
      ...
      file_subtype_{
          static_cast<FIXED_VERSION_FILE_SUB_TYPES>(header.file_subtype)},
      file_date_MS_{header.file_date_MS},
      file_date_LS_{header.file_date_LS} {}

uint32_t ResourceFixedFileInfo::signature() const { return signature_; }
...
uint32_t ResourceFixedFileInfo::file_flags() const { return file_flags_; }

FIXED_VERSION_OS ResourceFixedFileInfo::file_os() const { return file_os_; }
...
uint32_t ResourceFixedFileInfo::file_date_MS() const { return file_date_MS_; }

uint32_t ResourceFixedFileInfo::file_date_LS() const { return file_date_LS_; }
...
void ResourceFixedFileInfo::file_subtype(
    FIXED_VERSION_FILE_SUB_TYPES file_subtype) {
...
std::ostream& operator<<(std::ostream& os,
                         const ResourceFixedFileInfo& fixed_info) {
  // File Version (readable)
  std::string file_version_str;
  file_version_str +=
      std::to_string((fixed_info.file_version_MS() >> 16) & 0xFFFF);
  file_version_str += " - ";
  file_version_str += std::to_string(fixed_info.file_version_MS() & 0xFFFF);
  file_version_str += " - ";
  file_version_str +=
      std::to_string((fixed_info.file_version_LS() >> 16) & 0xFFFF);
  file_version_str += " - ";
  ...
  product_version_str +=
      std::to_string((fixed_info.product_version_MS() >> 16) & 0xFFFF);
  product_version_str += " - ";
  product_version_str +=
      std::to_string(fixed_info.product_version_MS() & 0xFFFF);
  product_version_str += " - ";
  product_version_str +=
      std::to_string((fixed_info.product_version_LS() >> 16) & 0xFFFF);
  product_version_str += " - ";
  product_version_str +=
      std::to_string(fixed_info.product_version_LS() & 0xFFFF);

  os << std::hex << std::left;
  os << std::setw(17) << std::setfill(' ')
     << "Signature:" << fixed_info.signature() << std::endl;
  os << std::setw(17) << std::setfill(' ')
     << "Struct version:" << fixed_info.struct_version() << std::endl;
  os << std::setw(17) << std::setfill(' ')
     << "File version:" << file_version_str << std::endl;
  os << std::setw(17) << std::setfill(' ')
     << "Product version:" << product_version_str << std::endl;
  os << std::setw(17) << std::setfill(' ')
     << "File OS:" << to_string(fixed_info.file_os()) << std::endl;
  os << std::setw(17) << std::setfill(' ')
     << "File type:" << to_string(fixed_info.file_type()) << std::endl;
  if (fixed_info.file_type() == FIXED_VERSION_FILE_TYPES::VFT_DRV ||
      fixed_info.file_type() == FIXED_VERSION_FILE_TYPES::VFT_FONT) {
    os << std::setw(17) << std::setfill(' ')
       << "File sub-type:" << to_string(fixed_info.file_subtype()) << std::endl;
  }
  return os;
}
