...
ResourceDialogItem& ResourceDialogItem::operator=(const ResourceDialogItem&) =
    default;
...
bool ResourceDialogItem::is_extended() const { return is_extended_; }

uint32_t ResourceDialogItem::extended_style() const { return ext_style_; }

std::set<EXTENDED_WINDOW_STYLES> ResourceDialogItem::extended_style_list()
    const {
...
bool ResourceDialogItem::has_extended_style(
    EXTENDED_WINDOW_STYLES style) const {
  return (ext_style_ & static_cast<uint32_t>(style)) != 0;
}

uint32_t ResourceDialogItem::style() const { return style_; }
...
int16_t ResourceDialogItem::x() const { return x_; }

int16_t ResourceDialogItem::y() const { return y_; }

int16_t ResourceDialogItem::cx() const { return cx_; }

int16_t ResourceDialogItem::cy() const { return cy_; }

uint32_t ResourceDialogItem::id() const { return id_; }
...
std::ostream& operator<<(std::ostream& os,
  ...
  const std::set<EXTENDED_WINDOW_STYLES>& ext_styles =
      dialog_item.extended_style_list();
  ...
  os << ", " << std::dec << dialog_item.x() << ", " << dialog_item.y() << ", "
     << dialog_item.cx() << ", " << dialog_item.cy();
