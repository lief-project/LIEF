...
RelocationEntry::RelocationEntry(const RelocationEntry& other)
    : LIEF::Relocation{other}, position_{other.position_}, type_{other.type_} {}
...
RelocationEntry::RelocationEntry()
    : position_{0}, type_{RELOCATIONS_BASE_TYPES::IMAGE_REL_BASED_ABSOLUTE} {}
...
uint16_t RelocationEntry::position() const { return position_; }

RELOCATIONS_BASE_TYPES RelocationEntry::type() const { return type_; }
...
void RelocationEntry::position(uint16_t position) { position_ = position; }

void RelocationEntry::type(RELOCATIONS_BASE_TYPES type) { type_ = type; }
...
void RelocationEntry::accept(Visitor& visitor) const { visitor.visit(*this); }
