...
Header::Header(const details::pe_header& header)
    : machine_(static_cast<MACHINE_TYPES>(header.Machine)),
      ...
      characteristics_(
          static_cast<HEADER_CHARACTERISTICS>(header.Characteristics))

{
  std::copy(reinterpret_cast<const uint8_t*>(header.signature),
            reinterpret_cast<const uint8_t*>(header.signature) +
                sizeof(details::PE_Magic),
            std::begin(signature_));
}

const Header::signature_t& Header::signature() const { return signature_; }

MACHINE_TYPES Header::machine() const { return machine_; }

uint16_t Header::numberof_sections() const { return numberOfSections_; }

uint32_t Header::time_date_stamp() const { return timeDateStamp_; }

uint32_t Header::pointerto_symbol_table() const {
  return pointerToSymbolTable_;
}

uint32_t Header::numberof_symbols() const { return numberOfSymbols_; }
...
void Header::machine(MACHINE_TYPES type) { machine_ = type; }

void Header::numberof_sections(uint16_t nbOfSections) {
  numberOfSections_ = nbOfSections;
}

void Header::time_date_stamp(uint32_t timestamp) { timeDateStamp_ = timestamp; }
...
void Header::signature(const Header::signature_t& sig) {
  std::copy(std::begin(sig), std::end(sig), std::begin(signature_));
}
...
std::ostream& operator<<(std::ostream& os, const Header& entry) {
  ...
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Signature: " << signature_str << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Machine: " << to_string(entry.machine_) << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Number Of Sections: " << entry.numberOfSections_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Pointer To Symbol Table: " << entry.pointerToSymbolTable_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Number Of Symbols: " << entry.numberOfSymbols_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Size Of Optional Header: " << entry.sizeOfOptionalHeader_ << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Characteristics: " << chara_str << std::endl;
  os << std::setw(30) << std::left << std::setfill(' ')
     << "Time Date Stamp: " << entry.timeDateStamp_ << std::endl;
