...
std::ostream& operator<<(std::ostream& os, const Header& entry) {
  ...
  const std::string& signature_str =
      fmt::format("{:02x} {:02x} {:02x} {:02x}", sig[0], sig[1], sig[2], sig[3]);
...
#define ENTRY(X) std::pair(Header::MACHINE_TYPES::X, #X)
  STRING_MAP enums2str{
      ENTRY(UNKNOWN),     ENTRY(ALPHA),     ENTRY(ALPHA64),   ENTRY(AM33),
      ENTRY(AMD64),       ENTRY(ARM),       ENTRY(ARMNT),     ENTRY(ARM64),
      ENTRY(EBC),         ENTRY(I386),      ENTRY(IA64),      ENTRY(LOONGARCH32),
      ENTRY(LOONGARCH64), ENTRY(M32R),      ENTRY(MIPS16),    ENTRY(MIPSFPU),
      ENTRY(MIPSFPU16),   ENTRY(POWERPC),   ENTRY(POWERPCFP), ENTRY(POWERPCBE),
      ENTRY(R4000),       ENTRY(RISCV32),   ENTRY(RISCV64),   ENTRY(RISCV128),
      ENTRY(SH3),         ENTRY(SH3DSP),    ENTRY(SH4),       ENTRY(SH5),
      ENTRY(THUMB),       ENTRY(WCEMIPSV2), ENTRY(ARM64EC),   ENTRY(ARM64X),
      ENTRY(CHPE_X86),
  };
#undef ENTRY
