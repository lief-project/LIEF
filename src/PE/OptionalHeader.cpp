...
std::vector<OptionalHeader::DLL_CHARACTERISTICS>
    OptionalHeader::dll_characteristics_list() const {
...
std::ostream& operator<<(std::ostream& os, const OptionalHeader& entry) {
  const std::vector<OptionalHeader::DLL_CHARACTERISTICS> list =
      entry.dll_characteristics_list();
  ...
  os << fmt::format("Magic:                      {:#06x} ({})\n",
                    (uint32_t)entry.magic(), to_string(entry.magic()))
     << fmt::format("Linker version:             {}.{}\n",
                    entry.major_linker_version(), entry.minor_linker_version())
     << fmt::format("Size of code:               {:#x}\n", entry.sizeof_code())
     << fmt::format("Size of initialized data:   {:#x}\n",
                    entry.sizeof_initialized_data())
     << fmt::format("Size of uninitialized data: {:#x}\n",
                    entry.sizeof_uninitialized_data())
     << fmt::format("Entrypoint:                 {:#x}\n",
                    entry.addressof_entrypoint())
     << fmt::format("Base of code:               {:#x}\n", entry.baseof_code())
     << fmt::format("Base of data:               {:#x}\n", entry.baseof_data())
     << fmt::format("Imagebase:                  {:#x}\n", entry.imagebase())
     << fmt::format("Section alignment:          {:#x}\n",
                    entry.section_alignment())
     << fmt::format("File alignment:             {:#x}\n", entry.file_alignment())
     << fmt::format("Operating system version:   {}.{}\n",
                    entry.major_operating_system_version(),
                    entry.minor_operating_system_version())
     << fmt::format("Image version:              {}.{}\n",
                    entry.major_image_version(), entry.minor_image_version())
     << fmt::format("Subsystem version:          {}.{}\n",
                    entry.major_subsystem_version(),
                    entry.minor_subsystem_version())
     << fmt::format("Win32 Version Value:        {:#x}\n",
                    entry.win32_version_value())
     << fmt::format("Size of image:              {:#x}\n", entry.sizeof_image())
     << fmt::format("Size of headers:            {:#x}\n", entry.sizeof_headers())
     << fmt::format("Checksum:                   {:#x}\n", entry.checksum())
     << fmt::format("Subsystem:                  {}\n",
                    to_string(entry.subsystem()))
     << fmt::format("Dll characteristics:        {}\n", fmt::join(list_str, ", "))
     << fmt::format("Size of stack reserve:      {:#x}\n",
                    entry.sizeof_stack_reserve())
     << fmt::format("Size of stack commit:       {:#x}\n",
                    entry.sizeof_stack_commit())
     << fmt::format("Size of heap reserve:       {:#x}\n",
                    entry.sizeof_heap_reserve())
     << fmt::format("Size of heap commit:        {:#x}\n",
                    entry.sizeof_heap_commit())
     << fmt::format("Loader flags:               {:#x}\n", entry.loader_flags())
...
const char* to_string(OptionalHeader::DLL_CHARACTERISTICS e) {
  CONST_MAP(OptionalHeader::DLL_CHARACTERISTICS, const char*, 11)
  enumStrings{
...
const char* to_string(OptionalHeader::SUBSYSTEM e) {
  CONST_MAP(OptionalHeader::SUBSYSTEM, const char*, 15)
  enumStrings{
      ...
      {OptionalHeader::SUBSYSTEM::EFI_BOOT_SERVICE_DRIVER,
      ...
      {OptionalHeader::SUBSYSTEM::WINDOWS_BOOT_APPLICATION,
