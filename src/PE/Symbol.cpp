...
int16_t Symbol::section_number() const { return section_number_; }

uint16_t Symbol::type() const { return type_; }
...
SYMBOL_STORAGE_CLASS Symbol::storage_class() const { return storage_class_; }

uint8_t Symbol::numberof_aux_symbols() const { return numberof_aux_symbols_; }

std::wstring Symbol::wname() const {
  return {std::begin(name_), std::end(name_)};
}

const Section* Symbol::section() const { return section_; }

Section* Symbol::section() {
  return const_cast<Section*>(static_cast<const Symbol*>(this)->section());
}

bool Symbol::has_section() const { return section_ != nullptr; }

void Symbol::accept(LIEF::Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const Symbol& entry) {
  ...
  if (entry.section_number() <= 0) {
    section_number_str =
        to_string(static_cast<SYMBOL_SECTION_NUMBER>(entry.section_number()));
  } else {
    if (entry.has_section()) {
      section_number_str = entry.section()->name();
    } else {
      section_number_str = std::to_string(
          static_cast<uint32_t>(entry.section_number()));  // section
    }
  }
  ...
  os << std::hex;
  os << std::left << std::setw(30) << name << std::setw(10) << entry.value()
     << std::setw(20) << section_number_str << std::setw(10)
     << to_string(entry.base_type()) << std::setw(10)
     << to_string(entry.complex_type()) << std::setw(10)
     << to_string(entry.storage_class());
