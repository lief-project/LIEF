...
static const std::map<MACHINE_TYPES, std::pair<ARCHITECTURES, std::set<MODES>>>
    arch_pe_to_lief{
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_UNKNOWN, {ARCH_NONE, {}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_AMD64, {ARCH_X86, {MODE_64}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_ARM,
         {ARCH_ARM, {MODE_32}}},  // MODE_LITTLE_ENDIAN
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_ARMNT,
         {ARCH_ARM, {MODE_32, MODE_V7, MODE_THUMB}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_ARM64,
         {ARCH_ARM64, {MODE_64, MODE_V8}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_I386, {ARCH_X86, {MODE_32}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_IA64, {ARCH_INTEL, {MODE_64}}},
        {MACHINE_TYPES::IMAGE_FILE_MACHINE_THUMB,
         {ARCH_ARM, {MODE_32, MODE_THUMB}}},
    };
...
Binary::Binary() { format_ = LIEF::EXE_FORMATS::FORMAT_PE; }

PE_TYPE Binary::type() const { return type_; }

Binary::Binary(const std::string& name, PE_TYPE type) : type_{type} {
  ...
  size_t sizeof_headers = dos_header().addressof_new_exeheader() +
                          ...
                          sizeof(details::pe_data_directory) *
                              details::DEFAULT_NUMBER_DATA_DIRECTORIES;
  if (type == PE_TYPE::PE32) {
    ...
    available_sections_space_ = (0x200 - /* sizeof headers */ sizeof_headers) /
  ...
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::EXPORT_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::IMPORT_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::RESOURCE_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::EXCEPTION_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::CERTIFICATE_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::BASE_RELOCATION_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::DEBUG));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::ARCHITECTURE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::GLOBAL_PTR));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::TLS_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::LOAD_CONFIG_TABLE));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::BOUND_IMPORT));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::IAT));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::DELAY_IMPORT_DESCRIPTOR));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::CLR_RUNTIME_HEADER));
  data_directories_.push_back(
      std::make_unique<DataDirectory>(DATA_DIRECTORY::RESERVED));
...
const TLS& Binary::tls() const { return tls_; }
...
uint64_t Binary::imagebase() const { return optional_header().imagebase(); }

uint64_t Binary::offset_to_virtual_address(uint64_t offset,
  ...
  const auto it_section =
      std::find_if(std::begin(sections_), std::end(sections_),
                   [RVA](const std::unique_ptr<Section>& section) {
                     const auto vsize_adj = std::max<uint64_t>(
...
DataDirectory& Binary::data_directory(DATA_DIRECTORY index) {
  return const_cast<DataDirectory&>(
      static_cast<const Binary*>(this)->data_directory(index));
}
...
bool Binary::has(DATA_DIRECTORY index) const {
  const auto it =
      std::find_if(std::begin(data_directories_), std::end(data_directories_),
...
bool Binary::has_rich_header() const { return has_rich_header_; }

bool Binary::has_tls() const { return has_tls_; }

bool Binary::has_imports() const { return has_imports_; }

bool Binary::has_signatures() const { return !signatures_.empty(); }

bool Binary::has_exports() const { return has_exports_; }
...
bool Binary::has_relocations() const { return has_relocations_; }

bool Binary::has_debug() const { return has_debug_; }

bool Binary::is_reproducible_build() const { return is_reproducible_build_; }
...
LoadConfiguration* Binary::load_configuration() {
  return const_cast<LoadConfiguration*>(
      static_cast<const Binary*>(this)->load_configuration());
}
...
Binary::it_sections Binary::sections() { return sections_; }
...
const Section* Binary::import_section() const {
  ...
  const DataDirectory& import_directory =
      data_directory(DATA_DIRECTORY::IMPORT_TABLE);
...
const DosHeader& Binary::dos_header() const { return dos_header_; }
...
OptionalHeader& Binary::optional_header() {
  return const_cast<OptionalHeader&>(
      static_cast<const Binary*>(this)->optional_header());
}
...
uint32_t Binary::sizeof_headers() const {
  ...
  size = static_cast<uint32_t>(
      LIEF::align(size, optional_header().file_alignment()));
  return size;
}
...
void Binary::remove(const Section& section, bool clear) {
  const auto it_section = std::find_if(
      std::begin(sections_), std::end(sections_),
      [&section](const std::unique_ptr<Section>& s) { return *s == section; });
...
Section& Binary::add_section(const Section& section, PE_SECTION_TYPES type) {
  ...
  const auto it_section = std::find_if(
      std::begin(sections_), std::end(sections_),
      [&type](const std::unique_ptr<Section>& s) { return s->is_type(type); });
  ...
  std::vector<uint8_t> content = {std::begin(content_ref),
                                  std::end(content_ref)};
  const auto section_size = static_cast<uint32_t>(content.size());
  const auto section_size_aligned = static_cast<uint32_t>(
      align(section_size, optional_header().file_alignment()));
  ...
  const uint64_t new_section_va =
      align(std::accumulate(std::begin(sections_), std::end(sections_),
  ...
  if (new_section->is_type(PE_SECTION_TYPES::TEXT)) {
    new_section->add_characteristic(
    ...
    optional_header().baseof_code(
        static_cast<uint32_t>(new_section->virtual_address()));
  ...
  if (type == PE_SECTION_TYPES::IMPORT) {
    ...
    data_directory(DATA_DIRECTORY::IMPORT_TABLE)
        .RVA(new_section->virtual_address());
    data_directory(DATA_DIRECTORY::IMPORT_TABLE)
        .size(new_section->sizeof_raw_data());
  ...
  if (type == PE_SECTION_TYPES::RELOCATION) {
    data_directory(DATA_DIRECTORY::BASE_RELOCATION_TABLE)
        .RVA(new_section->virtual_address());
    data_directory(DATA_DIRECTORY::BASE_RELOCATION_TABLE)
        .size(new_section->virtual_size());
    data_directory(DATA_DIRECTORY::BASE_RELOCATION_TABLE).section_ =
        new_section.get();
  }

  if (type == PE_SECTION_TYPES::RESOURCE) {
    data_directory(DATA_DIRECTORY::RESOURCE_TABLE)
        .RVA(new_section->virtual_address());
  ...
  if (type == PE_SECTION_TYPES::TLS) {
    data_directory(DATA_DIRECTORY::TLS_TABLE)
        .RVA(new_section->virtual_address());
...
Binary::it_relocations Binary::relocations() { return relocations_; }
...
void Binary::remove_all_relocations() { relocations_.clear(); }
...
Binary::it_imports Binary::imports() { return imports_; }

Binary::it_const_imports Binary::imports() const { return imports_; }

ImportEntry* Binary::add_import_function(const std::string& library,
                                         const std::string& function) {
  const auto it_import = std::find_if(
      std::begin(imports_), std::end(imports_),
      [&library](const Import& import) { return import.name() == library; });
...
void Binary::remove_all_libraries() { imports_.clear(); }

uint32_t Binary::predict_function_rva(const std::string& library,
                                      const std::string& function) {
  const auto it_import = std::find_if(
      imports_.cbegin(), imports_.cend(),
      [&library](const Import& imp) { return imp.name() == library; });
  ...
  size_t nb_functions = std::count_if(
      entries.cbegin(), entries.cend(), [&function](const ImportEntry& entry) {
  ...
  if (nb_functions > 1) {
    LIEF_ERR("{} is defined #{:d} times in {}", function, nb_functions,
             library);
    return 0;
  }

  uint32_t import_table_size = static_cast<uint32_t>(
      (imports().size() + 1) *
      sizeof(details::pe_import));  // +1 for the null entry
  ...
  for (auto it_imp = imports_.cbegin();
       (it_imp->name() != library && it_imp != imports_.cend()); ++it_imp) {
    if (type_ == PE_TYPE::PE32) {
  ...
  for (auto it_func = entries.cbegin();
       (it_func->name() != function && it_func != entries.cend()); ++it_func) {
    if (type_ == PE_TYPE::PE32) {
  ...
  const uint64_t next_virtual_address =
      align(std::accumulate(std::begin(sections_), std::end(sections_),
                            optional_header().section_alignment(),
                            [](uint64_t va, const std::unique_ptr<Section>& s) {
                              return std::max<uint64_t>(
...
const Export& Binary::get_export() const { return export_; }
...
ResourceNode* Binary::resources() {
  return const_cast<ResourceNode*>(
      static_cast<const Binary*>(this)->resources());
}

const ResourceNode* Binary::resources() const { return resources_.get(); }
...
Binary::debug_entries_t& Binary::debug() {
  return const_cast<debug_entries_t&>(
      static_cast<const Binary*>(this)->debug());
}

const Binary::debug_entries_t& Binary::debug() const { return debug_; }
...
Binary::it_const_signatures Binary::signatures() const { return signatures_; }
...
std::vector<uint8_t> Binary::authentihash(ALGORITHMS algo) const {
  ...
  for (const std::unique_ptr<Section>& sec : sections_) {
    ...
    uint32_t name_length =
        std::min<uint32_t>(sec_name.size() + 1, sizeof(name));
  ...
  for (const Section* sec : sections) {
    ...
    LIEF_DEBUG(
        "Authentihash:  Append section {:<8}: [0x{:04x}, 0x{:04x}] + "
        "[0x{:04x}] = [0x{:04x}, 0x{:04x}]",
        sec->name(), sec->offset(), sec->offset() + content.size(), pad.size(),
  ...
  if (!overlay_.empty()) {
    const DataDirectory& cert_dir =
        data_directory(DATA_DIRECTORY::CERTIFICATE_TABLE);
    LIEF_DEBUG("Add overlay and omit 0x{:08x} - 0x{:08x}", cert_dir.RVA(),
    ...
    if (cert_dir.RVA() > 0 && cert_dir.size() > 0 &&
      ...
      if (end_cert_offset <= overlay_.size()) {
        LIEF_DEBUG("Add [0x{:x}, 0x{:x}]", overlay_offset_,
                   overlay_offset_ + start_cert_offset);
        LIEF_DEBUG("Add [0x{:x}, 0x{:x}]", overlay_offset_ + end_cert_offset,
  ...
  // std::ofstream output_file{"/tmp/hash.blob", std::ios::out |
  // std::ios::binary | std::ios::trunc}; if (output_file) {
  //   std::copy(
  //       std::begin(out),
  //       std::end(out),
...
Signature::VERIFICATION_FLAGS Binary::verify_signature(
...
Signature::VERIFICATION_FLAGS Binary::verify_signature(
    const Signature& sig, Signature::VERIFICATION_CHECKS checks) const {
  Signature::VERIFICATION_FLAGS flags = Signature::VERIFICATION_FLAGS::OK;
  ...
  if (authhash != chash) {
    LIEF_INFO(
        "Authentihash and Content info's digest does not match:\n  {}\n  {}",
...
std::vector<Symbol>& Binary::symbols() {
  return const_cast<std::vector<Symbol>&>(
      static_cast<const Binary*>(this)->symbols());
}

const std::vector<Symbol>& Binary::symbols() const { return symbols_; }
...
LIEF::Binary::functions_t Binary::get_abstract_exported_functions() const {
  ...
  if (has_exports()) {
    for (const ExportEntry& entry : get_export().entries()) {
      ...
      if (!name.empty()) {
        result.emplace_back(name, entry.address(),
...
void Binary::hook_function(const std::string& library,
                           const std::string& function, uint64_t address) {
...
void Binary::patch_address(uint64_t address,
  ...
  if (offset + patch_value.size() > content.size()) {
    LIEF_ERR(
        "The patch value ({} bytes @0x{:x}) is out of bounds of the section "
        "(limit: 0x{:x})",
...
void Binary::patch_address(uint64_t address, uint64_t patch_value, size_t size,
  ...
  switch (size) {
    ...
    default: {
      LIEF_ERR("The provided size ({}) does not match the size of an integer",
               size);
      return;
    }
  }
...
std::vector<uint8_t> Binary::get_content_from_virtual_address(
    uint64_t virtual_address, uint64_t size,
    LIEF::Binary::VA_TYPES addr_type) const {
...
const std::vector<uint8_t>& Binary::overlay() const { return overlay_; }

std::vector<uint8_t>& Binary::overlay() { return overlay_; }

// Dos stub
// ========

const std::vector<uint8_t>& Binary::dos_stub() const { return dos_stub_; }

std::vector<uint8_t>& Binary::dos_stub() { return dos_stub_; }
...
// Rich Header
// -----------
RichHeader& Binary::rich_header() { return rich_header_; }

const RichHeader& Binary::rich_header() const { return rich_header_; }
...
LIEF::Binary::functions_t Binary::exception_functions() const {
  ...
  const DataDirectory& exception_dir =
      data_directory(DATA_DIRECTORY::EXCEPTION_TABLE);
  std::vector<uint8_t> exception_data = get_content_from_virtual_address(
      exception_dir.RVA(), exception_dir.size());
  VectorStream vs{std::move(exception_data)};
  const size_t nb_entries =
      vs.size() /
      sizeof(
          details::pe_exception_entry_x64);  // TODO: Handle other architectures
...
// Delay Imports
// ========================================================
bool Binary::has_delay_imports() const { return !delay_imports_.empty(); }

Binary::it_delay_imports Binary::delay_imports() { return delay_imports_; }
...
const DelayImport* Binary::get_delay_import(
    const std::string& import_name) const {
  const auto it_import =
      std::find_if(std::begin(delay_imports_), std::end(delay_imports_),
