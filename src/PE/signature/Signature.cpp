...
inline std::string time_to_string(const x509::date_t& date) {
  return fmt::format("{:d}/{:02d}/{:02d} - {:02d}:{:02d}:{:02d}", date[0], date[1],
...
Signature::VERIFICATION_FLAGS
    verify_ts_counter_signature(const SignerInfo& signer,
                                const PKCS9CounterSignature& cs,
  ...
  // Verify 1.
  const auto* content_type_data = static_cast<const ContentType*>(
      cs_signer.get_auth_attribute(Attribute::TYPE::CONTENT_TYPE)
  );
  if (content_type_data == nullptr) {
    LIEF_WARN("Missing ContentType in authenticated attributes in the counter "
              "signature's signer");
    return flags | Signature::VERIFICATION_FLAGS::INVALID_SIGNER;
  }

  if (content_type_data->oid() != /* PKCS #7 Data */ "1.2.840.113549.1.7.1") {
    LIEF_WARN("Bad OID for ContentType in authenticated attributes in the counter "
              "signature's signer ({})",
  ...
  // Verify 3.
  const auto* message_dg = static_cast<const PKCS9MessageDigest*>(
      cs_signer.get_auth_attribute(Attribute::TYPE::PKCS9_MESSAGE_DIGEST)
  );
  if (message_dg == nullptr) {
    LIEF_WARN("Missing MessageDigest in authenticated attributes in the counter "
              "signature's signer");
    return flags | Signature::VERIFICATION_FLAGS::INVALID_SIGNER;
  }
   ...
   * validity window.
   */
  const auto* signing_time = static_cast<const PKCS9SigningTime*>(
      cs_signer.get_auth_attribute(Attribute::TYPE::PKCS9_SIGNING_TIME)
  );
...
std::vector<uint8_t> Signature::hash(const uint8_t* buffer, size_t size,
                                     ALGORITHMS algo) {
  switch (algo) {
...
Signature::VERIFICATION_FLAGS Signature::check(VERIFICATION_CHECKS checks) const {
  ...
  const auto& spc_indirect_data =
      static_cast<const SpcIndirectData&>(content_info_.value());
  ...
  const std::vector<uint8_t> content_info_hash =
      Signature::hash(raw_content_info, digest_algo);
  ...
  if (!auth_attrs.empty()) {
    ...
    auto it_pkcs9_digest =
        std::find_if(auth_attrs.begin(), auth_attrs.end(),
    ...
    /*
     * If there is no authenticated attributes, then the encrypted digested should
     * match ENC(content_info_hash)
     */
   ...
   */
  const auto* counter = static_cast<const PKCS9CounterSignature*>(
      signer.get_unauth_attribute(Attribute::TYPE::PKCS9_COUNTER_SIGNATURE)
  );
  bool has_ms_counter_sig =
      signer.get_unauth_attribute(Attribute::TYPE::MS_COUNTER_SIGN) != nullptr;
