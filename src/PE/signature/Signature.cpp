...
inline std::string time_to_string(const x509::date_t& date) {
  return fmt::format("{:d}/{:02d}/{:02d} - {:02d}:{:02d}:{:02d}", date[0],
                     date[1], date[2], date[3], date[4], date[5]);
}

std::string Signature::flag_to_string(Signature::VERIFICATION_FLAGS flag) {
  CONST_MAP(VERIFICATION_FLAGS, const char*, 13)
  enumStrings{
...
Signature::VERIFICATION_FLAGS verify_ts_counter_signature(
    const SignerInfo& signer, const PKCS9CounterSignature& cs,
  ...
  if (cs_signer.cert() == nullptr) {
    LIEF_WARN(
        "Can't find x509 certificate associated with Counter Signature's "
        "signer");
    return flags | Signature::VERIFICATION_FLAGS::CERT_NOT_FOUND;
  }
  const x509& cs_cert = *cs_signer.cert();
  const SignerInfo::encrypted_digest_t& cs_enc_digest =
      cs_signer.encrypted_digest();
  ...
  const std::vector<uint8_t>& cs_hash =
      Signature::hash(cs_auth_data, cs_digest_algo);
  LIEF_DEBUG("Signed data digest: {}", hex_dump(cs_hash));
  bool check_sig =
      cs_cert.check_signature(cs_hash, cs_enc_digest, cs_digest_algo);
   ...
   * The Authenticode timestamp SignerInfo structure contains the following
   * authenticated attributes values:
   * 1. ContentType (1.2.840.113549.1.9.3) is set to PKCS #7 Data
   * (1.2.840.113549.1.7.1).
   * 2. Signing Time (1.2.840.113549.1.9.5) is set to the UTC time of timestamp
   * generation time.
  ...
  // Verify 1.
  const auto* content_type_data = reinterpret_cast<const ContentType*>(
      cs_signer.get_auth_attribute(SIG_ATTRIBUTE_TYPES::CONTENT_TYPE));
  if (content_type_data == nullptr) {
    LIEF_WARN(
        "Missing ContentType in authenticated attributes in the counter "
        "signature's signer");
  ...
  if (content_type_data->oid() != /* PKCS #7 Data */ "1.2.840.113549.1.7.1") {
    LIEF_WARN(
        "Bad OID for ContentType in authenticated attributes in the counter "
        "signature's signer ({})",
  ...
  // Verify 3.
  const auto* message_dg = reinterpret_cast<const PKCS9MessageDigest*>(
      cs_signer.get_auth_attribute(SIG_ATTRIBUTE_TYPES::PKCS9_MESSAGE_DIGEST));
  if (message_dg == nullptr) {
    LIEF_WARN(
        "Missing MessageDigest in authenticated attributes in the counter "
        "signature's signer");
    return flags | Signature::VERIFICATION_FLAGS::INVALID_SIGNER;
  }
  const std::vector<uint8_t>& dg_value = message_dg->digest();
  const std::vector<uint8_t> dg_cs_hash =
      Signature::hash(signer.encrypted_digest(), cs_digest_algo);
   ...
   * validity window.
   */
  const auto* signing_time = reinterpret_cast<const PKCS9SigningTime*>(
      cs_signer.get_auth_attribute(SIG_ATTRIBUTE_TYPES::PKCS9_SIGNING_TIME));
  if (signing_time != nullptr &&
      !is_true(checks & Signature::VERIFICATION_CHECKS::SKIP_CERT_TIME)) {
...
std::vector<uint8_t> Signature::hash(const std::vector<uint8_t>& input,
                                     ALGORITHMS algo) {
  switch (algo) {
    case ALGORITHMS::SHA_512: {
      std::vector<uint8_t> out(64);
      int ret =
          mbedtls_sha512(input.data(), input.size(), out.data(), /* is384 */ 0);
      if (ret != 0) {
        LIEF_ERR("Hashing {} bytes with SHA-512 failed! (ret: 0x{:x})",
                 input.size(), ret);
        return {};
      }
      return out;
    ...
    case ALGORITHMS::SHA_384: {
      std::vector<uint8_t> out(64);
      int ret =
          mbedtls_sha512(input.data(), input.size(), out.data(), /* is384 */ 1);
      if (ret != 0) {
        LIEF_ERR("Hashing {} bytes with SHA-384 failed! (ret: 0x{:x})",
                 input.size(), ret);
        return {};
      }
      return out;
    ...
    case ALGORITHMS::SHA_256: {
      ...
      int ret =
          mbedtls_sha256(input.data(), input.size(), out.data(), /* is224 */ 0);
      if (ret != 0) {
        LIEF_ERR("Hashing {} bytes with SHA-256 failed! (ret: 0x{:x})",
                 input.size(), ret);
        return {};
      }
      return out;
    ...
    case ALGORITHMS::SHA_1: {
      ...
      if (ret != 0) {
        LIEF_ERR("Hashing {} bytes with SHA-1 failed! (ret: 0x{:x})",
                 input.size(), ret);
        return {};
      }
      return out;
    ...
    case ALGORITHMS::MD5: {
      ...
      if (ret != 0) {
        LIEF_ERR("Hashing {} bytes with MD5 failed! (ret: 0x{:x})",
                 input.size(), ret);
        return {};
      }
      return out;
...
uint32_t Signature::version() const { return version_; }

const ContentInfo& Signature::content_info() const { return content_info_; }

Signature::it_const_crt Signature::certificates() const {
  return certificates_;
}

Signature::it_const_signers_t Signature::signers() const { return signers_; }

Signature::VERIFICATION_FLAGS Signature::check(
    VERIFICATION_CHECKS checks) const {
  ...
  if (signer.cert() == nullptr) {
    LIEF_WARN("Can't find certificate for which the issuer is {}",
              signer.issuer());
  ...
  const std::vector<uint8_t> content_info_hash =
      Signature::hash(std::move(raw_content_info), digest_algo);

  // Copy authenticated attributes
  SignerInfo::it_const_attributes_t auth_attrs =
  ...
  if (auth_attrs.size() > 0) {
    ...
    const std::vector<uint8_t> auth_attr_hash =
        Signature::hash(auth_data, digest_algo);
    LIEF_DEBUG("Authenticated attribute digest: {}", hex_dump(auth_attr_hash));
    bool check_sig =
        cert.check_signature(auth_attr_hash, enc_digest, digest_algo);
    ...
    auto it_pkcs9_digest = std::find_if(
        std::begin(auth_attrs), std::end(auth_attrs),
    ...
    if (it_pkcs9_digest == std::end(auth_attrs)) {
      LIEF_WARN(
          "Can't find the authenticated attribute: 'pkcs9-message-digest'");
      return flags | VERIFICATION_FLAGS::MISSING_PKCS9_MESSAGE_DIGEST;
    }

    const auto& digest_attr =
        reinterpret_cast<const PKCS9MessageDigest&>(*it_pkcs9_digest);
    ...
    /*
     * If there is no authenticated attributes, then the encrypted digested
     * should match ENC(content_info_hash)
     */
   ...
   */
  const auto* counter = reinterpret_cast<const PKCS9CounterSignature*>(
      signer.get_unauth_attribute(
          SIG_ATTRIBUTE_TYPES::PKCS9_COUNTER_SIGNATURE));
  ...
  for (const Attribute& attr : signer.unauthenticated_attributes()) {
    if (attr.type() == SIG_ATTRIBUTE_TYPES::GENERIC_TYPE) {
      if (reinterpret_cast<const GenericType&>(attr).oid() ==
          /* Ms-CounterSign */ "1.3.6.1.4.1.311.3.3.1") {
  ...
  if (counter != nullptr) {
    VERIFICATION_FLAGS cs_flags =
        verify_ts_counter_signature(signer, *counter, checks);
  ...
  bool should_check_cert_time =
      !timeless_signature ||
      is_true(checks & VERIFICATION_CHECKS::LIFETIME_SIGNING);
...
const x509* Signature::find_crt(const std::vector<uint8_t>& serialno) const {
  auto it_cert =
      std::find_if(std::begin(certificates_), std::end(certificates_),
...
const x509* Signature::find_crt_subject(const std::string& subject) const {
  auto it_cert = std::find_if(
      std::begin(certificates_), std::end(certificates_),
      [&subject](const x509& cert) { return cert.subject() == subject; });
  if (it_cert == std::end(certificates_)) {
    return nullptr;
  }
  return &(*it_cert);
...
const x509* Signature::find_crt_subject(
    const std::string& subject, const std::vector<uint8_t>& serialno) const {
  auto it_cert = std::find_if(
      std::begin(certificates_), std::end(certificates_),
...
const x509* Signature::find_crt_issuer(const std::string& issuer) const {
  auto it_cert = std::find_if(
      std::begin(certificates_), std::end(certificates_),
      [&issuer](const x509& cert) { return cert.issuer() == issuer; });
  if (it_cert == std::end(certificates_)) {
    return nullptr;
  }
  return &(*it_cert);
...
const x509* Signature::find_crt_issuer(
...
void Signature::accept(Visitor& visitor) const { visitor.visit(*this); }

inline void print_attr(SignerInfo::it_const_attributes_t& attrs,
                       std::ostream& os) {
  for (const Attribute& attr : attrs) {
    ...
    switch (attr.type()) {
      ...
      case SIG_ATTRIBUTE_TYPES::MS_SPC_NESTED_SIGN: {
        const auto& nested_attr =
            reinterpret_cast<const MsSpcNestedSignature&>(attr);
      ...
      case SIG_ATTRIBUTE_TYPES::PKCS9_SIGNING_TIME: {
        ...
        suffix = fmt::format("{}/{}/{} - {}:{}:{}", time[0], time[1], time[2],
...
std::ostream& operator<<(std::ostream& os, const Signature& signature) {
  ...
  for (const SignerInfo& signer : signers) {
    ...
    SignerInfo::it_const_attributes_t auth_attr =
        signer.authenticated_attributes();
    if (auth_attr.size() > 0) {
    ...
    SignerInfo::it_const_attributes_t unauth_attr =
        signer.unauthenticated_attributes();
