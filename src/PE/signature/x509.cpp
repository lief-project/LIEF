...
inline x509::VERIFICATION_FLAGS from_mbedtls_err(uint32_t err) {
  CONST_MAP(uint32_t, x509::VERIFICATION_FLAGS, 20)
  MBEDTLS_ERR_TO_LIEF = {
...
x509::KEY_TYPES x509::key_type() const {
  CONST_MAP(mbedtls_pk_type_t, x509::KEY_TYPES, 6)
  mtype2asi = {
...
bool x509::check_signature(const std::vector<uint8_t>& hash,
  ...
  CONST_MAP(ALGORITHMS, mbedtls_md_type_t, 5)
  LIEF2MBED_MD = {
   ...
   * data. On some samples, this encoding failed.
   *
