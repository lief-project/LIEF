...
/*
   ...
   GetLocalTime |
                                      +-----------------------+ +--------------+
                                                                     +--->|
   LocalSize   | Trampoline 2                 |    +--------------+
                                      +-----------------------+      |    |
   WriteFile   | |  mov rax, [new addr2] |      |    +--------------+ |  jmp rax
   |------+
                                      +-----------------------+
...
void Builder::build_import_table() {
  ...
  for (const Import& import : binary_->imports()) {
    ...
    entries_size +=
        2 * (import.entries().size() + 1) *
        sizeof(uint__);  // Once for Lookup table and the other for Import
                         // Address Table (IAT). +1 for the null entry
  ...
  // Address table (IAT). Identical to the lookup table until the library is
  // bound
  uint32_t iat_offset = lookuptable_offset + entries_size / 2;

  // Offset to the section which will contains hints/names of the imported
  // functions name
  ...
  size_t content_size_aligned =
      align(content.size(), binary_->optional_header().file_alignment());
  content.insert(std::end(content), content_size_aligned - content.size(), 0);

  // Create a new section to handle imports
  Section new_import_section{".l" + std::to_string(static_cast<uint32_t>(
                                        DATA_DIRECTORY::IMPORT_TABLE))};
  new_import_section.content(content);

  new_import_section.add_characteristic(
      SECTION_CHARACTERISTICS::IMAGE_SCN_CNT_CODE);

  const auto it_import_section = std::find_if(
  ...
  // As add_section will change DATA_DIRECTORY::IMPORT_TABLE we have to save it
  // before
  uint32_t offset_imports = binary_->rva_to_offset(
      binary_->data_directory(DATA_DIRECTORY::IMPORT_TABLE).RVA());
  Section& import_section =
      binary_->add_section(new_import_section, PE_SECTION_TYPES::IMPORT);
  ...
  if (patch_imports_) {
    ...
    auto* import_header = reinterpret_cast<details::pe_import*>(
        import_content.data() + roffset_import);
    uint32_t jumpOffsetTmp = trampolines_offset;
    while (import_header->ImportAddressTableRVA != 0) {
      uint32_t offsetTable =
          binary_->rva_to_offset(import_header->ImportLookupTableRVA) -
          original_import->pointerto_raw_data();
      uint32_t offsetIAT =
          binary_->rva_to_offset(import_header->ImportAddressTableRVA) -
          original_import->pointerto_raw_data();
      if (offsetTable > import_content.size() ||
          offsetIAT > import_content.size()) {
        // TODO: Better handle
        LIEF_ERR("Can't patch");
        break;
      }
      auto* lookupTable =
          reinterpret_cast<uint__*>(import_content.data() + offsetTable);
      auto* IAT = reinterpret_cast<uint__*>(import_content.data() + offsetIAT);

      while (*lookupTable != 0) {
        *IAT = static_cast<uint__>(binary_->optional_header().imagebase() +
  ...
  for (const Import& import : binary_->imports()) {
    ...
    header.ImportLookupTableRVA = static_cast<uint__>(
        import_section.virtual_address() + lookuptable_offset);
    ...
    header.NameRVA = static_cast<uint__>(import_section.virtual_address() +
                                         libraries_name_offset);
    header.ImportAddressTableRVA =
        static_cast<uint__>(import_section.virtual_address() + iat_offset);
    ...
    std::copy(std::begin(import_name), std::end(import_name),
    ...
    for (const ImportEntry& entry : import.entries()) {
      ...
      if (patch_imports_) {
        ...
        uint64_t address = binary_->optional_header().imagebase() +
                           import_section.virtual_address() + iat_offset;
        if (binary_->hooks_.count(import_name) > 0 &&
            binary_->hooks_[import_name].count(entry.name())) {
          address = binary_->hooks_[import_name][entry.name()];
          instructions = Builder::build_jmp_hook<PE_T>(
              binary_->optional_header().imagebase() +
                  import_section.virtual_address() + trampolines_offset,
              address);
        } else {
          instructions = Builder::build_jmp<PE_T>(
              binary_->optional_header().imagebase() +
                  import_section.virtual_address() + trampolines_offset,
              address);
        }
        std::copy(std::begin(instructions), std::end(instructions),
      ...
      if (!entry.is_ordinal()) {
        lookup_table_value =
            import_section.virtual_address() + functions_name_offset;
        ...
        std::copy(reinterpret_cast<const uint8_t*>(&hint),
        ...
        functions_name_offset +=
            functions_name_offset % 2;  // Require to be even
      }
      ...
      std::copy(reinterpret_cast<const uint8_t*>(&lookup_table_value),
                reinterpret_cast<const uint8_t*>(&lookup_table_value) +
                    sizeof(uint__),
                content.data() + lookuptable_offset);

      std::copy(reinterpret_cast<const uint8_t*>(&iat_value),
  ...
  // Insert null entry at the end
  std::memset(content.data() + import_table_offset, 0,
  ...
  const auto rva =
      static_cast<uint32_t>(import_section.virtual_address() + iat_offset);
  binary_->data_directory(DATA_DIRECTORY::IAT).RVA(rva);
  binary_->data_directory(DATA_DIRECTORY::IAT)
      .size(functions_name_offset - iat_offset + 1);
}

template <typename PE_T>
ok_error_t Builder::build_optional_header(
  ...
  optional_header_raw.MajorLinkerVersion =
      static_cast<uint8_t>(optional_header.major_linker_version());
  optional_header_raw.MinorLinkerVersion =
      static_cast<uint8_t>(optional_header.minor_linker_version());
  optional_header_raw.SizeOfCode =
      static_cast<uint32_t>(optional_header.sizeof_code());
  optional_header_raw.SizeOfInitializedData =
      static_cast<uint32_t>(optional_header.sizeof_initialized_data());
  optional_header_raw.SizeOfUninitializedData =
      static_cast<uint32_t>(optional_header.sizeof_uninitialized_data());
  optional_header_raw.AddressOfEntryPoint =
      static_cast<uint32_t>(optional_header.addressof_entrypoint());
  optional_header_raw.BaseOfCode =
      static_cast<uint32_t>(optional_header.baseof_code());

  if (std::is_same<PE_T, details::PE32>::value) {
    reinterpret_cast<details::pe32_optional_header*>(&optional_header_raw)
        ->BaseOfData = static_cast<uint32_t>(optional_header.baseof_data());
  }
  optional_header_raw.ImageBase =
      static_cast<uint__>(optional_header.imagebase());
  optional_header_raw.SectionAlignment =
      static_cast<uint32_t>(optional_header.section_alignment());
  optional_header_raw.FileAlignment =
      static_cast<uint32_t>(optional_header.file_alignment());
  optional_header_raw.MajorOperatingSystemVersion =
      static_cast<uint16_t>(optional_header.major_operating_system_version());
  optional_header_raw.MinorOperatingSystemVersion =
      static_cast<uint16_t>(optional_header.minor_operating_system_version());
  optional_header_raw.MajorImageVersion =
      static_cast<uint16_t>(optional_header.major_image_version());
  optional_header_raw.MinorImageVersion =
      static_cast<uint16_t>(optional_header.minor_image_version());
  optional_header_raw.MajorSubsystemVersion =
      static_cast<uint16_t>(optional_header.major_subsystem_version());
  optional_header_raw.MinorSubsystemVersion =
      static_cast<uint16_t>(optional_header.minor_subsystem_version());
  optional_header_raw.Win32VersionValue =
      static_cast<uint16_t>(optional_header.win32_version_value());
  optional_header_raw.SizeOfImage =
      static_cast<uint32_t>(optional_header.sizeof_image());
  optional_header_raw.SizeOfHeaders =
      static_cast<uint32_t>(optional_header.sizeof_headers());
  optional_header_raw.CheckSum =
      static_cast<uint32_t>(optional_header.checksum());
  optional_header_raw.Subsystem =
      static_cast<uint16_t>(optional_header.subsystem());
  optional_header_raw.DLLCharacteristics =
      static_cast<uint16_t>(optional_header.dll_characteristics());
  optional_header_raw.SizeOfStackReserve =
      static_cast<uint__>(optional_header.sizeof_stack_reserve());
  optional_header_raw.SizeOfStackCommit =
      static_cast<uint__>(optional_header.sizeof_stack_commit());
  optional_header_raw.SizeOfHeapReserve =
      static_cast<uint__>(optional_header.sizeof_heap_reserve());
  optional_header_raw.SizeOfHeapCommit =
      static_cast<uint__>(optional_header.sizeof_heap_commit());
  optional_header_raw.LoaderFlags =
      static_cast<uint32_t>(optional_header.loader_flags());
  optional_header_raw.NumberOfRvaAndSize =
      static_cast<uint32_t>(optional_header.numberof_rva_and_size());
  ...
  ios_.write(reinterpret_cast<const uint8_t*>(&optional_header_raw),
...
ok_error_t Builder::build_tls() {
  ...
  const auto it_tls = std::find_if(
      std::begin(binary_->sections_), std::end(binary_->sections_),
      [](const std::unique_ptr<Section>& section) {
        const std::set<PE_SECTION_TYPES>& types = section->types();
        return types.size() == 1 &&
               types.find(PE_SECTION_TYPES::TLS) != std::end(types);
      });
  ...
  if (it_tls == std::end(binary_->sections_)) {
    Section new_section{".l" +
                        std::to_string(static_cast<uint32_t>(
                            DATA_DIRECTORY::TLS_TABLE))};  // .l9 -> lief.tls
    new_section.characteristics(0xC0300040);
    uint64_t tls_section_size = sizeof(pe_tls);

    const uint64_t offset_callbacks =
        binary_->va_to_offset(tls_obj.addressof_callbacks());
    const uint64_t offset_rawdata =
        binary_->va_to_offset(tls_obj.addressof_raw_data().first);
    ...
    tls_section_size =
        align(tls_section_size, binary_->optional_header().file_alignment());
  ...
  tls_raw.RawDataStartVA =
      static_cast<uint__>(tls_obj.addressof_raw_data().first);
  tls_raw.RawDataEndVA =
      static_cast<uint__>(tls_obj.addressof_raw_data().second);
  tls_raw.AddressOfIndex = static_cast<uint__>(tls_obj.addressof_index());
  tls_raw.AddressOfCallback =
      static_cast<uint__>(tls_obj.addressof_callbacks());
  ...
  std::copy(reinterpret_cast<uint8_t*>(&tls_raw),
            reinterpret_cast<uint8_t*>(&tls_raw) + sizeof(pe_tls), data.data());
  ...
  if (section_callbacks == tls_section) {
    ...
    // data.insert(std::begin(data) + relative_offset + sizeof(uint__),
    // sizeof(uint__), 0);
  ...
  }
  {
    ...
    if (section_rawdata == tls_section) {
      ...
      const uint64_t relative_offset =
          offset_rawdata - section_rawdata->offset();
