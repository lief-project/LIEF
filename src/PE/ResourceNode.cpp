...
uint32_t ResourceNode::id() const { return id_; }

ResourceNode::it_childs ResourceNode::childs() { return childs_; }

ResourceNode::it_const_childs ResourceNode::childs() const { return childs_; }

const std::u16string& ResourceNode::name() const { return name_; }

bool ResourceNode::is_directory() const { return type_ == TYPE::DIRECTORY; }

bool ResourceNode::is_data() const { return type_ == TYPE::DATA; }

bool ResourceNode::has_name() const {
  return static_cast<bool>(id() & 0x80000000);
}

uint32_t ResourceNode::depth() const { return depth_; }
...
void ResourceNode::delete_child(uint32_t id) {
  const auto it_node =
      std::find_if(std::begin(childs_), std::end(childs_),
...
void ResourceNode::id(uint32_t id) { id_ = id; }

void ResourceNode::name(const std::string& name) { this->name(u8tou16(name)); }

void ResourceNode::name(const std::u16string& name) { name_ = name; }
...
void ResourceNode::accept(Visitor& visitor) const { visitor.visit(*this); }
...
std::ostream& operator<<(std::ostream& os, const ResourceNode& node) {
  ...
  os << " - ID: 0x" << std::setw(2) << std::setfill('0') << std::hex
     << node.id();
  if (node.has_name()) {
