...
void JsonVisitor::visit(const DosHeader& dos_header) {
  ...
  node_["used_bytes_in_the_last_page"] =
...
void JsonVisitor::visit(const OptionalHeader& optional_header) {
  ...
  node_["sizeof_uninitialized_data"] =
  ...
  node_["major_operating_system_version"] =
      optional_header.major_operating_system_version();
  node_["minor_operating_system_version"] =
...
void JsonVisitor::visit(const ExportEntry& export_entry) {
  ...
  if (export_entry.is_forwarded()) {
    const ExportEntry::forward_information_t& fwd_info =
        export_entry.forward_information();
...
void JsonVisitor::visit(const TLS& tls) {
  ...
  node_["addressof_raw_data"] = std::vector<uint64_t>{
      tls.addressof_raw_data().first, tls.addressof_raw_data().second};
...
void JsonVisitor::visit(const ResourcesManager& resources_manager) {
  ...
  if (resources_manager.has_string_table()) {
    ...
    try {
      for (const ResourceStringTable& string_table :
           resources_manager.string_table()) {
...
void JsonVisitor::visit(const LangCodeItem& resource_lci) {
  ...
  std::transform(std::begin(resource_lci.items()),
                 ...
                 [](const std::pair<std::u16string, std::u16string>& p) {
                   return std::pair<std::string, std::string>{
                       u16tou8(p.first), u16tou8(p.second)};
                 });
...
void JsonVisitor::visit(const LoadConfiguration& config) {
  ...
  node_["critical_section_default_timeout"] =
      config.critical_section_default_timeout();
  node_["decommit_free_block_threshold"] =
      config.decommit_free_block_threshold();
  node_["decommit_total_free_threshold"] =
...
void JsonVisitor::visit(const LoadConfigurationV1& config) {
  node_["guard_cf_check_function_pointer"] =
      config.guard_cf_check_function_pointer();
  node_["guard_cf_dispatch_function_pointer"] =
...
void JsonVisitor::visit(const LoadConfigurationV3& config) {
  node_["guard_address_taken_iat_entry_table"] =
      config.guard_address_taken_iat_entry_table();
  node_["guard_address_taken_iat_entry_count"] =
...
void JsonVisitor::visit(const LoadConfigurationV5& config) {
  ...
  node_["guard_rf_failure_routine_function_pointer"] =
      config.guard_rf_failure_routine_function_pointer();
  node_["dynamic_value_reloctable_offset"] =
      config.dynamic_value_reloctable_offset();
  node_["dynamic_value_reloctable_section"] =
...
void JsonVisitor::visit(const LoadConfigurationV6& config) {
  node_["guard_rf_verify_stackpointer_function_pointer"] =
