...
def test_multidex():
    WallpaperCropper2 = lief.OAT.parse(
        get_sample("OAT/OAT_064_AArch64_WallpaperCropper2.oat")
    )
    assert WallpaperCropper2 is not None
...
def test_oat_classes():
    ...
    # OAT Class 1
    cls = WallpaperCropper2.get_class(
        "com.android.keyguard.KeyguardTransportControlView$SavedState$1"
    )
    assert cls is not None
...
def test_oat_methods():
    ...
    assert method.oat_class == WallpaperCropper2.get_class(
        "com/android/gallery3d/ds/DsWallpaperSetting"
    )
