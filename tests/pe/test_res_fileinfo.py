...
def test_issue_686():
    ...
    """
    path = utils.get_sample(
        "PE/res/bdfa7195fc318cb4f232c63479fd904887e2989a8bbd57aab9eeb57210a7471a.neut"
    )
    pe = lief.PE.parse(path)

    assert pe is not None

    mgr686 = pe.resources_manager
    assert isinstance(mgr686, lief.PE.ResourcesManager)
    versions = mgr686.version
    assert len(versions) == 1
    sfi = versions[0].string_file_info
    assert sfi is not None
    assert len(sfi.children) == 1
...
def test_issue_687():
    ...
    """
    path = utils.get_sample(
        "PE/res/9b58db32f6224e213cfd130d6cd7a18b2440332bfd99e0aef4313de8099fa955.neut"
    )
    pe = lief.PE.parse(path)
    ...
    assert [d.title for d in dialogs[0].items] == [  # type: ignore
        "&More Information",
        "OK",
        "Phone Dialer 1.50",
    ...
    assert [d.title for d in dialogs[1].items] == [  # type: ignore
        "&Server Address",
        None,
        "OK",
        "Cancel",
    ...
    assert [d.title for d in dialogs[3].items] == [None]  # type: ignore

    assert dialogs[6].title == "More Speed Dial Entries"
    assert [d.title for d in dialogs[6].items] == [  # type: ignore
        118,
        "Select Entry to Dial",
        "List1",
        "Place &Call",
        "&Edit...",
        "Cancel",
    ...
    assert [d.title for d in dialogs[7].items] == [  # type: ignore
        "Entrie&s",
        "List1",
        "&OK",
        "&Cancel",
    ...
    assert [d.title for d in dialogs[8].items] == [  # type: ignore
        118,
        "Enter a display name and a phone number or network address",
        "&Display name:",
        None,
        "&Number or address:",
        None,
    ...
    assert [d.title for d in dialogs[9].items] == []  # type: ignore

    assert dialogs[10].title == "Take Picture"
    assert [d.title for d in dialogs[10].items] == [  # type: ignore
        "&Video Device",
        None,
        "&Format...",
        "&Source...",
    ...
    assert [d.title for d in dialogs[12].items] == [  # type: ignore
        "Enter the name of the user to add.  You must search the network directory to verify that the user exists.",
        "&User:",
        None,
        "&Search",
        "Search &Results:",
...
def test_issue_689():
    ...
    mgr689 = pe.resources_manager
    assert isinstance(mgr689, lief.PE.ResourcesManager)
    versions = mgr689.version
    assert len(versions) == 1
    sfi = versions[0].string_file_info
    assert sfi is not None
    assert len(sfi.children) == 1
    ...
    product_name = table["ProductName"]
    assert product_name is not None
    assert (
        product_name.encode("utf8")
        == b"\xe6\x95\xb0\xe5\xad\x97\xe7\xad\xbe\xe5\x90\x8d\xe5\xb7\xa5\xe5\x85\xb7(\xe5\x91\xbd\xe4\xbb\xa4\xe8\xa1\x8c)"
    )
    ...
    file_description = table["FileDescription"]
    assert file_description is not None
    assert (
        file_description.encode("utf8")
        == b"\xe6\x95\xb0\xe5\xad\x97\xe7\xad\xbe\xe5\x90\x8d\xe5\xb7\xa5\xe5\x85\xb7(\xe5\x91\xbd\xe4\xbb\xa4\xe8\xa1\x8c)"
    )
    company_name = table["CompanyName"]
    assert company_name is not None
    assert (
        company_name.encode("utf8")
        == b"\xe4\xb8\x8a\xe6\xb5\xb7\xe5\x9f\x9f\xe8\x81\x94\xe8\xbd\xaf\xe4\xbb\xb6\xe6\x8a\x80\xe6\x9c\xaf\xe6\x9c\x89\xe9\x99\x90\xe5\x85\xac\xe5\x8f\xb8"
    )
...
def test_issue_691():
    ...
    """
    path = utils.get_sample(
        "PE/res/1dc4c94163a436c401d163e317e5bdbb55a84600a63ad11805c892d4ad1e5be3.neut"
    )
    pe = lief.PE.parse(path)
    assert pe is not None

    mgr691 = pe.resources_manager
    assert isinstance(mgr691, lief.PE.ResourcesManager)
    versions = mgr691.version
    assert len(versions) == 1
    sfi = versions[0].string_file_info
    assert sfi is not None
    assert len(sfi.children) == 1
...
def test_issue_693():
    ...
    """
    path = utils.get_sample(
        "PE/res/4bfaa99393f635cd05d91a64de73edb5639412c129e049f0fe34f88517a10fc6.neut"
    )
    pe = lief.PE.parse(path)

    assert pe is not None

    mgr693a = pe.resources_manager
    assert isinstance(mgr693a, lief.PE.ResourcesManager)
    versions = mgr693a.version
    assert len(versions) == 1
    sfi = versions[0].string_file_info
    assert sfi is not None
    assert len(sfi.children) == 1
    ...
    path = utils.get_sample(
        "PE/res/731bb363a01f45b64c0065e1cdfe8cc653930f102f715bc5073ac77c1d4bae2a.neut"
    )
    pe = lief.PE.parse(path)

    assert pe is not None

    mgr693b = pe.resources_manager
    assert isinstance(mgr693b, lief.PE.ResourcesManager)
    versions = mgr693b.version
    assert len(versions) == 1
    sfi = versions[0].string_file_info
    assert sfi is not None
    assert len(sfi.children) == 1
