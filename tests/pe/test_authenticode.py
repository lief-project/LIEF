...
def test_api():
    ...
    assert avast.authentihash(lief.PE.ALGORITHMS.SHA_256) == from_hex(
        "a7:38:da:44:46:a4:e7:8a:b6:47:db:7e:53:42:7e:b0:79:61:c9:94:31:7f:4c:59:d7:ed:be:a5:cc:78:6d:80"
    )
    assert avast.authentihash(lief.PE.ALGORITHMS.SHA_256) == avast.authentihash_sha256

    assert avast.authentihash(lief.PE.ALGORITHMS.SHA_512) == from_hex(
        "2a:e7:4c:81:0d:65:7b:6a:49:48:94:ab:b9:7d:fa:03:18:5d:48:cf:cd:4e:c2:99:f6:49:5f:db:30:64:78:03:f6:60:90:ab:04:84:01:36:7e:b0:6e:f6:29:b1:d1:a8:49:51:c3:4e:b3:75:89:c9:74:62:a2:2e:d2:ac:6e:96"
    )
    ...
    assert isinstance(spc_indirect_data, lief.PE.SpcIndirectData)
    ...
    assert spc_indirect_data.digest == from_hex(
        "a7:38:da:44:46:a4:e7:8a:b6:47:db:7e:53:42:7e:b0:79:61:c9:94:31:7f:4c:59:d7:ed:be:a5:cc:78:6d:80"
    )
    ...
    assert cert_ca.serial_number == from_hex(
    ...
    assert cert_signer.serial_number == from_hex(
    ...
    assert (
        cert_signer.subject
        == "C=CZ, L=Praha, O=Avast Software s.r.o., OU=RE stapler cistodc, CN=Avast Software s.r.o."
    )

    assert (
        cert_ca.verify(cert_signer) == lief.PE.x509.VERIFICATION_FLAGS.BADCERT_EXPIRED
    )
    assert (
        cert_ca.verify(cert_ca) == lief.PE.x509.VERIFICATION_FLAGS.BADCERT_NOT_TRUSTED
    )
    assert (
        cert_signer.is_trusted_by([cert_ca])
    ...
    assert signer.serial_number == from_hex(
        "09:70:ef:4b:ad:5c:c4:4a:1c:2b:c3:d9:64:01:67:4c"
    )
    assert (
        signer.issuer
...
def test_json_serialization():
    ...
    with open(
        get_sample(
            "PE/PE32_x86-64_binary_avast-free-antivirus-setup-online-signature.json"
        ),
        "rb",
    ) as f:
...
def test_fail():
    ...
    avast_altered = parse_pe(
        "PE/PE32_x86-64_binary_avast-free-antivirus-setup-online-altered-dos-stub.exe"
    )
    assert avast_altered.verify_signature() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    assert (
        avast_altered.signatures[0].check() == lief.PE.Signature.VERIFICATION_FLAGS.OK
    )

    avast_altered = parse_pe(
        "PE/PE32_x86-64_binary_avast-free-antivirus-setup-online-altered-encrypted-digest.exe"
    )
    assert avast_altered.verify_signature() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    assert (
        avast_altered.signatures[0].check() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    ...
    avast_altered = parse_pe(
        "PE/PE32_x86-64_binary_avast-free-antivirus-setup-online-altered-content-info-digest.exe"
    )
    assert avast_altered.verify_signature() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    assert (
        avast_altered.signatures[0].check() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    ...
    avast_altered = parse_pe(
        "PE/PE32_x86-64_binary_avast-free-antivirus-setup-online-altered-pkcs9-msg-digest.exe"
    )
    assert avast_altered.verify_signature() != lief.PE.Signature.VERIFICATION_FLAGS.OK
    assert (
        avast_altered.signatures[0].check() != lief.PE.Signature.VERIFICATION_FLAGS.OK
...
def test_pkcs9_signing_time():
    ...
    assert isinstance(attr, lief.PE.PKCS9SigningTime)
...
def test_pkcs9_at_sequence_number():
    ...
    assert sig is not None
    nested_sig_attr = sig.signers[0].get_attribute(
        lief.PE.Attribute.TYPE.MS_SPC_NESTED_SIGN
    )
    assert nested_sig_attr is not None
    assert isinstance(nested_sig_attr, lief.PE.MsSpcNestedSignature)
    nested_sig = nested_sig_attr.signature
    at_seq_nb = nested_sig.signers[0].get_attribute(
        lief.PE.Attribute.TYPE.PKCS9_AT_SEQUENCE_NUMBER
    )
    assert at_seq_nb is not None
    assert isinstance(at_seq_nb, lief.PE.PKCS9AtSequenceNumber)
...
def test_spc_sp_opus_info():
    ...
    assert isinstance(spc, lief.PE.SpcSpOpusInfo)
...
def test_pkcs9_counter_signature():
    ...
    assert sig is not None
    counter_sign = sig.signers[0].get_attribute(
        lief.PE.Attribute.TYPE.PKCS9_COUNTER_SIGNATURE
    )
    assert counter_sign is not None
    assert isinstance(counter_sign, lief.PE.PKCS9CounterSignature)
    ...
    assert isinstance(content_type, lief.PE.ContentType)
    assert isinstance(sig_time, lief.PE.PKCS9SigningTime)
    assert isinstance(msg_digest, lief.PE.PKCS9MessageDigest)
    assert content_type.oid == "1.2.840.113549.1.7.1"
    assert sig_time.time == [2018, 7, 25, 18, 14, 50]
    assert msg_digest.digest == from_hex(
        "05:ca:7d:34:f0:ef:c2:70:33:4c:f9:90:77:a5:bc:86:6e:46:be:45"
    )
...
def test_ms_spc_nested_signature():
    ...
    assert attr is not None
    assert isinstance(attr, lief.PE.MsSpcNestedSignature)
    ...
    assert spc_indirect_data.digest_algorithm == lief.PE.ALGORITHMS.SHA_256
    assert spc_indirect_data.digest == from_hex(
        "90:a4:df:36:26:df:d9:8d:6b:3b:1d:42:74:5b:94:54:c5:e2:30:2e:d2:f8:23:70:16:3f:1e:e6:dd:7d:8c:91"
    )
    ...
    assert (
        nvidia_cert.subject
        == "C=US, ST=California, L=Santa Clara, O=NVIDIA Corporation, OU=IT-MIS, CN=NVIDIA Corporation"
    )
    assert nvidia_cert.serial_number == from_hex(
        "62:E7:45:E9:21:65:21:3C:97:1F:5C:49:0A:EA:12:A5"
    )

    assert (
        self_signed_ca.issuer
        == r"C=US, O=VeriSign\, Inc., OU=VeriSign Trust Network, OU=(c) 2008 VeriSign\, Inc. - For authorized use only, CN=VeriSign Universal Root Certification Authority"
    )
    assert (
        self_signed_ca.subject
        == r"C=US, O=VeriSign\, Inc., OU=VeriSign Trust Network, OU=(c) 2008 VeriSign\, Inc. - For authorized use only, CN=VeriSign Universal Root Certification Authority"
    )
    assert self_signed_ca.serial_number == from_hex(
    ...
    assert signer_cert.serial_number == from_hex(
    ...
    assert (
        self_signed_ca.verify(signer_cert)
        == lief.PE.x509.VERIFICATION_FLAGS.BADCERT_EXPIRED
    )
    assert (
        signer_cert.verify(nvidia_cert)
        == lief.PE.x509.VERIFICATION_FLAGS.BADCERT_EXPIRED
    )

    ca_bundles = lief.PE.x509.parse(get_sample("pkcs7/windows-ca-bundle.pem"))
    assert (
        self_signed_ca.is_trusted_by(ca_bundles) == lief.PE.x509.VERIFICATION_FLAGS.OK
    )
    assert (
        nvidia_cert.is_trusted_by(ca_bundles)
...
def test_self_signed():
    selfsigned = parse_pe("PE/PE32_x86-64_binary_self-signed.exe")
    ...
    assert (
        cert_ca.is_trusted_by(ca_bundles)
        == lief.PE.x509.VERIFICATION_FLAGS.BADCERT_NOT_TRUSTED
    )
...
def test_rsa_info():
    avast = parse_pe("PE/PE32_x86-64_binary_avast-free-antivirus-setup-online.exe")
    cert_ca, _cert_signer = avast.signatures[0].certificates
    assert cert_ca.key_type == lief.PE.x509.KEY_TYPES.RSA
    rsa_info = cert_ca.rsa_info
    assert rsa_info is not None
    ...
    assert (
        str(N)[:70]
        == "2645636708930440977121533117630461323983500317859148049010916849328467"
    )
    assert D == 0
    assert P == 0
    assert Q == 0
...
def test_verification_flags_str():
    ...
    assert (
        str(lief.PE.Signature.VERIFICATION_FLAGS.from_value(0))
        == "VERIFICATION_FLAGS.OK"
    )


def test_ms_manifest_binary_id():
    acres = parse_pe("PE/AcRes.dll")
    attr = (
        acres.signatures[0]
        .signers[0]
        .get_auth_attribute(lief.PE.Attribute.TYPE.MS_PLATFORM_MANIFEST_BINARY_ID)
    )
    assert attr is not None
    assert isinstance(attr, lief.PE.MsManifestBinaryID)
    assert attr.manifest_id == "Q3XarTZK62/v5aPftDNzWYB5ybbMDvHGQIYjVa+ja+0="


def test_ms_counter_signature():
    acres = parse_pe("PE/AppVClient.exe")
    sig = acres.signatures[0]
    _ms_counter_sig = sig.signers[0].get_unauth_attribute(
        lief.PE.Attribute.TYPE.MS_COUNTER_SIGN
    )
    assert _ms_counter_sig is not None
    ms_counter_sig = cast(lief.PE.MsCounterSign, _ms_counter_sig)
...
def test_playready_signature():
    pe = parse_pe("private/PE/Windows.Media.Protection.PlayReady.dll")

    sig = pe.signatures[0]
    spc = sig.signers[0].get_auth_attribute(
...
def test_font_signature():
    ...
    assert sig is not None
    value = sig.content_info.value
    assert isinstance(value, lief.PE.SpcIndirectData)
    assert (
        value.url
        == "\x00<\x00<\x00<\x00O\x00b\x00s\x00o\x00l\x00e\x00t\x00e\x00>\x00>\x00>"
    )
    attr = sig.signers[0].authenticated_attributes[4]
    assert isinstance(attr, lief.PE.SpcSpOpusInfo)
    assert attr.program_name == "this is a test"
    assert attr.more_info == "Monotype"
    ...
    attr = sig.signers[0].authenticated_attributes[3]
    assert isinstance(attr, lief.PE.SpcSpOpusInfo)
    assert attr.program_name == "Microsoft Typography"
    assert attr.more_info == "http://www.microsoft.com/Typography "


def test_issue_1219():
    pe = parse_pe(
        "PE/issue_1219_be5d6c4aa4b27548a06c2afaef3b4035abf65566e9a8bfd642b4a2032729656e.exe_MALWARE"
    )
