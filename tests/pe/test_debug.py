...
def test_pgo():
    ...
    dbg_section = debugs[0].section
    assert dbg_section is not None
    assert dbg_section.name == ".rdata"
    ...
    pgo_copy = pgo.copy()
    assert isinstance(pgo_copy, lief.PE.Pogo)
    assert len(pgo_copy.entries) == len(pgo.entries)
    entries = pgo.entries
...
def test_code_view_pdb():
    ...
    debug_code_view = list(
        filter(lambda deb: isinstance(deb, lief.PE.CodeViewPDB), sample.debug)
    )
    ...
    assert (
        code_view.filename
        == r"c:\users\romain\documents\visual studio 2015\Projects\HelloWorld\x64\Release\ConsoleApplication1.pdb"
    )
    assert code_view.copy() == code_view

    code_view_copy = code_view.copy()
    assert code_view_copy is not None
    assert isinstance(code_view_copy, lief.PE.CodeViewPDB)
    assert hash(code_view.parent) == hash(code_view_copy.parent)
...
def test_overlay_dbg():
    ...
    cv = cast(lief.PE.CodeView, pe.debug[0])
...
def test_pdbchecksum(tmp_path: Path):
    ...
    pdbchecksum = cast(lief.PE.PDBChecksum, pe.debug[1])
    assert isinstance(pdbchecksum, lief.PE.PDBChecksum)

    assert (
        bytes(pdbchecksum.hash).hex(":")
    ...
    pdbchecksum = cast(lief.PE.PDBChecksum, new_pe.debug[1])
...
def test_vc_features(tmp_path: Path):
    ...
    pe = lief.PE.parse(input_path)
    assert pe is not None
    vcfeat = cast(lief.PE.VCFeature, pe.debug[1])
    ...
    new_pe = lief.PE.parse(output_path)
    assert new_pe is not None
    vcfeat = cast(lief.PE.VCFeature, new_pe.debug[1])
    assert vcfeat.gs == 39
...
def test_ex_dll_characteristics(tmp_path: Path):
    ...
    pe = lief.PE.parse(input_path)
    assert pe is not None
    entry = cast(lief.PE.ExDllCharacteristics, pe.debug[3])
    ...
    assert not entry.has(
        lief.PE.ExDllCharacteristics.CHARACTERISTICS.CET_COMPAT_STRICT_MODE
    )
    assert (
        entry.ex_characteristics
        == lief.PE.ExDllCharacteristics.CHARACTERISTICS.HOTPATCH_COMPATIBLE
    )
    assert entry.ex_characteristics_list == [
        lief.PE.ExDllCharacteristics.CHARACTERISTICS.HOTPATCH_COMPATIBLE
    ]

    entry.characteristics = int(
    ...
    new_pe = lief.PE.parse(output_path)
    assert new_pe is not None
    entry = cast(lief.PE.ExDllCharacteristics, new_pe.debug[3])
...
def test_fpo(tmp_path: Path):
    ...
    pe = lief.PE.parse(input_path)
    assert pe is not None
    fpo = cast(lief.PE.FPO, pe.debug[1])
