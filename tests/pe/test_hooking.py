...
def test_hook_64(tmp_path):
    sample_path = get_sample('PE/PE64_x86-64_binary_HelloWorld.exe')
    output      = tmp_path / "pe64_hooking.exe"

    hello = lief.parse(sample_path)
    code = [0x48, 0x83, 0xc4, 0x48,                                     # add rsp, 0x48         ; Stack unwind
    ...
    if sys.platform.startswith("win"):
        ...
        p = Popen(["START", output.as_posix(), "foo"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=subprocess_flags)
        time.sleep(3)
...
def test_hook_32(tmp_path):
    sample_path = get_sample('PE/PE32_x86_binary_HelloWorld.exe')
    output      = tmp_path / "pe32_hooking.exe"

    hello = lief.parse(sample_path)
    code = [0x6a, 0x00,                   # push 0               ; hWnd
