...
) -> lief.assembly.aarch64.Instruction:
    return cast(
        lief.assembly.aarch64.Instruction,
        next(elf.disassemble_from_bytes(elf.assemble(addr, asm), addr)),
    )
...
def test_arm64e():
    ...
    _inst = next(macho.disassemble(0x016650))
    assert _inst is not None
    assert _inst.to_string() == "0x016650: adrp x17, #16384"
    ...
    _inst = next(macho.disassemble(0x01665C))
    assert _inst is not None
    assert _inst.to_string() == "0x01665c: braa x18, x19"


def test_pe_arm64():
    pe = parse_pe("PE/elf_reader.arm64.pe.exe")
...
def test_elf_arm64():
    ...
    instructions = list(elf.disassemble_from_bytes(bytes(text_section.content)))
    ...
    elf = parse_elf("ELF/libmonochrome-arm64.so")
