...
if __name__ == "__main__":
    if not is_linux() and not is_x86_64():
        print(
            "Melkor fuzzing is currently only supported on Linux x86-64",
            file=sys.stderr,
        )
        sys.exit(0)
    ...
    parser.add_argument(
        "--input-seed", dest="input_seed", type=str, default="/usr/bin/ls"
    )

    parser.add_argument(
        "--numberof_samples", "-n", dest="nb_samples", type=int, default=100
    )
