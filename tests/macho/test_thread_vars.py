...
def test_check_layout():
    macho = parse_macho("private/MachO/thread_variables.dylib").at(0)
    assert macho is not None
    ok, msg = lief.MachO.check_layout(macho)
    assert not ok
    assert (
        msg
        == "malformed thread-local, offset=0x9000000000 is larger than total size=0x000090\n"
    )
