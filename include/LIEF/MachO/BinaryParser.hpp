...
class LIEF_API BinaryParser : public LIEF::Parser {
  ...
  constexpr static size_t MAX_RELOCATIONS =
 ...
 private:
  ...
  using it_opaque_segments = void*;  // To avoid including Binary.hpp. It must
                                     // contains it_opaque_segments
  ...
  ok_error_t do_bind(BINDING_CLASS cls, uint8_t type, uint8_t segment_idx,
                     ...
                     it_opaque_segments segments_ptr, uint64_t offset = 0);

  template <class MACHO_T>
  ok_error_t do_rebase(uint8_t type, uint8_t segment_idx,
  ...
  ok_error_t parse_export_trie(uint64_t start, uint64_t end,
