...
enum class MACHO_SEGMENTS_FLAGS : size_t {
  ...
  SG_HIGHVM = 0x1u,  ///< The file contents for this segment are for the high
                     ///< part of the virtual memory space; the low part is zero
                     ///< filled (for stacks in core files).
  SG_FVMLIB = 0x2u,  ///< this segment is the VM that is allocated by a fixed VM
...
//! @brief These are the section type and attributes fields.  A MachO section
//! can have only one Type, but can have any of the attributes specified.
enum class MACHO_SECTION_TYPES : size_t {
  ...
  S_NON_LAZY_SYMBOL_POINTERS =
      0x06u,                       ///< Section with non-lazy symbol pointers.
  S_LAZY_SYMBOL_POINTERS = 0x07u,  ///< Section with lazy symbol pointers.
  S_SYMBOL_STUBS = 0x08u,  ///< Section with symbol stubs, byte size of stub in
                           ///< the Reserved2 field.
  ...
  S_GB_ZEROFILL = 0x0cu,  ///< Zero fill on demand section (that can be larger
                          ///< than 4 gigabytes).
  S_INTERPOSING =
...
enum class MACHO_SECTION_FLAGS : size_t {
  ...
  S_ATTR_NO_TOC = 0x40000000u,  ///< Section contains coalesced symbols that are
                                ///< not to be in a ranlib table of contents.
  S_ATTR_STRIP_STATIC_SYMS =
      0x20000000u,  ///< Ok to strip static symbols in this section in files
                    ///< with the MY_DYLDLINK flag.
  S_ATTR_NO_DEAD_STRIP = 0x10000000u,  ///< No dead stripping.
  S_ATTR_LIVE_SUPPORT =
      0x08000000u,  ///< Blocks are live if they reference live blocks.
...
enum class REBASE_OPCODES : uint8_t {
  ...
  REBASE_OPCODE_SET_TYPE_IMM =
      0x10u,  ///< Set type to immediate (lower 4-bits). Used for ordinal
              ///< numbers from 0-15
  ...
  REBASE_OPCODE_ADD_ADDR_IMM_SCALED =
      0x40u,  ///< Add segment's offset with immediate scaling
  REBASE_OPCODE_DO_REBASE_IMM_TIMES =
      0x50u,  ///< Rebase in the range of ``[segment's offset; segment's offset
  ...
  REBASE_OPCODE_DO_REBASE_ULEB_TIMES_SKIPPING_ULEB =
      0x80u  ///< Rebase and skip several bytes
};
...
enum class BIND_OPCODES : uint8_t {
  ...
  BIND_OPCODE_SET_DYLIB_ORDINAL_IMM =
      0x10u,  ///< Set ordinal to immediate (lower 4-bits). Used for ordinal
              ///< numbers from 0-15
  BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB =
      0x20u,  ///< Set ordinal to following ULEB128 encoding. Used for ordinal
              ///< numbers from 16+
  BIND_OPCODE_SET_DYLIB_SPECIAL_IMM =
      0x30u,  ///< Set ordinal, with 0 or negative number as immediate. the
  ...
  BIND_OPCODE_DO_BIND_ADD_ADDR_IMM_SCALED =
      0xB0u,  ///< Perform binding, also add immediate (lower 4-bits) using
              ///< scaling
  BIND_OPCODE_DO_BIND_ULEB_TIMES_SKIPPING_ULEB =
      0xC0u,  ///< Perform binding for several symbols (as following ULEB128),
              ///< and skip several bytes.
...
enum class EXPORT_SYMBOL_FLAGS : size_t {
  EXPORT_SYMBOL_FLAGS_KIND_MASK =
      0x03u,  ///< Mask to access to EXPORT_SYMBOL_KINDS
...
enum class MACHO_SYMBOL_TYPES : size_t {
  ...
  N_EXT = 0x01    ///< External symbol, a symbol that is either defined outside
                  ///< this file or that is defined in this file but can be
...
enum class N_LIST_TYPES : size_t {
  N_UNDF = 0x0u,  ///< The symbol is undefined. It is referenced in a different
                  ///< module.
  N_ABS =
      0x2u,  ///< The symbol is absolute. The linker doesn't update his value.
  N_SECT = 0xeu,  ///< The symbol is defined in the section number given in
                  ///< nlist_base.n_sect .
  N_PBUD = 0xcu,  ///< The symbol is undefined and the image is using a prebound
...
enum class SYMBOL_DESCRIPTIONS : size_t {
  ...
  REFERENCE_FLAG_UNDEFINED_NON_LAZY =
      0,  ///< Reference to an external non-lazy (data) symbol.
  REFERENCE_FLAG_UNDEFINED_LAZY = 1,  ///< Reference to an external lazy symbol
                                      ///< - that is, to a function call.
  REFERENCE_FLAG_DEFINED = 2,         ///< Defined in this module.
  REFERENCE_FLAG_PRIVATE_DEFINED =
      3,  ///< Defined in this module and is visible only to modules within this
          ///< shared library.
  REFERENCE_FLAG_PRIVATE_UNDEFINED_NON_LAZY =
      4,  ///< Defined in another module in this file, is a non-lazy (data)
          ///< symbol, and is visible only to modules within this shared
          ///< library.
  REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY =
      5,  ///< Defined in another module in this file, is a lazy (function)
          ///< symbol, and is visible only to modules within this shared
          ///< library.
...
enum class X86_RELOCATION : size_t {
  ...
  GENERIC_RELOC_SECTDIFF =
      2, /**< A relocation entry for an item that contains the difference of two
            section addresses. This is generally used for position-independent
            code generation. */
  GENERIC_RELOC_PB_LA_PTR =
      3, /**< contains the address from which to subtract; it must be followed
            by a X86_RELOCATION::GENERIC_RELOC_PAIR containing the address to
            subtract.*/
  GENERIC_RELOC_LOCAL_SECTDIFF =
      4, /**< Similar to X86_RELOCATION::GENERIC_RELOC_SECTDIFF except that this
            entry refers specifically to the address in this item. If the
            address is that of a globally visible coalesced symbol, this
            relocation entry does not change if the symbol is overridden. This
            is used to associate stack unwinding information with the object
            code this relocation entry describes.*/
  GENERIC_RELOC_TLV =
      5, /**< A relocation entry for a prebound lazy pointer. This is always a
            scattered relocation entry. The MachO::Relocation::value field
            contains the non-prebound value of the lazy pointer.*/
};
...
enum class ARM64_RELOCATION : size_t {
  ...
  ARM64_RELOC_GOT_LOAD_PAGEOFF12 = 6, /**< Offset within page of GOT slot,
                                         scaled by MachO::Relocation::size. */
  ARM64_RELOC_POINTER_TO_GOT = 7,     /**< For pointers to GOT slots. */
  ARM64_RELOC_TLVP_LOAD_PAGE21 =
      8, /**< PC-rel distance to page of TLVP slot. */
  ARM64_RELOC_TLVP_LOAD_PAGEOFF12 = 9, /**< Offset within page of TLVP slot,
                                          scaled by MachO::Relocation::size.*/
  ARM64_RELOC_ADDEND =
...
enum class CPU_SUBTYPES_ARM64 : size_t { CPU_SUBTYPE_ARM64_ALL = 0 };

enum class CPU_SUBTYPES_SPARC : size_t { CPU_SUBTYPE_SPARC_ALL = 0 };
...
static const HEADER_FLAGS header_flags_array[] = {
    HEADER_FLAGS::MH_NOUNDEFS,
    HEADER_FLAGS::MH_INCRLINK,
    HEADER_FLAGS::MH_DYLDLINK,
    HEADER_FLAGS::MH_BINDATLOAD,
    HEADER_FLAGS::MH_PREBOUND,
    HEADER_FLAGS::MH_SPLIT_SEGS,
    HEADER_FLAGS::MH_LAZY_INIT,
    HEADER_FLAGS::MH_TWOLEVEL,
    HEADER_FLAGS::MH_FORCE_FLAT,
    HEADER_FLAGS::MH_NOMULTIDEFS,
    HEADER_FLAGS::MH_NOFIXPREBINDING,
    HEADER_FLAGS::MH_PREBINDABLE,
    HEADER_FLAGS::MH_ALLMODSBOUND,
    HEADER_FLAGS::MH_SUBSECTIONS_VIA_SYMBOLS,
    HEADER_FLAGS::MH_CANONICAL,
    HEADER_FLAGS::MH_WEAK_DEFINES,
    HEADER_FLAGS::MH_BINDS_TO_WEAK,
    HEADER_FLAGS::MH_ALLOW_STACK_EXECUTION,
    HEADER_FLAGS::MH_ROOT_SAFE,
    HEADER_FLAGS::MH_SETUID_SAFE,
    HEADER_FLAGS::MH_NO_REEXPORTED_DYLIBS,
    HEADER_FLAGS::MH_PIE,
    HEADER_FLAGS::MH_DEAD_STRIPPABLE_DYLIB,
    HEADER_FLAGS::MH_HAS_TLV_DESCRIPTORS,
    HEADER_FLAGS::MH_NO_HEAP_EXECUTION,
    HEADER_FLAGS::MH_APP_EXTENSION_SAFE};

static const MACHO_SECTION_FLAGS section_flags_array[] = {
    MACHO_SECTION_FLAGS::S_ATTR_PURE_INSTRUCTIONS,
    MACHO_SECTION_FLAGS::S_ATTR_NO_TOC,
    MACHO_SECTION_FLAGS::S_ATTR_STRIP_STATIC_SYMS,
    MACHO_SECTION_FLAGS::S_ATTR_NO_DEAD_STRIP,
    MACHO_SECTION_FLAGS::S_ATTR_LIVE_SUPPORT,
    MACHO_SECTION_FLAGS::S_ATTR_SELF_MODIFYING_CODE,
    MACHO_SECTION_FLAGS::S_ATTR_DEBUG,
    MACHO_SECTION_FLAGS::S_ATTR_SOME_INSTRUCTIONS,
    MACHO_SECTION_FLAGS::S_ATTR_EXT_RELOC,
    MACHO_SECTION_FLAGS::S_ATTR_LOC_RELOC};
