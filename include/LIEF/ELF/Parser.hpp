...
class LIEF_API Parser : public LIEF::Parser {
 ...
 public:
  ...
  //!
  //! For weird binaries (e.g. sectionless) you can choose which method to use
  //! for counting dynamic symbols
  //!
  ...
  static std::unique_ptr<Binary> parse(
 ...
 protected:
  ...
  ok_error_t parse_dynamic_relocations(uint64_t relocations_offset,
                                       uint64_t size);
  ...
  template <typename ELF_T>
  ok_error_t parse_symbol_version_requirement(uint64_t offset,
                                              uint32_t nb_entries);
  ...
  template <typename ELF_T>
  ok_error_t parse_symbol_version_definition(uint64_t offset,
                                             uint32_t nb_entries);
  ...
  template <typename ELF_T, typename REL_T>
  uint32_t max_relocation_index(uint64_t relocations_offset,
                                uint64_t size) const;

  //! Check if the given Section is wrapped by the given segment
  static bool check_section_in_segment(const Section& section,
