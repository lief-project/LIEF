...
enum OAT_CLASS_TYPES {
  ...
  OAT_CLASS_SOME_COMPILED = 1,  //! A bitmap of which OatMethodOffsets are
                                //! present follows the OatClass.
  OAT_CLASS_NONE_COMPILED =
      2,  //! All methods are interpreted so no OatMethodOffsets are necessary.
};
...
enum OAT_CLASS_STATUS {
  ...
  STATUS_NOTREADY = 0,
  STATUS_IDX =
      1,  // Loaded, DEX idx in super_class_type_idx_ and interfaces_type_idx_.
  ...
  STATUS_RETRY_VERIFICATION_AT_RUNTIME =
      6,  // Compile time verification failed, retry at runtime.
...
static const HEADER_KEYS header_keys_list[] = {
    HEADER_KEYS::KEY_IMAGE_LOCATION,     HEADER_KEYS::KEY_DEX2OAT_CMD_LINE,
    HEADER_KEYS::KEY_DEX2OAT_HOST,       HEADER_KEYS::KEY_PIC,
    HEADER_KEYS::KEY_HAS_PATCH_INFO,     HEADER_KEYS::KEY_DEBUGGABLE,
    HEADER_KEYS::KEY_NATIVE_DEBUGGABLE,  HEADER_KEYS::KEY_COMPILER_FILTER,
    HEADER_KEYS::KEY_CLASS_PATH,         HEADER_KEYS::KEY_BOOT_CLASS_PATH,
    HEADER_KEYS::KEY_CONCURRENT_COPYING, HEADER_KEYS::KE_COMPILATION_REASON,
};
