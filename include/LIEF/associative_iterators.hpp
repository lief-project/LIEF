...
struct dict_iterator_pair {
 public:
  ...
  dict_iterator_pair(KEY key, VALUE value) : key_{key}, value_{value} {}

  add_const_t<key_ref_t> key() const {
    return const_cast<add_const_t<key_ref_t>>(key_);
  }

  typename std::enable_if<!std::is_const<key_ref_t>::value,
                          remove_const_t<key_ref_t>>::type
  key() const {
    return const_cast<remove_const_t<key_ref_t>>(
        static_cast<const dict_iterator_pair*>(this)->key());
  }
  ...
  typename std::enable_if<!std::is_const<value_ref_t>::value,
                          remove_const_t<value_ref_t>>::type
  value() const {
    return const_cast<remove_const_t<value_ref_t>>(
        static_cast<const dict_iterator_pair*>(this)->value());
  }
...
template <class CONTAINER_T,
          class ITERATOR_T = typename decay_t<CONTAINER_T>::iterator>
class dict_iterator
    : public std::iterator<
          std::bidirectional_iterator_tag,
          dict_iterator_pair<
              add_pointer_t<
                  decay_t<remove_pointer_t<typename CONTAINER_T::key_type>>>,
              add_pointer_t<decay_t<
                  remove_pointer_t<typename CONTAINER_T::mapped_type>>>>,
          ssize_t,
          dict_iterator_pair<
              add_pointer_t<
                  decay_t<remove_pointer_t<typename CONTAINER_T::key_type>>>,
 ...
 public:
  using DT = decay_t<T>;
  using value_ref_t =
      add_ref_t<remove_pointer_t<decay_t<typename CONTAINER_T::mapped_type>>>;
  using value_pointer_t = add_pointer_t<
      remove_pointer_t<decay_t<typename CONTAINER_T::mapped_type>>>;

  using key_pointer_t =
      add_pointer_t<remove_pointer_t<decay_t<typename CONTAINER_T::key_type>>>;
  ...
  using result_t = dict_iterator_pair<
      ...
      add_pointer_t<
          decay_t<remove_pointer_t<typename CONTAINER_T::mapped_type>>>>;
  ...
  dict_iterator& operator+=(
      const typename dict_iterator::difference_type& movement) {
  ...
  // remove_const_t<ref_t>>::type operator[](size_t n) {
  //   return const_cast<remove_const_t<ref_t>>(static_cast<const
  //   dict_iterator*>(this)->operator[](n));
  // }
  ...
  typename dict_iterator::difference_type operator-(
      const dict_iterator& rhs) const {
    return distance_ - rhs.distance_;
  }

  bool operator<(const dict_iterator& rhs) const { return (rhs - *this) > 0; }

  bool operator>(const dict_iterator& rhs) const { return rhs < *this; }

  bool operator>=(const dict_iterator& rhs) const { return !(*this < rhs); }

  bool operator<=(const dict_iterator& rhs) const { return !(*this > rhs); }

  dict_iterator begin() const { return container_; }

  dict_iterator cbegin() const { return begin(); }
  ...
  dict_iterator cend() const { return end(); }
  ...
  operator*() {
    return const_cast<remove_const_t<ref_t>>(
        static_cast<const dict_iterator*>(this)->operator*());
  }
...
// using const_ref_iterator = ref_iterator<CT, typename
// decay_t<CT>::const_iterator>;
