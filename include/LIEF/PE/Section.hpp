...
class LIEF_API Section : public LIEF::Section {
 ...
 public:
  ...
  Section();
  Section(const std::vector<uint8_t>& data, const std::string& name = "",
  ...
  inline const std::vector<uint8_t>& padding() const { return padding_; }
  ...
  //! relocations.
  //!
  //! For modern PE binaries, this value is usually set to 0 as the relocations
  //! are managed by PE::Relocation.
  uint32_t pointerto_relocation() const;

  //! The file pointer to the beginning of line-number entries for the section.
  //! This is set to zero if there are no COFF line numbers. This value should
  //! be zero for an image because COFF debugging information is deprecated and
  //! modern debug information relies on the PDB files.
 ...
 private:
  inline span<uint8_t> writable_content() { return content_; }
