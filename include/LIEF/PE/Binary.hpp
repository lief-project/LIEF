...
class LIEF_API Binary : public LIEF::Binary {
 ...
 public:
  ...
  //! Verify the binary with the Signature object provided in the first
  //! parameter It can be used to verify a detached signature:
  //!
  //! \code{.cpp}
  ...
  //! \endcode
  Signature::VERIFICATION_FLAGS verify_signature(
      const Signature& sig, Signature::VERIFICATION_CHECKS checks =
                                Signature::VERIFICATION_CHECKS::DEFAULT) const;
  ...
  //! Try to predict the RVA of the function `function` in the import library
  //! `library`
  //!
  //! @warning
  ...
  uint32_t predict_function_rva(const std::string& library,
  ...
  //! Return the ResourcesManager (class to manage resources more easily than
  //! the tree one)
  ...
  ImportEntry* add_import_function(const std::string& library,
                                   const std::string& function);
  ...
  // @deprecated This function will be removed in a future version of LIEF
  void hook_function(const std::string& library, const std::string& function,
                     uint64_t address);
