...
class LIEF_API TLS : public Object {
 ...
 public:
  ...
  //! @note
  //! These addresses are not RVA. It is addresses for which there should be a
  ...
  //! This location is in an ordinary data section, so it can be given a
  //! symbolic name that is accessible to the program.
  ...
  //! by the addressof_raw_data field. The total template size should be the
  //! same as the total size of TLS data in the image file. The zero fill is the
  //! amount of data that comes after the initialized nonzero data.
  ...
  //! used to describe alignment of section in object files. The other 28 bits
