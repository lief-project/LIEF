...
enum class IMPHASH_MODE {
  ...
  VT = PEFILE,    /**< Same as IMPHASH_MODE::PEFILE since Virus Total is using
                     pefile */
};
...
//! by ordinal.
//!
//! The ``strict`` boolean parameter enables to throw an LIEF::not_found
//! exception if the ordinal can't be resolved. Otherwise it skips the entry.
//!
//! @param[in] import Import to resolve
//! @param[in] strict If set to ``true``, throw an exception if the import can't
//! be resolved
