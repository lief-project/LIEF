...
class BinaryStream {
 public:
  ...
  inline STREAM_TYPE type() const { return stype_; }
  ...
  result<std::string> peek_string(
      size_t maxsize = ~static_cast<size_t>(0)) const;
  result<std::string> peek_string_at(
  ...
  virtual inline ok_error_t peek_data(std::vector<uint8_t>& container,
    ...
    const bool read_ok =
        offset <= this->size() && (offset + size) <= this->size();
    if (!read_ok) {
  ...
  virtual inline ok_error_t read_data(std::vector<uint8_t>& container,
                                      uint64_t size) {
  ...
  inline bool should_swap() const { return endian_swap_; }

 protected:
  virtual result<const void*> read_at(uint64_t offset, uint64_t size) const = 0;
  inline virtual ok_error_t peek_in(void* dst, uint64_t offset,
                                    uint64_t size) const {
...
class ScopedStream {
 public:
  ...
  explicit ScopedStream(BinaryStream& stream, uint64_t pos)
      : pos_{stream.pos()}, stream_{stream} {
    stream_.setpos(pos);
  }

  inline ~ScopedStream() { stream_.setpos(pos_); }
...
template <typename T>
std::unique_ptr<T[]> BinaryStream::peek_conv_array(size_t offset,
                                                   size_t size) const {
