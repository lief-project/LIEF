  ...
  for (size_t i = 0; i < dyn_entries.size(); ++i) {
    ...
    switch (entry.tag()) {
      ...
      case DynamicEntry::TAG::RUNPATH:
      {
        ...
        e->value = entry.value();
        e->runpath =
            reinterpret_cast<DynamicEntryRunPath*>(&entry)->runpath().c_str();
      ...
      case DynamicEntry::TAG::PREINIT_ARRAY:
      {
        ...
        e->array =
            static_cast<uint64_t*>(malloc((array.size() + 1) * sizeof(uint64_t)));
...
void destroy_dynamic_entries(Elf_Binary_t* c_binary) {
  ...
  for (size_t idx = 0; dynamic_entries[idx] != nullptr; ++idx) {
    switch (DynamicEntry::TAG(dynamic_entries[idx]->tag)) {
      ...
      {
        free(
            reinterpret_cast<Elf_DynamicEntry_Auxiliary_t*>(dynamic_entries[idx])
        );
        break;
      }
      ...
      case DynamicEntry::TAG::PREINIT_ARRAY:
      {
        Elf_DynamicEntry_Array_t* entry_array =
            reinterpret_cast<Elf_DynamicEntry_Array_t*>(dynamic_entries[idx]);
      ...
      case DynamicEntry::TAG::FLAGS:
      {
        Elf_DynamicEntry_Flags_t* entry_flags =
            reinterpret_cast<Elf_DynamicEntry_Flags_t*>(dynamic_entries[idx]);
        free(entry_flags);
        break;
      }
      case DynamicEntry::TAG::FLAGS_1:
