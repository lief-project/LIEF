  ...
  for (size_t i = 0; i < dyn_entries.size(); ++i) {
    ...
    switch (entry.tag()) {
      case DYNAMIC_TAGS::DT_NEEDED: {
        ...
        e->value = entry.value();
        e->name =
            reinterpret_cast<DynamicEntryLibrary*>(&entry)->name().c_str();
      ...
      case DYNAMIC_TAGS::DT_SONAME: {
        ...
        e->value = entry.value();
        e->name =
            reinterpret_cast<DynamicSharedObject*>(&entry)->name().c_str();
      ...
      case DYNAMIC_TAGS::DT_RUNPATH: {
        ...
        e->value = entry.value();
        e->runpath =
            reinterpret_cast<DynamicEntryRunPath*>(&entry)->name().c_str();
      ...
      case DYNAMIC_TAGS::DT_PREINIT_ARRAY: {
        ...
        const std::vector<uint64_t>& array =
            reinterpret_cast<DynamicEntryArray*>(&entry)->array();
        e->array = static_cast<uint64_t*>(
            malloc((array.size() + 1) * sizeof(uint64_t)));
      ...
      case DYNAMIC_TAGS::DT_FLAGS: {
        ...
        e->tag = static_cast<enum LIEF_ELF_DYNAMIC_TAGS>(entry.tag());
        e->value = entry.value();
        const DynamicEntryFlags::flags_list_t& flags =
            reinterpret_cast<DynamicEntryFlags*>(&entry)->flags();
        e->flags = static_cast<enum LIEF_ELF_DYNAMIC_FLAGS*>(
            malloc((flags.size() + 1) * sizeof(enum LIEF_ELF_DYNAMIC_FLAGS)));
        e->flags_1 = nullptr;
      ...
      case DYNAMIC_TAGS::DT_FLAGS_1: {
        ...
        const DynamicEntryFlags::flags_list_t& flags =
            reinterpret_cast<DynamicEntryFlags*>(&entry)->flags();
        e->flags_1 = static_cast<enum LIEF_ELF_DYNAMIC_FLAGS_1*>(
            malloc((flags.size() + 1) * sizeof(enum LIEF_ELF_DYNAMIC_FLAGS_1)));
        e->flags = nullptr;
        ...
        e->flags_1[flags.size()] =
            static_cast<enum LIEF_ELF_DYNAMIC_FLAGS_1>(0);
        c_binary->dynamic_entries[i] = reinterpret_cast<Elf_DynamicEntry_t*>(e);
      ...
      default: {
        c_binary->dynamic_entries[i] = static_cast<Elf_DynamicEntry_t*>(
            malloc(sizeof(Elf_DynamicEntry_t)));
        c_binary->dynamic_entries[i]->tag =
            static_cast<enum LIEF_ELF_DYNAMIC_TAGS>(entry.tag());
...
void destroy_dynamic_entries(Elf_Binary_t* c_binary) {
  ...
  for (size_t idx = 0; dynamic_entries[idx] != nullptr; ++idx) {
    switch (static_cast<DYNAMIC_TAGS>(dynamic_entries[idx]->tag)) {
      case DYNAMIC_TAGS::DT_NEEDED: {
        free(reinterpret_cast<Elf_DynamicEntry_Library_t*>(
            dynamic_entries[idx]));
        break;
      }

      case DYNAMIC_TAGS::DT_SONAME: {
        free(reinterpret_cast<Elf_DynamicEntry_SharedObject_t*>(
            dynamic_entries[idx]));
        break;
      }
      ...
      case DYNAMIC_TAGS::DT_RUNPATH: {
        free(reinterpret_cast<Elf_DynamicEntry_RunPath_t*>(
            dynamic_entries[idx]));
        break;
      }
