...
class ELF_Header : public Mirror<LIEF::ELF::Header> {
  ...
  uint64_t program_headers_offset() const {
    return get().program_headers_offset();
  }
  uint64_t section_headers_offset() const {
    return get().section_headers_offset();
  }
  uint32_t processor_flag() const {
    return get().processor_flag();
  }
  uint32_t header_size() const {
    return get().header_size();
  }
  uint32_t program_header_size() const {
    return get().program_header_size();
  }
  uint32_t numberof_segments() const {
    return get().numberof_segments();
  }
  uint32_t section_header_size() const {
    return get().section_header_size();
  }
