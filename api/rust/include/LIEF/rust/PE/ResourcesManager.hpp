...
class PE_ResourcesManager_string_entry_t : private Mirror<LIEF::PE::ResourcesManager::string_entry_t> {
  public:
  using lief_t = LIEF::PE::ResourcesManager::string_entry_t;
  using Mirror::Mirror;

  auto string() const { return get().string_u8(); }
...
class PE_ResourcesManager : private Mirror<LIEF::PE::ResourcesManager> {
  ...
  class it_icons :
      public Iterator<PE_ResourceIcon, LIEF::PE::ResourcesManager::it_const_icons>
  {
    public:
  ...
  class it_version :
      public ContainerIterator<PE_ResourceVersion, std::vector<LIEF::PE::ResourceVersion>>
  {
    public:
  ...
  class it_accelerator :
      public Iterator<PE_ResourceAccelerator, LIEF::PE::ResourcesManager::it_const_accelerators>
  {
    public:
  ...
  auto html() const { return get().html(); }
  auto icons() const { return std::make_unique<it_icons>(get()); }
  auto version() const { return std::make_unique<it_version>(get()); }
  auto accelerator() const { return std::make_unique<it_accelerator>(get()); }
  auto string_table() const { return std::make_unique<it_string_table_entry>(get()); }
};
