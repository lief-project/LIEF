...
class PE_RuntimeFunctionAArch64 : public PE_ExceptionInfo {
  ...
  PE_RuntimeFunctionAArch64(const lief_t& obj) :
...
class PE_unwind_aarch64_PackedFunction : public PE_RuntimeFunctionAArch64 {
  ...
  auto frame_size() const {
    return impl().frame_size();
  }
  auto reg_I() const {
    return impl().reg_I();
  }
...
class PE_unwind_aarch64_UnpackedFunction : public PE_RuntimeFunctionAArch64 {
  ...
  class it_const_epilog_scopes
    : public Iterator<
          PE_unwind_aarch64_UnpackedFunction_epilog_scope_t,
  ...
  auto xdata_rva() const {
    return impl().xdata_rva();
  }
  auto version() const {
    return impl().version();
  }
  ...
  auto epilog_count() const {
    return impl().epilog_count();
  }
  auto epilog_offset() const {
    return impl().epilog_offset();
  }
  auto code_words() const {
    return impl().code_words();
  }
  auto exception_handler() const {
    return impl().exception_handler();
  }
  auto unwind_code() const {
    return make_span(impl().unwind_code());
  }
