...
class asm_powerpc_operands_Memory_offset_t {
  public:
  uint64_t value = 0;
  uint32_t enum_type = 0;
};

class asm_powerpc_operands_Memory : public asm_powerpc_Operand {
  public:
  using lief_t = LIEF::assembly::powerpc::operands::Memory;

  uint64_t base() const {
    return to_int(impl().base());
  }

  asm_powerpc_operands_Memory_offset_t offset() const {
  ...
  static auto classof(const asm_powerpc_Operand& inst) {
