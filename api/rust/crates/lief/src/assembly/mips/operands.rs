...
impl FromFFI<ffi::asm_mips_Operand> for Operands {
    fn from_ffi(ptr: cxx::UniquePtr<ffi::asm_mips_Operand>) -> Self {
        unsafe {
            let op_ref = ptr.as_ref().unwrap();
            if ffi::asm_mips_operands_Memory::classof(op_ref) {
                let raw = {
                    type From = cxx::UniquePtr<ffi::asm_mips_Operand>;
                    type To = cxx::UniquePtr<ffi::asm_mips_operands_Memory>;
                    std::mem::transmute::<From, To>(ptr)
                };
                return Operands::Mem(Memory::from_ffi(raw));
            } else if ffi::asm_mips_operands_Register::classof(op_ref) {
                let raw = {
                    type From = cxx::UniquePtr<ffi::asm_mips_Operand>;
                    type To = cxx::UniquePtr<ffi::asm_mips_operands_Register>;
                    std::mem::transmute::<From, To>(ptr)
                };
                return Operands::Reg(Register::from_ffi(raw));
            } else if ffi::asm_mips_operands_Immediate::classof(op_ref) {
                let raw = {
                    type From = cxx::UniquePtr<ffi::asm_mips_Operand>;
                    type To = cxx::UniquePtr<ffi::asm_mips_operands_Immediate>;
                    std::mem::transmute::<From, To>(ptr)
                };
                return Operands::Imm(Immediate::from_ffi(raw));
            } else if ffi::asm_mips_operands_PCRelative::classof(op_ref) {
                let raw = {
                    type From = cxx::UniquePtr<ffi::asm_mips_Operand>;
                    type To = cxx::UniquePtr<ffi::asm_mips_operands_PCRelative>;
                    std::mem::transmute::<From, To>(ptr)
                };
...
pub struct Unknown {
    ptr: cxx::UniquePtr<ffi::asm_mips_Operand>,
}

impl FromFFI<ffi::asm_mips_Operand> for Unknown {
    fn from_ffi(ptr: cxx::UniquePtr<ffi::asm_mips_Operand>) -> Self {
        Self { ptr }
    }
}
