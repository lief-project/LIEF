...
void create<Binary>(py::module& m) {
  ...
  bin.def_property_readonly(
         "header", static_cast<no_const_getter<Header&>>(&Binary::header),
         "Return the OAT " RST_CLASS_REF(lief.OAT.Header) "",
         py::return_value_policy::reference)

      .def_property_readonly(
          "dex_files",
          static_cast<no_const_getter<Binary::it_dex_files>>(
              &Binary::dex_files),
          "Return an iterator over " RST_CLASS_REF(lief.DEX.File) "")

      .def_property_readonly(
          "oat_dex_files",
          static_cast<no_const_getter<Binary::it_oat_dex_files>>(
              &Binary::oat_dex_files),
      ...
      .def_property_readonly("has_class", &Binary::has_class,
                             "Check if the class if the given name is present "
                             "in the current OAT binary")

      .def("get_class",
           static_cast<no_const_func<Class*, const std::string&>>(
               &Binary::get_class),
           "Return the " RST_CLASS_REF(lief.OAT.Class) " from its name",
           "class_name"_a, py::return_value_policy::reference)

      .def("get_class",
           static_cast<no_const_func<Class*, size_t>>(&Binary::get_class),
           "Return the " RST_CLASS_REF(lief.OAT.Class) " from its **index**",
           "class_index"_a, py::return_value_policy::reference)

      .def_property_readonly("dex2dex_json_info", &Binary::dex2dex_json_info)

      .def("__eq__", &Binary::operator==)
      .def("__ne__", &Binary::operator!=)
      .def("__hash__", [](const Binary& bin) { return Hash::hash(bin); })
