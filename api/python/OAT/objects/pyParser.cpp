...
void create<Parser>(py::module& m) {
  ...
  m.def("parse",
        static_cast<std::unique_ptr<Binary> (*)(const std::string&)>(
            &Parser::parse),
        "Parse the given OAT file and return a " RST_CLASS_REF(
            lief.OAT.Binary) " object",
        "oat_file"_a, py::return_value_policy::take_ownership);

  m.def("parse",
        static_cast<std::unique_ptr<Binary> (*)(
            const std::string&, const std::string&)>(&Parser::parse),
        "Parse the given OAT with its VDEX file and return a " RST_CLASS_REF(
            lief.OAT.Binary) " object",
        "oat_file"_a, "vdex_file"_a, py::return_value_policy::take_ownership);
