...
void create<DynamicEntryRunPath>(py::module& m) {
  py::class_<DynamicEntryRunPath, DynamicEntry>(m, "DynamicEntryRunPath",
      ...
      .def(py::init<const std::string&>(), "Constructor from a (run)path",
           "path"_a = "")

      .def(py::init<const std::vector<std::string>&>(),
           "Constructor from a list of paths", "paths"_a)
      ...
      .def_property(
          ...
          static_cast<setter_t<const std::string&>>(
              &DynamicEntryRunPath::runpath),
          "Runpath raw value")

      .def_property("paths",
                    static_cast<getter_t<std::vector<std::string>>>(
                        &DynamicEntryRunPath::paths),
                    static_cast<setter_t<const std::vector<std::string>&>>(
                        &DynamicEntryRunPath::paths),
                    "Paths as a list")

      .def("insert", &DynamicEntryRunPath::insert,
           "Insert a ``path`` at the given ``position``", "position"_a,
           "path"_a, py::return_value_policy::reference)

      .def("append", &DynamicEntryRunPath::append, "Append the given ``path`` ",
           "path"_a, py::return_value_policy::reference)

      .def("remove", &DynamicEntryRunPath::remove, "Remove the given ``path`` ",
           "path"_a, py::return_value_policy::reference)
      ...
      .def("__hash__",
           [](const DynamicEntryRunPath& entry) { return Hash::hash(entry); })

      .def("__str__", [](const DynamicEntryRunPath& entry) {
