...
void create<Symbol>(py::module& m) {
  py::class_<Symbol, LIEF::Symbol>(m, "Symbol",
    ...
    "Class which represents an ELF symbol"
      ...
      .def_property("value",  // Even though it is already defined in the base
                              // class (Abstract/Symbol) We keep the definition
                              // to provide a dedicated documentation
      ...
      .def_property("size", static_cast<getter_t<uint64_t>>(&Symbol::size),
                    ...
                    "object's size is the number of "
                    "bytes contained in the object. This member holds `0` if "
                    "the symbol has no size or "
                    "an unknown size.")

      .def_property("shndx", static_cast<getter_t<uint16_t>>(&Symbol::shndx),
                    static_cast<setter_t<uint16_t>>(&Symbol::shndx),
                    "Section index associated with the symbol")

      .def_property_readonly("has_version", &Symbol::has_version,
                             "Check if this symbols has a " RST_CLASS_REF(
                                 lief.ELF.SymbolVersion) "")

      .def_property_readonly("symbol_version",
                             static_cast<SymbolVersion* (Symbol::*)(void)>(
      ...
      .def_property_readonly("is_static", &Symbol::is_static,
                             "True if the symbol is a static one (i.e. from "
                             "the ``.symtab`` section")

      .def_property_readonly("is_function", &Symbol::is_function,
                             "True if the symbol is a function")

      .def_property_readonly("is_variable", &Symbol::is_variable,
                             "True if the symbol is a variable")

      .def_property("exported", &Symbol::is_exported, &Symbol::set_exported,
                    "Whether the symbol is **exported**")

      .def_property("imported", &Symbol::is_imported, &Symbol::set_imported,
