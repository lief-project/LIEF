...
void create<CorePrStatus>(py::module& m) {
  ...
  cls.def_property("siginfo",
                   static_cast<getter_t<const CorePrStatus::siginfo_t&>>(
                       &CorePrStatus::siginfo),
                   static_cast<setter_t<const CorePrStatus::siginfo_t&>>(
                       &CorePrStatus::siginfo),
      ...
      .def_property("pid", static_cast<getter_t<int32_t>>(&CorePrStatus::pid),
                    static_cast<setter_t<int32_t>>(&CorePrStatus::pid),
                    "Process ID")

      .def_property("ppid", static_cast<getter_t<int32_t>>(&CorePrStatus::ppid),
                    static_cast<setter_t<int32_t>>(&CorePrStatus::ppid),
                    "Process parent ID")

      .def_property("pgrp", static_cast<getter_t<int32_t>>(&CorePrStatus::pgrp),
                    static_cast<setter_t<int32_t>>(&CorePrStatus::pgrp),
                    "Process group ID")

      .def_property("sid", static_cast<getter_t<int32_t>>(&CorePrStatus::sid),
      ...
      .def_property(
          ...
          "Cumulative user time (" RST_CLASS_REF(
              lief.ELF.CorePrStatus.timeval) ")")
      ...
      .def_property(
          ...
          "Cumulative system time (" RST_CLASS_REF(
              lief.ELF.CorePrStatus.timeval) ")")

      .def_property("register_context",
                    static_cast<getter_t<const CorePrStatus::reg_context_t&>>(
                        &CorePrStatus::reg_context),
                    static_cast<setter_t<const CorePrStatus::reg_context_t&>>(
                        &CorePrStatus::reg_context),
                    "Current registers state as a dictionary where the keys "
                    "are " RST_CLASS_REF(
                        lief.ELF.CorePrStatus
                            .REGISTERS) " and the values the register's value")

      .def(
          "get",
          [](const CorePrStatus& status,
             CorePrStatus::REGISTERS reg) -> py::object {
            bool error;
          ...
          "Return the register value", "register"_a)

      .def("set", &CorePrStatus::set, "Set register value", "register"_a,
           "value"_a)

      .def("has", &CorePrStatus::has,
           "Check if a value is associated with the given register",
           "register"_a)

      .def("__getitem__", &CorePrStatus::operator[], "",
      ...
      .def("__contains__", &CorePrStatus::has, "")

      .def("__eq__", &CorePrStatus::operator==)
      .def("__ne__", &CorePrStatus::operator!=)
      .def("__hash__",
           [](const CorePrStatus& note) { return Hash::hash(note); })

      .def("__str__", [](const CorePrStatus& note) {
