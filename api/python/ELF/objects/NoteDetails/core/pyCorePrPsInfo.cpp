...
void create<CorePrPsInfo>(py::module& m) {
  py::class_<CorePrPsInfo, NoteDetails>(m, "CorePrPsInfo")
      ...
      .def_property("uid", static_cast<getter_t<uint32_t>>(&CorePrPsInfo::uid),
                    static_cast<setter_t<uint32_t>>(&CorePrPsInfo::uid),
                    "Process User ID")

      .def_property("gid", static_cast<getter_t<uint32_t>>(&CorePrPsInfo::gid),
                    static_cast<setter_t<uint32_t>>(&CorePrPsInfo::gid),
                    "Process Group ID")

      .def_property("pid", static_cast<getter_t<int32_t>>(&CorePrPsInfo::pid),
                    static_cast<setter_t<int32_t>>(&CorePrPsInfo::pid),
                    "Process ID")

      .def_property("ppid", static_cast<getter_t<int32_t>>(&CorePrPsInfo::ppid),
                    static_cast<setter_t<int32_t>>(&CorePrPsInfo::ppid),
                    "Process parent ID")

      .def_property("pgrp", static_cast<getter_t<int32_t>>(&CorePrPsInfo::pgrp),
                    static_cast<setter_t<int32_t>>(&CorePrPsInfo::pgrp),
                    "Process session group ID")

      .def_property("sid", static_cast<getter_t<int32_t>>(&CorePrPsInfo::sid),
      ...
      .def("__hash__",
           [](const CorePrPsInfo& note) { return Hash::hash(note); })

      .def("__str__", [](const CorePrPsInfo& note) {
