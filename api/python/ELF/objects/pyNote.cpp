...
void create<Note>(py::module& m) {
  py::class_<Note, LIEF::Object>(m, "Note",
      ...
      .def(py::init<const std::string&, NOTE_TYPES,
                    const std::vector<uint8_t>&>(),
           "Constructor from a ``name``, ``type`` and ``description``",
           "name"_a, "type"_a, "description"_a)

      .def_property_readonly(
          "details", static_cast<NoteDetails& (Note::*)(void)>(&Note::details),
          "Parse the given note description and return a " RST_CLASS_REF(
              lief.ELF.NoteDetails) " object",
      ...
      .def_property(
          "type", static_cast<getter_t<NOTE_TYPES>>(&Note::type),
          static_cast<setter_t<NOTE_TYPES>>(&Note::type),
          "Return the type of the note. It can be one of the " RST_CLASS_REF(
              lief.ELF.NOTE_TYPES) " values")

      .def_property(
          "type_core", static_cast<getter_t<NOTE_TYPES_CORE>>(&Note::type_core),
          static_cast<setter_t<NOTE_TYPES_CORE>>(&Note::type_core),
          "Return the type of the note for ELF Core (ET_CORE). It Can be one "
          "of the " RST_CLASS_REF(lief.ELF.NOTE_TYPES_CORE) " values")
      ...
      .def_property_readonly("is_core", &Note::is_core,
                             "True if the note is associated with a coredump")

      .def_property_readonly("is_android", &Note::is_android,
                             R"delim(
      ...
      .def_property_readonly("size", &Note::size, "Size of the **raw** note")

      .def("__eq__", &Note::operator==)
      .def("__ne__", &Note::operator!=)
      .def("__hash__", [](const Note& note) { return Hash::hash(note); })

      .def("__str__", [](const Note& note) {
