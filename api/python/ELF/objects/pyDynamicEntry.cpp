...
void create<DynamicEntry>(py::module& m) {
  ...
  py::class_<DynamicEntry, LIEF::Object>(m, "DynamicEntry",
      ...
      .def(py::init<DYNAMIC_TAGS, uint64_t>(),
           "Constructor from a " RST_CLASS_REF(
               lief.ELF.DYNAMIC_TAGS) " and value",
           "tag"_a, "value"_a)

      .def_property(
          "tag", static_cast<getter_t<DYNAMIC_TAGS>>(&DynamicEntry::tag),
          static_cast<setter_t<DYNAMIC_TAGS>>(&DynamicEntry::tag),
          "Return the entry's " RST_CLASS_REF(
              lief.ELF.DYNAMIC_TAGS) " which represent the entry type")
      ...
      .def("__hash__",
           [](const DynamicEntry& entry) { return Hash::hash(entry); })

      .def("__str__", [](const DynamicEntry& dynamicEntry) {
