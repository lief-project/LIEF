...
void create<Relocation>(py::module& m) {
  ...
  py::class_<Relocation, LIEF::Relocation>(m, "Relocation",
      ...
      .def(py::init<uint64_t, uint32_t, int64_t, bool>(), "address"_a,
           "type"_a = 0, "addend"_a = 0, "is_rela"_a = false)
      ...
      .def_property("info", static_cast<getter_t<uint32_t>>(&Relocation::info),
      ...
      .def_property("type", static_cast<getter_t<uint32_t>>(&Relocation::type),
      ...
      .def_property_readonly(
          "has_symbol", &Relocation::has_symbol,
          "``True`` if a " RST_CLASS_REF(
              lief.ELF.Symbol) " is associated with the relocation")
      ...
      .def_property_readonly("has_section", &Relocation::has_section,
                             R"delim(
      ...
      .def_property_readonly("is_rela",
                             ...
                             "``True`` if the relocation **uses** the "
                             ":attr:`~lief.ELF.Relocation.addend` proprety")

      .def_property_readonly("is_rel",
                             static_cast<getter_t<bool>>(&Relocation::is_rel),
                             "``True`` if the relocation **doesn't use** the "
                             ":attr:`~lief.ELF.Relocation.addend` proprety")

      .def("__eq__", &Relocation::operator==)
      .def("__ne__", &Relocation::operator!=)
      .def("__hash__",
           [](const Relocation& relocation) { return Hash::hash(relocation); })
