...
void create<DelayImportEntry>(py::module& m) {
  py::class_<DelayImportEntry, LIEF::Symbol>(m, "DelayImportEntry",
      ...
      .def_property(
          "data", static_cast<getter_t<uint64_t>>(&DelayImportEntry::data),
          static_cast<setter_t<uint64_t>>(&DelayImportEntry::data), "Raw value")

      .def_property_readonly("is_ordinal", &DelayImportEntry::is_ordinal,
                             "``True`` if it is an import by ordinal")

      .def_property_readonly("ordinal", &DelayImportEntry::ordinal,
                             "Ordinal value (if any). See: "
                             ":attr:`~lief.PE.DelayImportEntry.is_ordinal`")

      .def_property_readonly("hint", &DelayImportEntry::hint,
                             "Index into the :attr:`lief.PE.Export.entries` "
                             "that is used to speed-up the symbol resolution")

      .def_property_readonly("iat_value", &DelayImportEntry::iat_value,
                             R"delim(
