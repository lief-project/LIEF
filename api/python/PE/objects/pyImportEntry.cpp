...
void create<ImportEntry>(py::module& m) {
  py::class_<ImportEntry, LIEF::Symbol>(m, "ImportEntry",
      ...
      .def(py::init<uint64_t, const std::string&>(),
           "Constructor from a :attr:`~lief.PE.ImportEntry.data` and an "
           "optionally :attr:`~lief.PE.ImportEntry.name`",
           "data"_a, "name"_a = "")
      ...
      .def_property("data", static_cast<getter_t<uint64_t>>(&ImportEntry::data),
                    static_cast<setter_t<uint64_t>>(&ImportEntry::data),
                    "Raw value")

      .def_property_readonly("is_ordinal", &ImportEntry::is_ordinal,
                             "``True`` if it is an import by ordinal")

      .def_property_readonly("ordinal", &ImportEntry::ordinal,
                             "Ordinal value (if any). See: "
                             ":attr:`~lief.PE.ImportEntry.is_ordinal`")

      .def_property_readonly("hint", &ImportEntry::hint,
                             "Index into the :attr:`lief.PE.Export.entries` "
                             "that is used to speed-up the symbol resolution")

      .def_property_readonly("iat_value", &ImportEntry::iat_value,
                             "Value of the current entry in the Import Address "
                             "Table. It should match the lookup table value.")

      .def_property_readonly(
          "iat_address", &ImportEntry::iat_address,
