...
void create<LoadConfiguration>(py::module& m) {
  py::class_<LoadConfiguration, LIEF::Object>(m, "LoadConfiguration",
    ...
    It's the base class for any future versions of the structure
      ...
      .def_property_readonly("version", &LoadConfiguration::version,
                             "(SDK) Version of the structure. (" RST_CLASS_REF(
                                 lief.PE.WIN_VERSION) ")")
      ...
      .def_property_readonly(
          "size", static_cast<getter_t<uint32_t>>(&LoadConfiguration::size),
          "Size of the structure which is an alias for " RST_ATTR_REF(
      ...
      .def_property("global_flags_clear",
                    static_cast<getter_t<uint32_t>>(
                        &LoadConfiguration::global_flags_clear),
                    static_cast<setter_t<uint32_t>>(
                        &LoadConfiguration::global_flags_clear),
                    "The global loader flags to clear for this process as the "
                    "loader start the process.")
      ...
      .def_property(
          ...
          "The global loader flags to set for this process as the loader "
          "starts the process.")
      ...
      .def_property("critical_section_default_timeout",
                    ...
                    static_cast<setter_t<uint32_t>>(
                        &LoadConfiguration::critical_section_default_timeout),
                    "The default timeout value to use for is process’s "
                    "critical sections that are abandoned.")
      ...
      .def_property("decommit_free_block_threshold",
                    ...
                    static_cast<setter_t<uint64_t>>(
                        &LoadConfiguration::decommit_free_block_threshold),
                    "Memory that must be freed before it is returned to the "
                    "system, in bytes.")
      ...
      .def_property(
          "lock_prefix_table",
          static_cast<getter_t<uint64_t>>(
              &LoadConfiguration::lock_prefix_table),
          static_cast<setter_t<uint64_t>>(
              &LoadConfiguration::lock_prefix_table),
      ...
      .def_property(
          ...
          static_cast<getter_t<uint64_t>>(
              &LoadConfiguration::process_affinity_mask),
          static_cast<setter_t<uint64_t>>(
              &LoadConfiguration::process_affinity_mask),
          "Setting this field to a non-zero value is equivalent to calling "
          "``SetProcessAffinityMask`` with this value during process startup "
          "(.exe only)")

      .def_property("process_heap_flags",
                    static_cast<getter_t<uint32_t>>(
                        &LoadConfiguration::process_heap_flags),
                    static_cast<setter_t<uint32_t>>(
                        &LoadConfiguration::process_heap_flags),
                    R"delim(
      ...
      .def_property("dependent_load_flags",
                    static_cast<getter_t<uint16_t>>(
                        &LoadConfiguration::dependent_load_flags),
                    static_cast<setter_t<uint16_t>>(
                        &LoadConfiguration::dependent_load_flags),
                    "On recent the version of the structure, Microsoft renamed "
                    "reserved1 to DependentLoadFlags. "
                    "This is an alias for " RST_ATTR_REF(
      ...
      .def("__hash__",
           [](const LoadConfiguration& config) { return Hash::hash(config); })
