...
void create<RichEntry>(py::module& m) {
  py::class_<RichEntry, LIEF::Object>(m, "RichEntry",
      ...
      .def_property("id", static_cast<getter_t<uint16_t>>(&RichEntry::id),
      ...
      .def_property("count", static_cast<getter_t<uint32_t>>(&RichEntry::count),
                    static_cast<setter_t<uint32_t>>(&RichEntry::count),
                    "*Occurrence* count")

      .def("__eq__", &RichEntry::operator==)
      .def("__ne__", &RichEntry::operator!=)
      .def("__hash__", [](const RichEntry& entry) { return Hash::hash(entry); })
