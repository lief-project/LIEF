...
void create<DosHeader>(py::module& m) {
  py::class_<DosHeader, LIEF::Object>(m, "DosHeader",
      ...
      .def(py::init<>())
      .def_property("magic", static_cast<getter_t>(&DosHeader::magic),
      ...
      .def_property("initial_sp", static_cast<getter_t>(&DosHeader::initial_sp),
                    static_cast<setter_t>(&DosHeader::initial_sp))

      .def_property("checksum", static_cast<getter_t>(&DosHeader::checksum),
                    static_cast<setter_t>(&DosHeader::checksum))

      .def_property("initial_ip", static_cast<getter_t>(&DosHeader::initial_ip),
      ...
      .def_property("oem_id", static_cast<getter_t>(&DosHeader::oem_id),
                    static_cast<setter_t>(&DosHeader::oem_id))

      .def_property("oem_info", static_cast<getter_t>(&DosHeader::oem_info),
                    static_cast<setter_t>(&DosHeader::oem_info))

      .def_property("addressof_new_exeheader",
                    static_cast<getter_abs_t<uint32_t>>(
                        &DosHeader::addressof_new_exeheader),
                    static_cast<setter_abs_t<uint32_t>>(
                        &DosHeader::addressof_new_exeheader))

      .def("__eq__", &DosHeader::operator==)
      .def("__ne__", &DosHeader::operator!=)
      .def("__hash__",
           [](const DosHeader& dos_header) { return Hash::hash(dos_header); })
