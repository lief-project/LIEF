...
void create<ResourceIcon>(py::module& m) {
  py::class_<ResourceIcon, LIEF::Object>(m, "ResourceIcon")
      .def_property("id", static_cast<getter_t<uint32_t>>(&ResourceIcon::id),
      ...
      .def_property("lang",
                    ...
                    "Language (" RST_CLASS_REF(
                        lief.PE.RESOURCE_LANGS) ") associated with the icon")

      .def_property(
          "sublang",
          static_cast<getter_t<RESOURCE_SUBLANGS>>(&ResourceIcon::sublang),
          static_cast<setter_t<RESOURCE_SUBLANGS>>(&ResourceIcon::sublang),
          "Sub language (" RST_CLASS_REF(
              lief.PE.RESOURCE_SUBLANGS) ") associated with the icon")
      ...
      .def("save", &ResourceIcon::save, "Save the icon to the given filepath",
           "filepath"_a)

      .def("__eq__", &ResourceIcon::operator==)
      .def("__ne__", &ResourceIcon::operator!=)
      .def("__hash__",
           [](const ResourceIcon& icon) { return Hash::hash(icon); })

      .def("__str__", [](const ResourceIcon& icon) {
