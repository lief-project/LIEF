...
void create<ResourceAccelerator>(py::module& m) {
  py::class_<ResourceAccelerator, LIEF::Object>(m, "ResourceAccelerator")
      ...
      .def_property_readonly(
          "flags", static_cast<getter_t<int16_t>>(&ResourceAccelerator::flags),
          "Describe the keyboard accelerator characteristics.")

      .def_property_readonly(
          "ansi", static_cast<getter_t<int16_t>>(&ResourceAccelerator::ansi),
          "An ANSI character value or a virtual-key code that identifies the "
          "accelerator key.")

      .def_property_readonly(
          "id", static_cast<getter_t<uint16_t>>(&ResourceAccelerator::id),
          "An identifier for the keyboard accelerator.")

      .def_property_readonly(
          "padding",
          static_cast<getter_t<int16_t>>(&ResourceAccelerator::padding),
          "The number of bytes inserted to ensure that the structure is "
          "aligned on a DWORD boundary.")

      .def("__eq__", &ResourceAccelerator::operator==)
      .def("__ne__", &ResourceAccelerator::operator!=)
      .def("__hash__",
           [](const ResourceAccelerator& acc) { return Hash::hash(acc); })

      .def("__str__", [](const ResourceAccelerator& acc) {
