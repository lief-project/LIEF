...
void create<Import>(py::module& m) {
  ...
  imp.def(py::init<>(), "Default constructor")

      .def(py::init<const std::string&>(), "Constructor from a library name",
           "library_name"_a)

      .def_property_readonly("forwarder_chain", &Import::forwarder_chain,
                             "The index of the first forwarder reference")

      .def_property_readonly("timedatestamp", &Import::timedatestamp,
                             R"delim(
      ...
      .def_property_readonly(
          ...
          "Iterator over the " RST_CLASS_REF(
              lief.PE.ImportEntry) " (functions)",
          py::return_value_policy::reference)

      .def_property(
          "name",
          [](const Import& obj) { return safe_string_converter(obj.name()); },
      ...
      .def("get_function_rva_from_iat", &Import::get_function_rva_from_iat,
           "Return the relative virtual address of the given function within "
           "the *Import Address Table*",
           "function_name"_a)

      .def("add_entry",
           static_cast<ImportEntry& (Import::*)(const ImportEntry&)>(
               &Import::add_entry),
           "Add an " RST_CLASS_REF(
               lief.PE.ImportEntry) " (function) to the current import",
           "entry"_a, py::return_value_policy::reference)

      .def("add_entry",
           static_cast<ImportEntry& (Import::*)(const std::string&)>(
               &Import::add_entry),
           "Add an " RST_CLASS_REF(
               lief.PE.ImportEntry) " (function) to the current import",
           "function_name"_a, py::return_value_policy::reference)

      .def("get_entry",
           static_cast<no_const_func<ImportEntry*, const std::string&>>(
               &Import::get_entry),
           "Return the " RST_CLASS_REF(
               lief.PE.ImportEntry) " with the given name or None if not found",
           "function_name"_a, py::return_value_policy::reference)

      .def("__eq__", &Import::operator==)
      .def("__ne__", &Import::operator!=)
      .def("__hash__", [](const Import& import) { return Hash::hash(import); })
