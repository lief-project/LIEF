...
def print_libraries(binary):
    ...
    print("== Libraries ==")
    print(
        f_title.format("Name", "Timestamp", "Current Version", "Compatibility Version")
    )
...
def print_segments(binary):
    ...
    f_value = (
        "|{:<20}|0x{:<13x} |0x{:<13x} |0x{:<13x} |0x{:<13x} |0x{:<13x} |0x{:<13x} |{}"
    )
    print("== Segments ==")
    print(
        f_title.format(
    ...
    for section in binary.sections:
        ...
        if len(section.relocations) > 0:
            for idx, reloc in enumerate(section.relocations):
                ...
                print(
                    "    [Reloc #{:d} {section}] {name:<10} 0x{address:<6x} {type:<20} {size:d} {pcrel} {scat}".format(
                        idx,
                        section=secname,
                        name=name,
...
def print_symbols(binary):
    ...
    f_value = (
        "|{:<" + str(maxsize) + "} |0x{:<3x} |0x{:<16x} |0x{:<13x} |0x{:<13x} | {:s}"
    )
    print("== Symbols ==")
    print(
        f_title.format(
...
def print_dynamic_symbol_command(binary):
    ...
    print(
        format_dec.format(
            "Number of external symbols", dyscmd.nb_external_define_symbols
        )
    )
    ...
    print(
        format_dec.format("Number of entries in module table", dyscmd.nb_module_table)
    )

    print(
        format_hex.format(
            "External reference table offset", dyscmd.external_reference_symbol_offset
        )
    )
    print(
        format_dec.format(
            "Number of external reference", dyscmd.nb_external_reference_symbols
        )
    )
    ...
    print(
        format_hex.format(
            "External relocation offset", dyscmd.external_relocation_offset
        )
    )
    print(
        format_dec.format(
            "Number of external relocations", dyscmd.nb_external_relocations
        )
    )
...
def main():
    ...
    parser.add_argument(
        "--uuid", action="store_true", dest="show_uuid", help="Display the UUID command"
    )

    parser.add_argument(
        "--main", action="store_true", dest="show_main", help="Display the Main command"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="show_dynamic_symbol_command",
        help="Display the 'Symbol Command' command",
    )
    ...
    for binary in binaries:
        ...
        if (
            args.show_dynamic_symbol_command or args.show_all
        ) and binary.has_dynamic_symbol_command:
        ...
        if (
            args.show_segment_split_info or args.show_all
        ) and binary.has_segment_split_info:
