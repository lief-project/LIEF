...
def print_header(binary):
    ...
    print(
        format_dec.format(
            "Header size in paragraphs:", dos_header.header_size_in_paragraphs
        )
    )
    print(
        format_dec.format(
            "Minimum extra paragraphs:", dos_header.minimum_extra_paragraphs
        )
    )
    print(
        format_dec.format(
            "Maximum extra paragraphs", dos_header.maximum_extra_paragraphs
        )
    )
    ...
    print(
        format_hex.format(
            "Address of relocation table:", dos_header.addressof_relocation_table
        )
    )
    ...
    print(
        format_dec.format("Major linker version:", optional_header.major_linker_version)
    )
    print(
        format_dec.format("Minor linker version:", optional_header.minor_linker_version)
    )
    print(format_dec.format("Size of code:", optional_header.sizeof_code))
    print(
        format_dec.format(
            "Size of initialized data:", optional_header.sizeof_initialized_data
        )
    )
    print(
        format_dec.format(
            "Size of uninitialized data:", optional_header.sizeof_uninitialized_data
        )
    )
    ...
    print(
        format_dec.format("Major image version:", optional_header.major_image_version)
    )
    print(
        format_dec.format("Minor image version:", optional_header.minor_image_version)
    )
    print(
        format_dec.format(
            "Major subsystem version:", optional_header.major_subsystem_version
        )
    )
    print(
        format_dec.format(
            "Minor subsystem version:", optional_header.minor_subsystem_version
        )
    )
    print(
        format_dec.format("WIN32 version value:", optional_header.win32_version_value)
    )
    ...
    print(
        format_hex.format(
            "Size of stack reserve:", optional_header.sizeof_stack_reserve
        )
    )
    print(
        format_hex.format("Size of stack commit:", optional_header.sizeof_stack_commit)
    )
    print(
        format_hex.format("Size of heap reserve:", optional_header.sizeof_heap_reserve)
    )
...
def print_sections(binary):
    ...
    f_value = (
        "|{:<10} | 0x{:<14x} | 0x{:<14x} | 0x{:<16x} | 0x{:<14x} | {:<9.2f} | {:<9}"
    )
    print(
        f_title.format(
            "Name",
...
def print_tls(binary):
    ...
    print(
        "{:<33} 0x{:<10x} 0x{:<10x}".format(
            "Address of raw data:", tls.addressof_raw_data[0], tls.addressof_raw_data[1]
        )
    )
...
def print_export(binary):
    ...
    print(
        f_value.format(
            exports.name,
            exports.export_flags,
            exports.timestamp,
            exports.major_version,
            exports.minor_version,
...
def print_load_configuration(binary):
    ...
    print(
        format_dec.format(
            "Critical section default timeout:", config.critical_section_default_timeout
        )
    )
    print(
        format_hex.format(
            "Decommit free block threshold:", config.decommit_free_block_threshold
        )
    )
    print(
        format_hex.format(
            "Decommit total free threshold:", config.decommit_total_free_threshold
        )
    )
    ...
    print(
        format_hex.format("Virtual memory threshold:", config.virtual_memory_threshold)
    )
    ...
    if isinstance(config, lief.PE.LoadConfigurationV1):
        ...
        print(
            format_hex.format(
                "GCF check function pointer:", config.guard_cf_check_function_pointer
            )
        )
        print(
            format_hex.format(
                "GCF dispatch function pointer:",
    ...
    if isinstance(config, lief.PE.LoadConfigurationV2):
        ...
        print(
            format_hex.format(
                " " * 3 + "Catalog offset:", code_integrity.catalog_offset
            )
        )
    ...
    if isinstance(config, lief.PE.LoadConfigurationV3):
        ...
        print(
            format_hex.format(
                "Guard long jump target table:", config.guard_long_jump_target_table
            )
        )
        print(
            format_hex.format(
                "Guard long jump target count:", config.guard_long_jump_target_count
            )
        )
    ...
    if isinstance(config, lief.PE.LoadConfigurationV4):
        print(
            format_hex.format(
                "Dynamic value relocation table:", config.dynamic_value_reloc_table
            )
        )
        print(
            format_hex.format(
                "Hybrid metadata pointer:", config.hybrid_metadata_pointer
            )
        )

    if isinstance(config, lief.PE.LoadConfigurationV5):
        print(
            format_hex.format("GRF failure routine:", config.guard_rf_failure_routine)
        )
        print(
            format_hex.format(
                "GRF failure routine function pointer:",
                config.guard_rf_failure_routine_function_pointer,
            )
        )
        print(
            format_hex.format(
                "Dynamic value reloctable offset:",
                config.dynamic_value_reloctable_offset,
            )
        )
        print(
            format_hex.format(
                "Dynamic value reloctable section:",
    ...
    if isinstance(config, lief.PE.LoadConfigurationV6):
        print(
            format_hex.format(
                "GRF verify stackpointer function pointer:",
...
def main():
    ...
    parser.add_argument(
        ...
        "--data-directories",
        action="store_true",
        dest="show_data_directories",
        help="Display data directories",
    )

    parser.add_argument(
        "--dbg", action="store_true", dest="show_debug", help="Display debug directory"
    )
