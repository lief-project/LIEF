...
def print_crt(indent: int, crt: lief.PE.x509):
    ...
    if len(crt.key_usage) > 0:
        print(
            "{}  Key usage          : {}".format(
                " " * indent, " - ".join(str(e).split(".")[-1] for e in crt.key_usage)
            )
        )
    if len(crt.ext_key_usage) > 0:
        print(
            "{}  Ext key usage      : {}".format(
                " " * indent,
                " - ".join(lief.PE.oid_to_string(e) for e in crt.ext_key_usage),
            )
        )
...
def print_pkcs_counter_sig(indent: int, auth: lief.PE.PKCS9CounterSignature):
    ...
    print(
        "{}   Serial Number       : {}".format(" " * indent, signer.serial_number.hex())
    )
...
def print_all(sig: lief.PE.Signature, indent: int = 2):
    ...
    print(
        "{}  Content Type    : {!s} ({})".format(
            " " * indent, ci.content_type, lief.PE.oid_to_string(ci.content_type)
        )
    )
    ...
    for signer in sig.signers:
        ...
        print("{}  Issuer              : {}".format(" " * indent, signer.issuer))
        print(
            "{}  Digest Algorithm    : {}".format(" " * indent, signer.digest_algorithm)
        )
        print(
            "{}  Encryption Algorithm: {}".format(
                " " * indent, signer.encryption_algorithm
...
def main():
    ...
    parser.add_argument(
        "-s", "--save", dest="ext_file_path", help="Extract and save the PKCS #7"
    )
