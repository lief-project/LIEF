...
void create<MapItem>(py::module& m) {
  py::class_<MapItem, LIEF::Object> mapitem(m, "MapItem",
  ...
  mapitem
      .def_property_readonly(
          "type", static_cast<getter_t<MapItem::TYPES>>(&MapItem::type),
          "" RST_CLASS_REF(lief.DEX.MapItem.TYPES) " of the item")

      .def_property_readonly(
          "offset", static_cast<getter_t<uint32_t>>(&MapItem::offset),
          "Offset from the start of the file to the items in question")

      .def_property_readonly(
          "size", static_cast<getter_t<uint32_t>>(&MapItem::size),
          "count of the number of items to be found at the indicated offset")

      .def("__eq__", &MapItem::operator==)
      .def("__ne__", &MapItem::operator!=)
      .def("__hash__", [](const MapItem& mlist) { return Hash::hash(mlist); })

      .def("__str__", [](const MapItem& mlist) {
