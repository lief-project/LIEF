...
void create<SegmentCommand>(py::module& m) {
  ...
  try {
     ...
     */
    init_ref_iterator<SegmentCommand::it_relocations>(seg_cmd,
                                                      "it_relocations");
  ...
  seg_cmd.def(py::init<>())
      ...
      .def_property_readonly(
          "sections",
          static_cast<no_const_getter<SegmentCommand::it_sections>>(
              &SegmentCommand::sections),
          "Segment's sections")

      .def_property_readonly(
          "relocations",
          static_cast<no_const_getter<SegmentCommand::it_relocations>>(
              &SegmentCommand::relocations),
          "Segment's relocations")

      .def_property_readonly(
          "index", &SegmentCommand::index,
      ...
      .def_property(
          ...
          static_cast<setter_t<SegmentCommand::content_t>>(
              &SegmentCommand::content),
          "Segment's content")
      ...
      .def("has",
           static_cast<bool (SegmentCommand::*)(const Section&) const>(
               &SegmentCommand::has),
           "Check if the given " RST_CLASS_REF(
               lief.MachO.Section) " belongs to the current segment",
           "section"_a)

      .def("has_section",
           static_cast<bool (SegmentCommand::*)(const std::string&) const>(
               &SegmentCommand::has_section),
           "Check if the given section name belongs to the current segment",
           "section_name"_a)

      .def("add_section",
           static_cast<Section& (SegmentCommand::*)(const Section&)>(
               &SegmentCommand::add_section),
           "", "section"_a, py::return_value_policy::reference)
      ...
      .def("__str__", [](const SegmentCommand& segment) {
