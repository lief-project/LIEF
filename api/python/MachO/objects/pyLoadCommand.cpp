...
void create<LoadCommand>(py::module& m) {
  py::class_<LoadCommand, LIEF::Object>(
      m, "LoadCommand", "Based class for the Mach-O load commands")
      .def(py::init<>())
      ...
      .def_property("size", static_cast<getter_t<uint32_t>>(&LoadCommand::size),
                    static_cast<setter_t<uint32_t>>(&LoadCommand::size),
                    "Size of the command (should be greather than "
                    "``sizeof(load_command)``)")
      ...
      .def("__str__", [](const LoadCommand& command) {
