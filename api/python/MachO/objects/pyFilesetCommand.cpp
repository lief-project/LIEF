...
void create<FilesetCommand>(py::module& m) {
  py::class_<FilesetCommand, LoadCommand>(
      m, "FilesetCommand",
      ...
      .def_property_readonly(
          ...
          "Return the " RST_CLASS_REF(
              lief.MachO.Binary) " object associated with the entry",
          py::return_value_policy::reference)

      .def("__eq__", &FilesetCommand::operator==)
      .def("__ne__", &FilesetCommand::operator!=)
      .def("__hash__",
           [](const FilesetCommand& main) { return Hash::hash(main); })

      .def("__str__", [](const FilesetCommand& main) {
