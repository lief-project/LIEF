...
void create<Parser>(py::module& m) {
  // Parser (Parser)
  m.def("parse",
        static_cast<std::unique_ptr<FatBinary> (*)(const std::string&,
                                                   const ParserConfig&)>(
    ...
    One can configure the parsing with the ``config`` parameter. See :class:`~lief.MachO.ParserConfig`,
        ...
        "filename"_a, "config"_a = ParserConfig::deep(),
        py::return_value_policy::take_ownership);

  m.def("parse",
        static_cast<std::unique_ptr<FatBinary> (*)(
            const std::vector<uint8_t>&, const std::string&,
            const ParserConfig&)>(&LIEF::MachO::Parser::parse),
        R"delim(
    Parse the given binary (from raw bytes) and return a :class:`~lief.MachO.FatBinary` object

    One can configure the parsing with the ``config`` parameter. See :class:`~lief.MachO.ParserConfig`
    )delim",
        "raw"_a, "name"_a = "", "config"_a = ParserConfig::quick(),
        py::return_value_policy::take_ownership);
  ...
  m.def(
      ...
      "io"_a, "name"_a = "", "config"_a = ParserConfig::quick(),
      py::return_value_policy::take_ownership);
}
