...
LIEF::dwarf::editor::Type& TypeEngine::add_type(const BinaryNinja::Type& type) {
  ...
  switch (class_type) {
    ...
    case BoolTypeClass:
    {
      ...
      std::unique_ptr<dw::editor::BaseType> btype =
          unit_.create_base_type(name_str, type.GetWidth(),
                                 dw::editor::BaseType::ENCODING::BOOLEAN);

      return *mapping_.insert({name_str, std::move(btype)}).first->second;
    }
    ...
    case IntegerTypeClass:
    {
      ...
      std::unique_ptr<dw::editor::BaseType> btype =
          unit_.create_base_type(name_str, type.GetWidth(),
                                 type.IsSigned() ?
                                     dw::editor::BaseType::ENCODING::SIGNED :
    ...
    case FloatTypeClass:
    {
      ...
      std::unique_ptr<dw::editor::BaseType> float_type =
          unit_.create_base_type(name_str, type.GetWidth(),
                                 dw::editor::BaseType::ENCODING::FLOAT);
      return *mapping_.insert({name_str, std::move(float_type)}).first->second;
    }
    ...
    case PointerTypeClass:
    {
      ...
      std::unique_ptr<dw::editor::PointerType> pointer =
          child_pointer.pointer_to();
      return *mapping_.insert({name_str, std::move(pointer)}).first->second;
    }
    ...
    case StructureTypeClass:
    {
      ...
      if (bn_struct == nullptr) {
        ...
        return *mapping_.insert({name_str, unit_.create_void_type()})
                    .first->second;
      }
      ...
      switch (bn_struct->GetStructureType()) {
        case ClassStructureType:
          struct_type =
              unit_.create_structure(struct_name,
                                     dw::editor::StructType::TYPE::CLASS);
          break;

        case UnionStructureType:
          struct_type =
              unit_.create_structure(struct_name,
                                     dw::editor::StructType::TYPE::UNION);
          break;

        case StructStructureType:
          struct_type =
              unit_.create_structure(struct_name,
                                     dw::editor::StructType::TYPE::STRUCT);
          break;
      }
    ...
    case EnumerationTypeClass:
    {
      ...
      switch (type.GetWidth()) {
        case sizeof(uint8_t):
        {
          enum_type->set_underlying_type(*unit_.create_base_type(
      ...
      return *mapping_.insert({name_str, std::move(enum_type)}).first->second;
    }
    ...
    case NamedTypeReferenceClass:
    {
      ...
      if (!alias) {
        ...
        std::unique_ptr<dw::editor::Type> generic =
            unit_.create_generic_type(name_str);
        return *mapping_.insert({name_str, std::move(generic)}).first->second;
      }
      ...
      BN_DEBUG("alias->GetTypeName(): {}",
               binaryninja::to_string(alias->GetTypeName()));
    ...
    case ArrayTypeClass:
    {
      ...
      if (api_compat::as_bool(element_type)) {
        dw::editor::Type& dw_element_type =
            add_type(api_compat::get_type(element_type));
      ...
      std::unique_ptr<dw::editor::Type> dw_element_type =
          unit_.create_generic_type("element_t");
    ...
    case FunctionTypeClass:
    {
      std::string func_type_name =
          name_str.empty() ? fmt::format("__function_{}__", ++func_id_) : name_str;
      ...
      return *mapping_.insert({name_str, std::move(func_type)}).first->second;
    }
...
void TypeEngine::add_member(const BinaryNinja::StructureMember& member,
  ...
  if (member.bitWidth > 0) {
    S.add_bitfield(member.name, add_type(api_compat::get_type(member_type)),
                   member.bitWidth, member.offset * 8 + member.bitPosition);
    return;
  }

  S.add_member(member.name, add_type(api_compat::get_type(member_type)),
               member.offset);
#else
  auto member_type = member.type;
  S.add_member(member.name, add_type(api_compat::get_type(member_type)),
