...
Ref<Type> TypeBuilder::get_or_create(const std::string& name) {
  ...
  if (name == "IMAGE_ARM64EC_METADATA_V1") {
    ...
    struct_builder.AddMember(u32(), "Version")
    ...
    return create_struct(*S, "_" + format_type(name), format_type(name));
  }
  ...
  if (name == "IMAGE_ARM64EC_METADATA_V2") {
    ...
    builder.AddMember(RVA(), "AuxDelayloadIAT")
  ...
  if (name == "IMAGE_GUARD") {
    ...
    F.AddMemberWithValue("IMAGE_GUARD_CF_INSTRUMENTED",
                         (uint32_t)IMAGE_GUARD::CF_INSTRUMENTED);

    F.AddMemberWithValue("IMAGE_GUARD_CFW_INSTRUMENTED",
                         (uint32_t)IMAGE_GUARD::CFW_INSTRUMENTED);

    F.AddMemberWithValue("IMAGE_GUARD_CF_FUNCTION_TABLE_PRESENT",
                         (uint32_t)IMAGE_GUARD::CF_FUNCTION_TABLE_PRESENT);

    F.AddMemberWithValue("IMAGE_GUARD_SECURITY_COOKIE_UNUSED",
                         (uint32_t)IMAGE_GUARD::SECURITY_COOKIE_UNUSED);

    F.AddMemberWithValue("IMAGE_GUARD_PROTECT_DELAYLOAD_IAT",
                         (uint32_t)IMAGE_GUARD::PROTECT_DELAYLOAD_IAT);

    F.AddMemberWithValue("IMAGE_GUARD_DELAYLOAD_IAT_IN_ITS_OWN_SECTION",
                         (uint32_t)IMAGE_GUARD::DELAYLOAD_IAT_IN_ITS_OWN_SECTION);

    F.AddMemberWithValue(
        "IMAGE_GUARD_CF_EXPORT_SUPPRESSION_INFO_PRESENT",
        (uint32_t)IMAGE_GUARD::CF_EXPORT_SUPPRESSION_INFO_PRESENT
    );

    F.AddMemberWithValue("IMAGE_GUARD_CF_ENABLE_EXPORT_SUPPRESSION",
                         (uint32_t)IMAGE_GUARD::CF_ENABLE_EXPORT_SUPPRESSION);

    F.AddMemberWithValue("IMAGE_GUARD_CF_LONGJUMP_TABLE_PRESENT",
                         (uint32_t)IMAGE_GUARD::CF_LONGJUMP_TABLE_PRESENT);

    F.AddMemberWithValue("IMAGE_GUARD_RF_INSTRUMENTED",
                         (uint32_t)IMAGE_GUARD::RF_INSTRUMENTED);

    F.AddMemberWithValue("IMAGE_GUARD_RF_ENABLE",
                         (uint32_t)IMAGE_GUARD::RF_ENABLE);

    F.AddMemberWithValue("IMAGE_GUARD_RF_STRICT",
                         (uint32_t)IMAGE_GUARD::RF_STRICT);

    F.AddMemberWithValue("IMAGE_GUARD_RETPOLINE_PRESENT",
                         (uint32_t)IMAGE_GUARD::RETPOLINE_PRESENT);

    F.AddMemberWithValue("IMAGE_GUARD_EH_CONTINUATION_TABLE_PRESENT",
                         (uint32_t)IMAGE_GUARD::EH_CONTINUATION_TABLE_PRESENT);

    F.AddMemberWithValue("IMAGE_GUARD_XFG_ENABLED",
                         (uint32_t)IMAGE_GUARD::XFG_ENABLED);

    F.AddMemberWithValue("IMAGE_GUARD_CASTGUARD_PRESENT",
                         (uint32_t)IMAGE_GUARD::CASTGUARD_PRESENT);

    F.AddMemberWithValue("IMAGE_GUARD_MEMCPY_PRESENT",
                         (uint32_t)IMAGE_GUARD::MEMCPY_PRESENT);

    Ref<Enumeration> enum_ = F.Finalize();
    Ref<Type> enum_ty = Type::EnumerationType(default_arch, enum_, /*width=*/4,
                                              /*isSigned=*/false);
  ...
  if (name == "IMAGE_ARM64EC_CODE_RANGE_ENTRY_POINT") {
    StructureBuilder builder;
    builder.AddMember(RVA(), "StartRva")
  ...
  if (name == "IMAGE_ARM64EC_METADATA_REDIRECTION") {
    StructureBuilder builder;
    builder.AddMember(RVA(), "From").AddMember(RVA(), "To");
  ...
  if (name == "IMAGE_ARM64_RUNTIME_FUNCTION_ENTRY") {
    StructureBuilder builder;
    builder.AddMember(RVA(), "BeginAddress").AddMember(RVA(), "UnwindData");
...
#if BN_BITFIELD_SUPPORT
    builder.SetPacked(true)
        .AddMemberAtBitOffset(u32(), "FunctionLength", /*bitOffset=*/0,
                              /*bitWidth=*/18)
        .AddMemberAtBitOffset(u32(), "Version", /*bitOffset=*/18, /*bitWidth=*/2)
        .AddMemberAtBitOffset(u32(), "ExceptionDataPresent", /*bitOffset=*/20,
                              /*bitWidth=*/1)
        .AddMemberAtBitOffset(u32(), "EpilogInHeader", /*bitOffset=*/21,
                              /*bitWidth=*/1)
        .AddMemberAtBitOffset(u32(), "EpilogCount", /*bitOffset=*/22,
                              /*bitWidth=*/5)
        .AddMemberAtBitOffset(u32(), "CodeWords", /*bitOffset=*/27,
                              /*bitWidth=*/5);
#else
    builder.AddMember(u32(), "HeaderData");
#endif
  ...
  if (name == "IMAGE_ARM64_RUNTIME_FUNCTION_EXTENDED_ENTRY") {
    StructureBuilder builder;
    builder
        .AddMember(get_or_create("IMAGE_ARM64_RUNTIME_FUNCTION_ENTRY_XDATA"),
                   "base")
        .AddMember(
            get_or_create("IMAGE_ARM64_RUNTIME_FUNCTION_ENTRY_XDATA_EXTENDED"),
            "extension"
        );
