...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < header.numberof_sections; ++i) {
    ...
    fprintf(stdout,
            ...
            "%.6f "
            "\n",
            section->name, ELF_SECTION_TYPES_to_string(section->type),
            section->virtual_address, section->size, section->offset,
            section->link, section->alignment, section->entry_size,
            section->entropy);
    if (section->size > 3) {
  ...
  for (i = 0; dynamic_symbols[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            "%-3s "
            "\n",
            symbol->name, ELF_SYMBOL_TYPES_to_string(symbol->type),
            SYMBOL_BINDINGS_to_string(symbol->binding), symbol->other,
            symbol->shndx, symbol->value, symbol->size, import_export);
  }
  ...
  for (i = 0; segments[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            " "
            "\n",
            SEGMENT_TYPES_to_string(segment->type), segment->flags,
            segment->virtual_address, segment->virtual_size, segment->offset,
            segment->size, segment->alignment);
    if (segment->size > 3) {
      fprintf(stdout, "content[0..3]: %02x %02x %02x\n", segment->content[0],
              segment->content[1], segment->content[2]);
    }
  }
    ...
    switch (entry->tag) {
      case LIEF_ELF_DT_NEEDED: {
        ...
        fprintf(stdout,
                ...
                "%-20s "
                "\n",
                DYNAMIC_TAGS_to_string(e->tag), e->value, e->name);
        break;
      }
      case LIEF_ELF_DT_SONAME: {
        Elf_DynamicEntry_SharedObject_t* e =
            (Elf_DynamicEntry_SharedObject_t*)entry;
        fprintf(stdout,
                ""
                "%-20s "
                "0x%010" PRIx64
      ...
      case LIEF_ELF_DT_RPATH: {
        ...
        fprintf(stdout,
                ...
                "%-20s "
                "\n",
                DYNAMIC_TAGS_to_string(e->tag), e->value, e->rpath);
        break;
      }
      ...
      case LIEF_ELF_DT_RUNPATH: {
        ...
        fprintf(stdout,
                ...
                "%-20s "
                "\n",
                DYNAMIC_TAGS_to_string(e->tag), e->value, e->runpath);
        break;
      }
      ...
      case LIEF_ELF_DT_FLAGS: {
        ...
        fprintf(stdout,
                ...
                "%-20s "
                "0x%010" PRIx64 " ",
                DYNAMIC_TAGS_to_string(e->tag), e->value);
      ...
      default: {
        fprintf(stdout,
                ...
                " "
                "\n",
                DYNAMIC_TAGS_to_string(entry->tag), entry->value);
        break;
      }
    }
  }
