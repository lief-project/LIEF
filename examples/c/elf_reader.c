...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < header.numberof_sections && sections[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            "%.6f "
            "\n",
            section->name, section->type, section->virtual_address, section->size,
            section->offset, section->link, section->alignment,
            section->entry_size, section->entropy);
  ...
  for (i = 0; dynamic_symbols[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            "%-3s "
            "\n",
            symbol->name, symbol->type, symbol->binding, symbol->other,
  ...
  for (i = 0; segments[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            "0x%06" PRIx64 " "
            "\n",
            segment->type, segment->flags, segment->virtual_address,
            segment->virtual_size, segment->offset, segment->size,
            segment->alignment);
