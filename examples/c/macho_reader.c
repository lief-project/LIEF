...
void print_binary(Macho_Binary_t* binary) {
  ...
  for (size_t i = 0; commands[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            " "
            "\n",
            LOAD_COMMAND_TYPES_to_string(command->command), command->size,
            command->offset);
    if (command->size > 3) {
  ...
  for (size_t i = 0; segments[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            " "
            "\n",
            segment->name, segment->virtual_address, segment->virtual_size,
            segment->file_size, segment->file_offset, segment->max_protection,
            segment->init_protection, segment->numberof_sections,
            segment->flags);
  ...
  for (size_t i = 0; sections[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            section->numberof_relocations, section->flags,
            MACHO_SECTION_TYPES_to_string(section->type), section->reserved1,
            section->reserved2, section->reserved3, section->virtual_address,
            section->offset, section->size, section->entropy);
    if (section->size > 3) {
      fprintf(stdout, "content[0..3]: %02x %02x %02x\n", section->content[0],
  ...
  for (size_t i = 0; symbols[i] != NULL; ++i) {
    ...
    fprintf(stdout,
            ...
            " "
            "\n",
            symbol->name, (uint32_t)symbol->type,
            (uint32_t)symbol->numberof_sections, symbol->description,
            symbol->value);
  }
}
