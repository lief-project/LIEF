...
Similar to the {ref}`BinaryNinja plugin <plugins-binaryninja>`, LIEF can be used
as a Ghidra plugin.
...
First, download the LIEF plugin package from {sub-ref}`lief-plugin-url` or
from the packages listed above. Then, follow the official procedure to install
extensions: [GhidraDocs/GettingStarted.md](https://github.com/NationalSecurityAgency/ghidra/blob/175cf9488722df3f8a718236c9e2c0ebfcd8cdb7/GhidraDocs/GettingStarted.md#extensions)

:::{admonition} Shared Library
:class: warning

In addition to the previously installed package, the plugins require the
installation of `LIEF.dll`, `libLIEF.dylib`, or `libLIEF.so` next to the
extension directory. You can download this library from the following list:
...
or from here: {sub-ref}`lief-sdk-url`. Some plugins (like the {ref}`DWARF Plugin <plugins-ghidra-dwarf>`)
require the extended version, which can be downloaded from {sub-ref}`lief-extended-url`.

Given this shared library, you must copy it into the Ghidra settings
...
`CodeBrowser` tool on a binary and ensuring that you can see the LIEF
...
This means that `libLIEF.so`, `libLIEF.dylib`, or `LIEF.dll` is not
correctly installed in the `Extensions/` directory. The details of the error
provide the expected path(s).
