...
<div class="card shadow-light">
  <div class="card-body">
    <h3 class="card-title"><i class="fab fa-github text-darkblue  mr-2"></i>Get Access</h3>
...
To access the extended version, you must **oauth-login** with **GitHub** here: {sub-ref}`lief-extended-url`.
...
please contact {sub-ref}`lief-extended-email`.
...
With the C++/Rust SDK, you can call {sub-ref}`lief-extended-version-info` to
...
:::{note}
LIEF is currently using LLVM {sub-ref}`lief-llvm-version`.
:::
