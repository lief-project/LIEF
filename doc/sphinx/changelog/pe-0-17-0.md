...
LIEF's PE functionality with its ELF and Mach-O counterparts.
If your codebase uses LIEF to process PE binaries, you may encounter issues when
...
1. Join our Discord server and ask your questions: {sub-ref}`lief-discord`
...
As described in issue {issue}`1043`, if a section name exceeds
...
You can access this COFF string using {sub-ref}`lief-pe-section-coff_string`, while
...
{sub-ref}`lief-pe-loadconfig`, regularly evolve with new Windows releases.
New Windows versions may introduce attributes for additional
functionality, such as Control Flow Guard. To manage these changes,
{sub-ref}`lief-pe-loadconfig` previously used a form of inheritance. For each new
attribute, a new class, such as `LoadConfigurationV12`, would inherit from
`LoadConfigurationV11`. This approach, though systematic, resulted in numerous
modifications -- especially in Rust -- raising questions about the decision to use
inheritance.

Starting with LIEF v0.17.0, {sub-ref}`lief-pe-loadconfig` has adopted a **flat**
representation. In this new model, version-dependent attributes are represented
with an `option<>`-like container.
...
Additionally, {sub-ref}`lief-pe-loadconfig` now provides an API that allows access to
the internal data associated with this structure:

- {sub-ref}`lief-pe-chpe_metadata`
- {sub-ref}`lief-pe-dynamic-relocation`
- {sub-ref}`lief-pe-enclave-configuration`
- {sub-ref}`lief-pe-volatile-metadata`
...
LIEF version 0.17.0 supports more types of debug entries and provides a
user-friendly API:

- {sub-ref}`lief-pe-vcfeature`
- {sub-ref}`lief-pe-fpo`
- {sub-ref}`lief-pe-exdllcharacteristics`
- {sub-ref}`lief-pe-pdbchecksum`

As described in the {ref}`Debug Modification <pe_debug_modification>`
section, you can modify debug entries to perform various actions:
...
Some interfaces exposed by the {sub-ref}`lief-pe-resource-manager` have been redesigned
to make them more consistent with their underlying representation in the PE
format.

For example, `lief.PE.LangCodeItem` has been renamed and refactored into
{sub-ref}`lief-pe-resourcestringtable`.

Resource-related enums have been re-scoped to avoid definition conflicts
between Windows headers and LIEF:
...
{sub-ref}`lief-pe-exceptioninfo` structure, which is specialized for x64 and
...
Given a {sub-ref}`lief-pe-binary` instance, one can iterate over these
{sub-ref}`lief-pe-exceptioninfo` entries using {sub-ref}`lief-pe-binary-exceptions`:
...
Thank you to [Eli Friedman](https://github.com/efriedma-quic) and [Martin Storsjö](https://github.com/mstorsjo)
for their work on reverse engineering this undocumented unwind opcode.
:::

LIEF provides a detailed API for the structures involved in PE
exceptions and stack unwinding.

:::{admonition} Performance Considerations
:class: warning

In-depth analysis of exception metadata can introduce significant overhead.
Therefore, {sub-ref}`lief-pe-parser-config-parse_exceptions` must be
**explicitly** enabled.
:::
...
ARM64X or ARM64EC: {sub-ref}`lief-pe-binary-is_arm64ec`, {sub-ref}`lief-pe-binary-is_arm64x`.

For an ARM64X binary, you can use {sub-ref}`lief-pe-binary-nested_pe_binary` to access the
embedded ARM64EC **representation** of the binary.

:::{admonition} Performance Considerations
:class: warning

Parsing a nested ARM64EC binary can introduce additional overhead compared to
the 0.16.x version.
Therefore, {sub-ref}`lief-pe-parser-config-parse_arm64x_binary` must be
**explicitly** enabled.
:::

## Other Changes

The workaround involving `undef.h` has been removed. This file was used to
`#undef` certain Windows defines that conflicted with LIEF enum definitions.
You can now safely include Windows headers alongside LIEF headers. If you
encounter any conflicts, please report the issue.
...
The *"PE from Scratch"* feature has been removed, as it had significant bugs
that often resulted in corrupted binaries. This feature will be reintroduced in
a future release. If you rely on this feature, please reach out.

```{eval-rst}

:List of changes:

  * Fixed missing original forwarded function name (:issue:`1166`)

  * |lief-pe-datadirectory| now exposes |lief-pe-datadirectory-content| to
