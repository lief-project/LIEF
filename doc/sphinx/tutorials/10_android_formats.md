...
documentation is available [here](https://source.android.com/devices/tech/dalvik/dex-format).
...
{{ literalinclude("../../code/python/tuto_android.py", "oat-symbols") }}
...
`invoke-virtual`, with optimized ones [^footnote-1]), starting with **Android N**,
...
There also exist tools [^footnote-4] [^footnote-5] [^footnote-6] to extract DEX files from OAT/VDEX files,
but extraction [^footnote-3] is either limited to OAT [^footnote-4] or VDEX [^footnote-5]. LIEF aims to
...
{class}`lief.OAT.Binary` class extends {sub-ref}`lief-elf-binary`:
...
{{ literalinclude("../../code/python/tuto_android.py", "oat-extract-dex") }}
...
{{ literalinclude("../../code/python/tuto_android.py", "dex-strings") }}
...
{{ literalinclude("../../code/python/tuto_android.py", "dex-source-files") }}
...
source filename: [source_file_idx](https://source.android.com/devices/tech/dalvik/dex-format#class-def-item).
...
{{ literalinclude("../../code/python/tuto_android.py", "dex-deobfuscate") }}
...
re-instantiating well-known classes [^footnote-2] at each boot, Android uses the ART
...
{{ literalinclude("../../code/python/tuto_android.py", "art-header") }}
...
[^footnote-2]: Usually those from the Android Framework.

[^footnote-3]: Other tools may have additional features like a disassembler or
    pseudo-code generation that are not covered in LIEF.

[^footnote-4]: Dextra by Jonathan Levin: <http://newandroidbook.com/tools/dextra.html>

[^footnote-5]: vdexExtractor by Anestis Bechtsoudis: <https://github.com/anestisb/vdexExtractor>

[^footnote-6]: smali by JesusFreke: <https://github.com/JesusFreke/smali/wiki/DeodexInstructions>
