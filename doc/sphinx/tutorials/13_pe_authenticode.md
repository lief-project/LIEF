...
types, as defined in the official documentation [^footnote-1].
...
to sponsorship from the [CERT Gouvernemental of Luxembourg](https://www.govcert.lu/en/),
we have refactored the Authenticode parser [^footnote-2] and implemented signature
...
the {sub-ref}`lief-pe-binary-signatures` attribute [^footnote-3]:

{{ literalinclude("../../code/python/tuto_authenticode.py", "signatures") }}
...
{sub-ref}`lief-pe-signature` object, which mirrors the PKCS #7 container and
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "signature-info") }}
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "authentihash") }}
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "raw-der") }}
...
The [pe_authenticode_reader.py](https://github.com/lief-project/LIEF/blob/main/api/python/examples/pe_authenticode_reader.py)
script in the [examples/](https://github.com/lief-project/LIEF/tree/main/api/python/examples) directory
...
{sub-ref}`lief-pe-binary-verify_signature` method. This method returns
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "verify") }}
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "verify-detached", emphasize_lines="1,4") }}
...
described in the RFCs and official Authenticode documentation [^footnote-4].
...
{sub-ref}`lief-pe-signature-check`.
:::
...
{{ literalinclude("../../code/python/tuto_authenticode.py", "is-trusted-bundle") }}

{{ literalinclude("../../code/python/tuto_authenticode.py", "is-trusted-list") }}
...
[mbedtls](https://github.com/Mbed-TLS/mbedtls), which provides the following
...
{{ literalinclude("../../code/cpp/tuto_authenticode.cpp", "ios-main") }}
...
- Minimizing dependencies so that the static version of LIEF does not require external libraries [^footnote-5].
...
Bits blog post [^footnote-6]. For Authenticode techniques used by Dropbox, refer to the
Microsoft website [^footnote-7]. If you are interested in how PKCS #7 integrity works,
refer to *Manually verify PKCS#7 signed data with OpenSSL* [^footnote-8].

```{rubric} References
```

[^footnote-1]: <http://download.microsoft.com/download/9/c/5/9c5b2167-8017-4bae-9fde-d599bac8184a/Authenticode_PE.docx>

[^footnote-2]: Which is now exception-free.

[^footnote-3]: This tutorial uses the Python API, but the C++ API is very similar.

[^footnote-4]: See [src/PE/signature/Signature.cpp - check()](https://github.com/lief-project/LIEF/tree/master/src/PE/signature/Signature.cpp) for the implementation.

[^footnote-5]: Except for the C/C++ STL.

[^footnote-6]: <https://blog.trailofbits.com/2020/05/27/verifying-windows-binaries-without-windows/>

[^footnote-7]: <https://docs.microsoft.com/en-us/archive/blogs/ieinternals/caveats-for-authenticode-code-signing>

[^footnote-8]: <http://qistoph.blogspot.com/2012/01/manual-verify-pkcs7-signed-data-with.html>

```{rubric} API
```

- {sub-ref}`lief-pe-binary-verify_signature`
...
- {sub-ref}`lief-pe-binary-signatures`
- {sub-ref}`lief-pe-signature`
- {class}`lief.PE.x509`
- {class}`lief.PE.ContentInfo`
