...
[@ikoz](https://twitter.com/ikoz) in the post "[Using Frida on Android without root](https://koz.io/using-frida-on-android-without-root/)". In this tutorial,
...
[Phrack](http://phrack.org/issues/61/8.html) [^footnote-1], and LIEF provides a
user-friendly API [^footnote-2] to perform it.
...
`readelf` (Unix) or with [elf_reader.py](https://github.com/lief-project/LIEF/blob/main/api/python/examples/elf_reader.py) (Linux, Windows, OSX):
...
{{ literalinclude("../../code/python/tuto_frida.py", "inject", prepend="import lief") }}
...
the `lib/` directory of the APK only if [^footnote-4]:
...
<p class="center"><a class="reference external" href="https://github.com/frida/frida-core/blob/289a08b237eeab1fb8ec3e2f41ed726de44b5d66/lib/gadget/gadget.vala#L500-L509">lib/gadget/gadget.vala</a></p>
...
[^footnote-1]: Note that LIEF **does not** modify the {attr}`~lief.ELF.DynamicEntry.TAG.DEBUG` entry ...

[^footnote-2]: Modifying the ELF Dynamic section is not as simple as the API might suggest.

[^footnote-3]: In the ELF format, these are located in the {attr}`~lief.ELF.DynamicEntry.TAG.INIT_ARRAY` or {attr}`~lief.ELF.DynamicEntry.TAG.INIT` entries.

[^footnote-4]: For those interested, checks are performed in the `framework_base/core/jni/com_android_internal_content_NativeLibraryHelper.cpp` file.
