...
[tutorials repository](https://github.com/lief-project/tutorials/tree/master/11_macho_modification).
...
binary. Export information is stored in a [trie](https://en.wikipedia.org/wiki/Trie)
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "rebase-opcodes", prepend="import lief") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "relocations") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "bind-opcodes") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "export-trie") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "export-info") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "remove-signature") }}

## Code Injection with Shared Libraries

As explained in the talk on format modification [^footnote-1], one way to inject code
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "add-library") }}
...
{{ literalinclude("../../code/python/tuto_macho_mod.py", "add-section") }}

We can then change the entry point by setting the
{attr}`lief.MachO.MainCommand.entrypoint` attribute:

{{ literalinclude("../../code/python/tuto_macho_mod.py", "set-entrypoint") }}

Finally, we remove the signature and reconstruct the binary:

{{ literalinclude("../../code/python/tuto_macho_mod.py", "remove-sig-write") }}
...
You can also check other tools such as optool [^footnote-2] or insert_dylib [^footnote-3].

```{rubric} References
```

[^footnote-1]: <https://www.romainthomas.fr/publication/static-instrumentation/>

[^footnote-2]: <https://github.com/alexzielenski/optool>

[^footnote-3]: <https://github.com/Tyilo/insert_dylib>
