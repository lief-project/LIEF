...
to it, whereas executables should not expose functions [^footnote-1].
...
In this case, the **check** function is located at address: `0x72A` [^footnote-2].

Now that we have identified the address, we can export it as a named function:
`check_found`.

{{ literalinclude("../../code/python/tuto_elf_bin2lib.py", "export") }}
...
If `dlopen` returns an error, please read [the following section about glibc >= 2.29](#glibc229).
...
{title-reference}`dlopen` function:
...
reason is that it [does not seem trivial to support](https://sourceware.org/bugzilla/show_bug.cgi?id=11754) all possible use
cases (issues with certain relocations and ELF constructors).

These glibc versions now [implement a check](https://patchwork.ozlabs.org/project/glibc/patch/20190312130235.8E82C89CE49C@oldenburg2.str.redhat.com/)
...
{{ literalinclude("../../code/python/tuto_elf_bin2lib.py", "remove-pie", prepend="import lief", emphasize_lines="5") }}
...
investigate the PE and Mach-O cases [^footnote-3].

```{rubric} Notes
```

[^footnote-1]: Some functions can be exported by the linker, such as `_init`.

[^footnote-2]: The mapped virtual address will be `BASE + 0x72A`, where `BASE` is randomly chosen by ASLR.

[^footnote-3]: In macOS, all executables are compiled with the PIE flag.
