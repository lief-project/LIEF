...
ELF core [^footnote-1] files provide information about the CPU state and memory state of
...
blog post: [Anatomy of an ELF core file](https://www.gabriel.urdhr.fr/2015/05/29/core-file/).
...
Specifically, in C++, we must downcast using the {title-reference}`classof` function:

{{ literalinclude("../../code/cpp/tuto_coredump.cpp", "corefile-downcast") }}

This is roughly equivalent in Python to:

{{ literalinclude("../../code/python/tuto_coredump.py", "corefile-check") }}
:::
...
{{ literalinclude("../../code/python/tuto_coredump.py", "iter-files") }}
...
{{ literalinclude("../../code/python/tuto_coredump.py", "registers") }}
...
{{ literalinclude("../../code/python/tuto_coredump.py", "set-register") }}
...
the [Triton](https://triton.quarkslab.com/) API:
...
[^footnote-1]: <https://www.gabriel.urdhr.fr/2015/05/29/core-file/>
